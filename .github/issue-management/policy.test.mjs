...
test('removes reserved labels from Issues before validation', async (t) => {
  ...
  assert.deepEqual(requests, [
    {
      url: 'https://api.github.com/repos/singula-ai/alego/issues/42/labels/kind%2Fbug-fix',
      method: 'DELETE',
    },
    {
      url: 'https://api.github.com/repos/singula-ai/alego/issues/42/labels/bug-fix',
      method: 'DELETE',
    },
  ])
})
...
test('deletes a stale audit comment after repairing its only violation', async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      { path: '/repos/singula-ai/alego/issues/42', method: 'GET' },
      { path: '/graphql', method: 'POST' },
      {
        path: '/repos/singula-ai/alego/issues/42/labels/kind%2Fbug-fix',
        method: 'DELETE',
      },
      {
        path: '/repos/singula-ai/alego/issues/42/comments?per_page=100',
        method: 'GET',
      },
      {
        path: '/repos/singula-ai/alego/issues/comments/99',
        method: 'DELETE',
      },
    ],
  )
