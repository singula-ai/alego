"""Locate and execute the bundled alego CLI shipped with the Python SDK runtime.
...
- **exe (production)**: single-file Node executables named
  ``alego-sdk-runtime-<platform>-<arch>`` for Linux/macOS and an
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@singula-ai/alego/lib/bin.js`` on a
...
Both carriers execute the same alego command grammar. The Python SDK selects the
``sdk`` profile and requires an explicit Harness home; the installed ``alego``
console command requires ``ALEGO_HOME`` for the same reason.
"""
...
PACKAGE_METADATA_FILENAME = "alego-runtime.json"

RUNTIME_MODE_ENV_VAR = "ALEGO_RUNTIME_MODE"
...
_EXE_ACQUISITION_HINT = (
    ...
    "alego checkout, or install the matching `alego-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"alego-runtime-bin is missing {metadata}")
    return root
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"alego-sdk-runtime-{tag}{extension}"
    if not path.is_file():
        raise FileNotFoundError(
            f"alego-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    ripgrep = (
    ...
    if not ripgrep.is_file():
        raise FileNotFoundError(
            f"alego-runtime-bin is missing the ripgrep sidecar at {ripgrep}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"alego-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    ``ALEGO_RUNTIME_MODE`` environment variable (``exe`` | ``node``), then
    ...
    raise ValueError(
        f"unsupported Alego runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if (
        ...
        raise FileNotFoundError(
            "no bundled Alego SDK runtime exists for this platform "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in an alego "
...
def main() -> None:
    """Launch the CLI with explicit ALEGO_HOME; wait on Windows, replace the process on POSIX."""
    if not os.environ.get("ALEGO_HOME", "").strip():
        print(
            "alego: the Python runtime command requires an explicit ALEGO_HOME; "
            "it never uses ~/.alego implicitly",
            file=sys.stderr,
        )
        raise SystemExit(2)
