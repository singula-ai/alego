"""Locate the bundled Alego SDK runtime shipped with this package.
...
- **exe (production)**: single-file Node executables named
  ``alego-jsonrpc-agent-pkg-<platform>-<arch>`` (platform in {linux, macos}, arch in
...
the client SDK injects via ``$ALEGO_CORDIS_CONFIG`` for zero-config runs — the
...
def bundled_default_config_path() -> Path:
    ...
    The client SDK injects this path via ``$ALEGO_CORDIS_CONFIG`` when the caller
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"alego-runtime-bin is missing the default runtime config at {path}"
        )
    return path
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"alego-jsonrpc-agent-pkg-{tag}"
    if not path.is_file():
        raise FileNotFoundError(
...
def _current_platform_tag() -> str:
    ...
    if plat is None or arch is None:
        raise FileNotFoundError(
            "no bundled alego-jsonrpc-agent executable exists for this platform "
