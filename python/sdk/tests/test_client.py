...
from alego import Alego, HarnessClient, HarnessConfig, Notification, RunResult, SdkProtocolError
from alego.errors import JsonRpcError
...
json.dump({
    ...
    "ALEGO_CWD": os.environ.get("ALEGO_CWD"),
    "ALEGO_SESSION_ROOT": os.environ.get("ALEGO_SESSION_ROOT"),
    "ALEGO_CORDIS_CONFIG": os.environ.get("ALEGO_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["ALEGO_CWD"] is None
    assert dumped_env["ALEGO_SESSION_ROOT"] is None
    assert dumped_env["ALEGO_CORDIS_CONFIG"] is None
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("ALEGO_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with Alego(_launch_args=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-alego"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(_launch_args=(sys.executable, str(script))) as client:
        ...
        assert init.serverInfo.name == "fake-alego"
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from alego import AlegoConfig, Session
    ...
    assert "profile" not in inspect.signature(Alego.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in AlegoConfig.__dataclass_fields__
    assert "max_tokens" in AlegoConfig.__dataclass_fields__
    assert "reasoning_effort" in AlegoConfig.__dataclass_fields__
    ...
    assert {"alego_bin", "profile", "patches", "alego_home"} <= set(
        AlegoConfig.__dataclass_fields__
    )
    assert {"alego_bin", "profile", "patches", "alego_home"} <= set(
        HarnessConfig.__dataclass_fields__
    )
    assert "initialize_timeout_seconds" in AlegoConfig.__dataclass_fields__
    assert "initialize_timeout_seconds" in HarnessConfig.__dataclass_fields__
    assert AlegoConfig().initialize_timeout_seconds == 30.0
    ...
    for removed in ("cordis", "session_root", "runtime_bin", "bridge_bin", "launch_args_override"):
        assert removed not in AlegoConfig.__dataclass_fields__
...
def _install_fake_bundled_alego(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Install a fake runtime package that records alego argv and serves lifecycle calls."""
    runtime = tmp_path / "alego.py"
    runtime.write_text(
        """
import json
import os
...
json.dump({
    "argv": sys.argv[1:],
    "ALEGO_HOME": os.environ.get("ALEGO_HOME"),
    "ALEGO_CORDIS_CONFIG": os.environ.get("ALEGO_CORDIS_CONFIG"),
...
""".strip()
    ...
    module_dir = tmp_path / "alego_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "alego_runtime", raising=False)


def test_client_default_launch_uses_bundled_alego_sdk_profile_and_explicit_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    home = tmp_path / "home"
    ...
    patch.write_text("[]\n")
    _install_fake_bundled_alego(tmp_path, monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ALEGO_HOME", str(tmp_path / "ambient-home"))
    ...
    with HarnessClient(HarnessConfig(
        ...
        alego_home=str(home),
        env={"ENV_DUMP": str(env_dump), "ALEGO_HOME": str(tmp_path / "env-home")},
    )) as client:
    ...
    assert json.loads(env_dump.read_text()) == {
        ...
        "ALEGO_HOME": str(home),
        "ALEGO_CORDIS_CONFIG": None,
    }


def test_client_accepts_explicit_environment_alego_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    ...
    with HarnessClient(
        HarnessConfig(profile="custom", env={"ENV_DUMP": str(env_dump), "ALEGO_HOME": str(home)})
    ) as client:
...
def test_client_rejects_an_implicit_default_alego_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _install_fake_bundled_alego(tmp_path, monkeypatch)
    monkeypatch.delenv("ALEGO_HOME", raising=False)

    with pytest.raises(ValueError, match="explicit alego_home or non-empty ALEGO_HOME"):
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install alego-runtime-bin"):
        HarnessClient(HarnessConfig(alego_home="/explicit/home")).start()
