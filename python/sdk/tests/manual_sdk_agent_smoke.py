"""Drive the repo-source alego SDK profile through the SDK and a keyless mock SSE server.
...
def run_smoke(repo_root: Path, keep_sessions: bool) -> None:
    alego_home = Path(tempfile.mkdtemp(prefix="alego-sdk-smoke-home-"))
    session_root = alego_home / "sessions"
    ...
    if keep_sessions:
        print(f"kept_alego_home={alego_home}")
    else:
        shutil.rmtree(alego_home)
        print("removed temporary alego home")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the alego checkout.",
    )
