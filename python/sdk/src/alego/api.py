...
class Alego:
    """Reusable synchronous SDK for running Alego agent turns.
    ...
    def __init__(self, config: AlegoConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either AlegoConfig or keyword options, not both")
        self.config = config or AlegoConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["ALEGO_SESSION_ROOT"] = self.config.session_root
        if self.config.cordis is not None:
            env["ALEGO_CORDIS_CONFIG"] = self.config.cordis
        env["ALEGO_CWD"] = cwd
...
class Session:
    def __init__(self, harness: Alego, session_id: str) -> None:
