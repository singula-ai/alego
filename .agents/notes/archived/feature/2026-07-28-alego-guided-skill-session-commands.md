# Agent Note: `alego migrate`/`alego upgrade` seed the first turn with a skill

Status: implemented
Archived: 2026-08-03

English | [中文](2026-07-28-alego-guided-skill-session-commands.zh.md)

## Problem

Two recurring flows begin with the user manually invoking one skill and answering its questions: migrating from another coding agent, and upgrading this checkout. Both require the user to know the skill exists and to type `/skill:alego-migrate` or `/skill:alego-upgrade` as the session's first turn. A dedicated entry command that drops the user straight into that guided session removes the discovery step.

## Decision

`alego migrate` and `alego upgrade` boot the ordinary TUI as a fresh session whose first turn auto-invokes a bundled skill (`alego-migrate`, `alego-upgrade`), exactly as if the user typed `/skill:<name>` and pressed Enter.
...
**Freshness is gated in the launcher, not the TUI.** `runSkillSession` always mints a fresh session and provides the slot only when `resumeSessionId === undefined`, so a later `alego --resume <id>` of that session is an ordinary TUI session with no re-injection. The TUI stays generic: it invokes whatever skill it is handed, once, at startup.

**`migrate`/`upgrade` take no default-surface options** (`upgrade` additionally carries the [experimental gate](2026-07-31-experimental-subcommand-gate.md)'s `--experimental`). They carry no `--resume`, `--config`, or `-p`; a guided fresh-session entry has nothing to resume or reconfigure. Any leaked default-surface option fails loud, matching the `web`/`meta` rejection pattern in the Commander adapter. The two modes share one `SkillSessionInvocation` discriminant (`mode: 'migrate' | 'upgrade'`); `bin.ts` maps the mode to `alego-${mode}`.

The `alego-migrate` skill is bundled under `skills/` (shipped through `ALEGO_BUNDLED_SKILL_DIR`, like `alego-upgrade`). It asks which source agent (opencode/pi/Claude Code/Codex) if unstated, then maps each capability — workspace instructions, personal overlay, skills, hooks, MCP, API/env — to its ALEGO equivalent, grounded in the actual repo surfaces (the `hooks-claude`/`hooks-codex` bridges, `~/.alego/{config.yaml,.env,AGENTS.md,skills/}`, `AGENTS.md`/`CLAUDE.md`, `mcporter`), and states plainly when a capability has no equivalent.
...
No keyless PTY snapshot: per the maintainer's scope call for this change, unit coverage plus interactive verification suffices, and the seed rides the already-snapshotted `/skill:` render path. Both commands were verified interactively in tmux from a scratch cwd: `alego migrate` loaded `alego-migrate` and asked which source agent; `alego upgrade` loaded `alego-upgrade`, which pulled in `alego-customize` and began checkout discovery.
...
**Seed a natural-language instruction ("use the alego-migrate skill…") instead of `/skill:<name>`.** Rejected here: the literal skill-invocation path renders the skill body into the first turn deterministically, identical to the manual command, rather than depending on the model choosing to load the skill.

**Support `--resume` on `migrate`/`upgrade`.** Rejected: these are one-shot guided entries. A resumed session is an ordinary TUI session reachable through the default surface's `alego --resume <id>`; re-injecting the skill on resume would duplicate the first turn.
