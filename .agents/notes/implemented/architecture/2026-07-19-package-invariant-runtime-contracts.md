...
The central `alego-invariants` service owns only configuration, registration uniqueness, child-fiber lifecycle, rollback, disposal, and package-attributed failure. It exposes no generic plugin-shape, service-shape, or startup-assertion helpers and imports no product package.
...
|---|---|
| `alego-session` | Strict sequence growth, turn/step enclosure, and same-step tool call/result pairing. |
| `alego-agent` | Non-repeating agent status and terminal disposal transitions. |
| `alego-scope` | Scoped-event carrier presence and routed-subject consistency. |
| `alego-agent-loop` | Explicitly marked, frozen loop request reconstruction from the session event log. |
| `alego-llm` | Stream block grammar, delta type/index matching, single usage, closed blocks, and terminal finish. |
| `alego-llm-retry` | Durable retry records identify the open turn's latest closed step, remain unique per step, increase monotonically, and stay within retry and non-negative timer bounds. |
| `alego-tools` | Monotonic pre/execute/post stages and immutable final execution/result snapshots. |
| `alego-system-prompt` | Authoritative assembly section, tool, and variable data constraints. |
| `alego-compaction` | Compaction start/summary/end pairing, range endpoints, token counts, and successful-summary presence. |
| `alego-hook-protocol` | Hook invocation/result correlation, dialect, identity, and duration constraints. |
| `alego-sandbox-policy` | Durable `sandbox/mode` events use the closed sandbox-mode vocabulary. |
| `alego-fs` | Filesystem decision/observation events carry usable target and version identities. |
| `alego-goal` | Durable goal snapshots preserve source attribution, rendered content, revisions, lifecycle and timestamp relationships, and sequential admitted rounds. |
| `alego-goal-round-driver` | Goal-sourced continuation messages match the prompt reconstructed from the preceding durable goal state. |
| `alego-subagent` | Provider add/remove and child start/end events preserve identity and pairing. |
| `alego-permission-presets` | Durable permission decisions name a preset in the active permission table. |
| `alego-user-approval` | Approval asked/decided records pair by call and use valid outcomes and policies. |
| `alego-workflow` | Workflow and child-agent start/end events preserve run metadata, identity, outcome, count, and error relations. |
| `alego-jobs` | Current and terminal task snapshots preserve id/kind, owner, status, and timestamp relationships. |
| `alego-tool-todo` | Durable whole-list snapshots use unique trimmed items and closed statuses. |
| `alego-time-context` | Plugin-attributed clock readings agree with the session's open turn, next pre-step position, and elapsed baseline; rendered time parses and does not postdate its event. |
