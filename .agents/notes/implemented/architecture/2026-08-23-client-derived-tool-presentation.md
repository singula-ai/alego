...
The Client connection stops re-exporting `ToolCallView` and `ToolResultView` from `alego-tools/presentation` for Session consumers.
...
- Session history does not reference `alego-tools/presentation`, `ctx.tools`, `presenterScopeFor`, or `backscanArgs`.
...
Changes to this decision use `alego-pre-push-checks` to select commands for the final diff. Required evidence includes:
...
- `ALEGO_SNAPSHOT=replay pnpm run test:web`, without refreshing existing presentation goldens;
