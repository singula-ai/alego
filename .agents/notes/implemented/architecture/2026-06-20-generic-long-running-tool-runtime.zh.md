...
`JobRegistry` 是 `@singula-ai/alego-jobs` 中的 Service Definition；进程内 Service Provider 是 `@singula-ai/alego-jobs-local` 中的 `LocalJobRegistry`（该拆分记录在[任务注册表约定 Agent Note](../../archived/architecture/2026-07-26-job-registry-seam.md)中）。
