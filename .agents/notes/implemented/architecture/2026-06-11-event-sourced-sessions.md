...
- Persistence stays a plugin concern; the in-memory store ships in alego-session.
...
- Derivation cost grows with log length — compaction (alego-compaction) is the intended mitigation, not log mutation.
