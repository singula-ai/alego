...
`JobRegistry` is the Service Definition in `@singula-ai/alego-jobs`; the process-local provider is `LocalJobRegistry` in `@singula-ai/alego-jobs-local` (the [task-registry contract Agent Note](../../archived/architecture/2026-07-26-job-registry-seam.md) records that split).
