...
- `foldConsumedWork()` replaces `alego-session`'s `findLastMessageTurnEnd()` and moves to `alego-agent`, which owns the inbox marker it reads; the one-shot in-process path folds the same answer and does not classify a cut-short one-shot child as `completed`.
