...
**The `alego` CLI (`apps/cli`, npm name `@alego/cli`).** `apps/*` is the product-assembly tier over `packages/*` libraries. One bin dispatches the default interactive TUI, `-p`/`--prompt` headless turns, and the `web` surface. The TUI boots `examples/tui-agent/cordis.yml` (or `--config`) with the invoking directory as the workspace. From a source checkout, the root `pnpm alego` script runs the same entry with tsx's ESM hook without building; the [source-launch decision](../architecture/2026-07-29-alego-source-launch-tsx-esm.md) owns the runtime vector and the [source-launch/build separation decision](../simplification/2026-08-12-separate-source-launch-from-build.md) owns artifact generation.

**Personal config (`alego-app-boot`).** The personal overlay lives in the Harness home — `$ALEGO_HOME`, else `~/.alego` — resolved by the shared [`resolveAlegoHome`](../architecture/2026-07-24-single-harness-home-resolver.md) (`@alego/home-paths`), the same single root skills and AGENTS.md resolve against. The alego TUI, Web, and headless surfaces consume its two optional files; the demo bins boot their committed trees verbatim:
...
- `alego-app-boot` depends on `js-yaml` and imports the include's `!!js` YAML dialect (`entryListSchema`) directly, and, like `apps/cli`, depends on `@alego/home-paths` for `resolveAlegoHome`.
