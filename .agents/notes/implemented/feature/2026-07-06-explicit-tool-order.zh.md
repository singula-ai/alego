...
配置传递沿用 `persona` 的先例，`toolOrder` 与之并列，位于每个组合的 `alego-system-prompt` 配置行。有一个 schemastery 细节至关重要：schemastery 数组默认为 `[]`，但省略的 `toolOrder` 必须保持 ABSENT（= 字典序），而不是变成一个显式配置的空列表（无效——缺少 rest 条目），因此每个接受该字段的 schema 都将默认值强制为 `undefined`。
