# Agent Note: SessionTelemetryBackend anonymous user id ($ALEGO_HOME/.anonymous-user-id) and the OTel Resource user.id
...
- One `$ALEGO_HOME` is one stable user in the OTel feed; separate homes are separate users by construction, with no cross-home linking mechanism.
