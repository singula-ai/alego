...
[CLI reference README](../../../../apps/cli/reference/README.zh.md) 记录了这一部署口径：会话日志上传默认关闭，`ALEGO_TELEMETRY_MODE=FEEDBACK_ONLY` 和 `ALEGO_TELEMETRY_MODE=FULL` 是两种显式启用选项，显式开启后的导出可能包含完整会话内容。恢复后的[测试阶段引导声明](2026-08-13-shared-modal-product-onboarding.zh.md)不包含遥测文案，因此产品仍不提供任何关于开启上传的提示。
