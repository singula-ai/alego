...
- `@alego/shell-env` is a new shipped package; `alego-tool-bash`'s `alegoHome` config moved there, so compositions mounting the shell tools must also mount `shell-env` (the spine bundles do).
