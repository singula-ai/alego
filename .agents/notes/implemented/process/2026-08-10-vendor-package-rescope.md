...
- No upstream name remains in the publication set. `publish-npm-baseline.ts` now requires every published package to be `@singula-ai/*` with no vendored exemption, so regressing the rename fails before packing.
