...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `alego-tool-bash` (`bash`), `alego-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `alego-tool-subagent` (`subagent`). Example-only tools are excluded.
