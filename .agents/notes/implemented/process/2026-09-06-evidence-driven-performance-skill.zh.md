...
[alego-speed-up-perf skill](../../../skills/alego-speed-up-perf/SKILL.md)（技能）引导广泛调查收敛到范围明确、可测量的用户路径。它结合聚焦的成本归因与独立计时的后端和浏览器端点、合成负载分布、可比较的冷态／热态与保留内存条件，以及收紧预算的负向对照。下方历史证据区分已合并实现、已被替代的提案、作者报告的测量值和估计值。
...
[会话打开性能门禁决策](../testing/2026-09-04-session-open-performance-gate.zh.md)继续负责测试通道机制与校准。[简化 skill](../../../skills/alego-find-simplifications/SKILL.md)继续负责以删除为目标的调查。两者均未被替代：本工作流增加面向性能的候选选择、测量可比性和停止条件，而不替换它们的决策。
