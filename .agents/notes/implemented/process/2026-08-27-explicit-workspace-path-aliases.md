...
The generator emits an alias only for a package whose declared name is exactly `@singula-ai/alego-<directory>`, because that is the only shape a wildcard could ever have resolved: it substituted the specifier's suffix into `packages/<group>/<suffix>/src`. Packages named after something other than their directory — `@singula-ai/alego-typert-protocol` at `packages/typert/protocol`, the `alego-client-*` and `alego-host-*` families — already carry hand-written aliases and are left alone. A specifier claimed by two package directories throws rather than picking one, because an explicit map cannot express the group-order tiebreak the wildcard used; no such collision exists today.
...
Four wildcards remain, each with a single candidate: `alego-host-*/invariant`, `alego-client-*/invariant`, `alego-client-*/client`, and the five `alego-host-<name>/*` subpath maps. One candidate costs one probe, so expanding them would trade file size for nothing.

## Resolution differences this change makes

Every `@singula-ai/alego-*` specifier appearing in repository sources — 1,023 distinct — resolves to the same target as before, with eleven exceptions that now resolve where they previously did not. All eleven previously reached built `lib/` output through the workspace symlink rather than source.

Seven are `/invariant` subpaths: `alego-invariants/invariant`, `alego-lsp/invariant`, `alego-lsp-stdio/invariant`, `alego-tool-lsp/invariant`, `alego-terminal/invariant`, `alego-terminal-bash/invariant`, and `alego-tool-terminal/invariant`.

The deleted `alego-*/invariant` wildcard omitted the `lsp`, `terminal`, `client`, and `host` groups. The `client` and `host` omissions are deliberate and documented — those families have dedicated wildcards because their package names prefix the group directory. The `lsp` and `terminal` omissions have no such reason, and `packages/runtime-diagnostics/invariants` appeared in neither list. Those seven specifiers therefore resolved through the workspace symlink and the package's `./invariant` export to built `lib/types/*.d.ts` instead of to source, which contradicts the rule that static gates resolve workspace imports through `paths` to `src` and pass on a clean tree. Making the aliases uniform resolves them to source like every sibling.

The other four are whole packages the coverage assertion surfaced: `alego-client-ui-directory-picker-browse`, `alego-client-ui-directory-picker-native`, `alego-experimental-agent-team-profile`, and `alego-experimental-agent-team-web-profile`. Each is named `alego-<group>-<directory>`, which no wildcard could ever substitute, and each sits beside siblings that do carry hand-written aliases — they were simply missing. They now carry one too.
...
The win is confined to the tsx source launch. Vitest resolves through `vite-tsconfig-paths`, which matches in-process and checks file existence without ever constructing a Node module error, so it never paid the decoration cost: an A/B over one package's suite measured 5,934/5,829/5,908 ms against 5,878/5,851/5,905 ms, which is noise. Repository gate scripts import few `@singula-ai/alego-*` packages and likewise show no separable difference. Shipped users run built `lib/` under plain Node and were never affected.
