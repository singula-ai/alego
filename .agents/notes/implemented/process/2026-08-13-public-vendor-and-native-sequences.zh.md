...
| alego | `packages/*/*` + `apps/*`(221 个成员) | `restricted` |

`check-workspace-constraints.ts` 按各自序列的级别校验每个 manifest,这是阻止 scope 漂移的那道闸:新增的 `vendor/*` 包留在 `restricted`、或某个 alego 成员被改成 `public`,都会让 workspace 约束失败。
...
**一次性把整个 scope 改成 public。** 暂不采用:那会让下一次 alego 发布因为一次 manifest 改动而顺带变成公开,而不是出自一个刻意的发布决定。先公开这两条依赖序列,是能让每一步的已发布包都保持可安装的顺序,也是将来决定公开 alego 时的前置条件。
