...
- `ALEGO_SESSION_JSONL` 不复存在；shell-env 不再注册持久化贡献方。该变量只有在 `compression: 'none'` 时才是诚实的——默认的 `.jsonl.zstd` 产物无法从 bash 读取。
...
- [会话标识与日志位置](../feature/2026-07-10-agent-session-identity-and-log-location.zh.md)——部分被取代：其 `ALEGO_SESSION_ID` 与 shell-env 注册表决策仍然有效；其 `locate`/`ALEGO_SESSION_JSONL`/`transcript_path` 决策在此移除。
