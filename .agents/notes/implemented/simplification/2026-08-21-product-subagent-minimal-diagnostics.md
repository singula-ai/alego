...
| Exit code and signal | `alego-subprocess` handle | Displayed independently when observed, without inferring missing values |
...
| Diagnostic delivery and byte limit | `alego-subagent` and its foreground or Job consumers | One bounded optional string remains separate from assistant output |
