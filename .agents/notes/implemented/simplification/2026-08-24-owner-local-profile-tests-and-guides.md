...
The `demo:acp` and `demo:cordis` scripts are absent. ACP starts through `alego --profile acp`; the Cordis guide starts `alego web` with its explicit overlay. `demo:ptc` remains as a thin wrapper over `alego --profile headless` with `ALEGO_TOOLS_MODE=ptc`.
