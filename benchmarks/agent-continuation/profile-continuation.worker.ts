/** End-to-end SDK continuation through built alego sdk-minimal with an explicitly mounted file editor. */
...
async function run(root: string): Promise<ProfileReport> {
  ...
  await writeFile(patch, [
    ...
    '- insert:',
    '    - id: fs-local', "      name: '@singula-ai/alego-fs-local'",
    '    - id: str-replace-editor', "      name: '@singula-ai/alego-tool-str-replace-editor'",
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ALEGO_AGENTS_HOME: join(home, 'agents'),
  }
  const harness = new Alego({
    alegoBin: join(import.meta.dirname, '..', '..', '..', 'apps', 'cli', 'lib', 'bin.js'),
    profile: 'sdk-minimal', alegoHome: home, processCwd: cwd, cwd,
...
assertBuiltBenchmarkRuntime(import.meta.url, { '@singula-ai/alego-sdk-client': import.meta.resolve('@singula-ai/alego-sdk-client') })
