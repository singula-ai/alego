...
export function assertBuiltBenchmarkRuntime(
  ...
  if (!moduleUrl.endsWith('.js') || !moduleUrl.includes('/benchmarks/.alego-build/')) {
    throw new Error(`benchmark worker is not running from benchmarks/.alego-build: ${moduleUrl}`)
  }
