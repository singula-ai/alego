...
import { createAssistantMessage, createSystemMessage, createUserMessage, createToolResultMessage, ToolCallId } from '@singula-ai/alego-llm'
...
import { Session, SessionId, SESSION_FORMAT_VERSION } from '@singula-ai/alego-session'
...
export function syntheticHistory(): string {
  ...
  for (let turn = 1; turn <= HISTORY_TURNS; turn++) {
    ...
    if (turn === 1) session.append('system/message', {
      turn, step: 1, message: createSystemMessage('', '@singula-ai/alego-system-prompt'),
    }, { surfaceOp: 'append' })
