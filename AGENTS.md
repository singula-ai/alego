...
packages/    @alego/<pkg> workspaces at packages/<group>/<pkg>/
...
native/      @alego/node-addon-landlock-run source of record (see native/README.md)
...
- Every npm package is `@alego/<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@alego/cordis` is a peerDependency (+ dev) of every harness package.
