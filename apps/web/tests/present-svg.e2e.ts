...
import type {} from '@singula-ai/alego-tool-present/types'
import { deriveReplayScript, parseSessionLog } from '@singula-ai/alego-llm-replay'
import {
...
describe('web e2e: requested SVG is explicitly delivered', () => {
  ...
  beforeAll(async () => {
    ...
    if (MODE !== 'record') {
      replayRoot = await mkdtemp(join(tmpdir(), 'alego-present-svg-replay-'))
