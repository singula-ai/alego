...
// Record: ALEGO_SNAPSHOT=record writes session.v3.jsonl, then a keyless
...
import { ToolCallId, expandAssistantStream } from '@singula-ai/alego-llm'
...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it.skipIf(MODE === 'record')('renders the system prompt disclosure inside the expanded Turn process', async () => {
    ...
    expect(await opaque.textContent()).toContain('You are an AI agent powered by Alego.')
