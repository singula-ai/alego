...
import { canonicalPath, writableRoots } from '@singula-ai/alego-sandbox'
import { SessionId } from '@singula-ai/alego-session'
// These imports carry the tools/sandboxPolicy/approval Context merges.
import { RUN_CODE_NAME } from '@singula-ai/alego-tools'
...
/**
 * `glob` and `grep` come from `alego-tool-fs-search`, which spawns the PACKAGED
...
it('assembles the shipped Web transport, catalog, guidance, and defaults', async () => {
  ...
  try {
    expect(scaffold.ctx.commands.list(commandHandle.agent)).toContainEqual({
      definitionId: '@singula-ai/alego-command-feedback',
      name: 'feedback',
