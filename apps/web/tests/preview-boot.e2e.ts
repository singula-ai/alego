...
} from '@singula-ai/alego-experimental-webworker-packer'
import {
...
function requireVfsAssets(): PreviewAssets {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'alego-preview-boot-'))
...
async function bootPreview(origin: string, browser: Browser): Promise<void> {
  ...
  try {
    ...
    const exercised = await page.evaluate(async () => {
      ...
      const transport = (globalThis as typeof globalThis & { __ALEGO_TRANSPORT__?: PreviewTransport }).__ALEGO_TRANSPORT__
      ...
      await createDirectory('/alego/workspace/.agents/skills', 'runtime-created')
  ...
  try {
    ...
    const sessionCount = await page.evaluate(async () => {
      const transport = (globalThis as typeof globalThis & {
        __ALEGO_TRANSPORT__?: { fetch(input: string, init: RequestInit): Promise<Response> }
      }).__ALEGO_TRANSPORT__
