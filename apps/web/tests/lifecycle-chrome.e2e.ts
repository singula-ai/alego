...
describe('web e2e: lifecycle & chrome (workspace flow / reload / dark mode)', () => {
  ...
  it('sends the first prompt from the empty-state hero (all modes)', async () => {
    ...
    await expect.poll(() => page.getByText('Build AI Agents with ALEGO', { exact: false }).count(), { timeout: 15_000 }).toBe(1)
    const input = page.locator('[data-composer-input]').first()
    await input.waitFor({ timeout: 10_000 })
    if (MODE !== 'record') {
      await page.getByText('Build AI Agents with ALEGO', { exact: false }).hover()
  ...
  it.skipIf(MODE === 'record')('recovers the whole surface across a reload from the log alone', async () => {
    ...
    // Selection persisted (alego.sessions.current) and history replayed: the
