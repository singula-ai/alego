...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('ALEGO_CLIENT_BUILD_PROFILE') === 'official') {
    expect(document.querySelector('svg[viewBox="30 0 54 24"]')).not.toBeNull()
    expect(screen.queryByText('ALEGO Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="0 0 24 18"]')).not.toBeNull()
    const version = clientBuildValue('ALEGO_CLIENT_VERSION')
    if (version === undefined) throw new Error('default client build record must carry ALEGO_CLIENT_VERSION')
    const commit = clientBuildValue('ALEGO_CLIENT_COMMIT_HASH')
    const buildVersion = version
      + (commit === undefined ? '' : `-${commit}`)
      + (clientBuildValue('ALEGO_CLIENT_GIT_DIRTY') === 'true' ? '-dirty' : '')
    screen.getByText('ALEGO Local Build')
...
it('boots without ui-chat and does not select another conversation view implicitly', async () => {
  mountAssembledApp('?fixture', { exclude: ['@singula-ai/alego-client-ui-chat'] })

  const tree = await screen.findByRole('tree', { name: 'Sessions' }, { timeout: 10_000 })
  const boot = Reflect.get(window, '__ALEGO_BOOT__') as { entries: Array<{ id: string }> } | undefined
  expect(boot?.entries.some(entry => entry.id === '@singula-ai/alego-client-ui-chat')).toBe(false)
