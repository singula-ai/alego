...
function seedLog(): string {
  ...
  return [
    ...
    at(2, {
      type: 'system/message',
      data: { turn: 1, step: 1, message: createSystemMessage('', '@singula-ai/alego-system-prompt') },
      surfaceOp: 'append',
    }),
    at(3, {
      type: 'user/message',
...
describe('web e2e: agent-preset selection', () => {
  ...
  beforeAll(async () => {
    ...
    presetRoot = await realpath(await mkdtemp(join(tmpdir(), 'alego-web-e2e-refusing-')))
