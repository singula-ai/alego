...
describe.skipIf(MODE === 'record')('web e2e: Open In under SSH', () => {
  ...
  beforeAll(async () => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('alego.open-in-app.choice', JSON.stringify('vscode'))
    })
  })
  ...
  it('hides a remembered app after the real host returns an empty catalog', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('alego.open-in-app.choice'))).toBe('"vscode"')
