...
import type {} from '@singula-ai/alego-webhook'
import {
...
const TITLE = 'Review singula-ai/alego#314'
...
describe.skipIf(MODE === 'record')('web e2e: GitHub ready-for-review', () => {
  ...
  beforeAll(async () => {
    previousPort = process.env.ALEGO_GITHUB_WEBHOOK_PORT
    previousSecret = process.env.ALEGO_GITHUB_WEBHOOK_SECRET
    const port = await freePort()
    process.env.ALEGO_GITHUB_WEBHOOK_PORT = String(port)
    process.env.ALEGO_GITHUB_WEBHOOK_SECRET = SECRET
  ...
  afterAll(async () => {
    ...
    if (previousPort === undefined) Reflect.deleteProperty(process.env, 'ALEGO_GITHUB_WEBHOOK_PORT')
    else process.env.ALEGO_GITHUB_WEBHOOK_PORT = previousPort
    if (previousSecret === undefined) Reflect.deleteProperty(process.env, 'ALEGO_GITHUB_WEBHOOK_SECRET')
    else process.env.ALEGO_GITHUB_WEBHOOK_SECRET = previousSecret
  })
  ...
  it('isolates ingress and creates a browsable Workspace Session', async () => {
    ...
    const payload = {
      ...
      pull_request: {
        ...
        html_url: 'https://github.com/singula-ai/alego/pull/314',
        draft: false,
        user: { login: 'octocat' },
