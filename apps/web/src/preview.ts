 ...
 */
import AlegoWorker from '@singula-ai/alego-experimental-webworker-runtime/worker?worker'
import {
  chooseWorkerHostSource, connectWorkerHost, IMAGE_FILE_NAME,
} from '@singula-ai/alego-experimental-webworker-runtime/client'

const image = `preview/${IMAGE_FILE_NAME}`
const source = await chooseWorkerHostSource({ image })
await connectWorkerHost(new AlegoWorker({ name: 'alego-host' }), { image, overlays: source.overlays })
