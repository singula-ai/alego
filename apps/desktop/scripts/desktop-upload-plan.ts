...
/**
 * Validate the completed package record, alego version, update metadata, hashes, and target files.
...
export async function createDesktopUploadPlan(
  ...
  const alegoVersion = await manifestVersion(join(repositoryRoot, 'package.json'), 'alego package')
  const desktopVersion = await manifestVersion(join(appRoot, 'package.json'), 'desktop package')
  if (alegoVersion !== desktopVersion) {
    throw new Error(`desktop upload: desktop version ${desktopVersion} does not match current alego version ${alegoVersion}`)
  }
  ...
  if (buildRecord.schemaVersion !== 1
    ...
    || buildRecord.version !== alegoVersion
    || buildRecord.environment !== update.environment
    || buildRecord.publicUrl !== update.publicUrl) {
    throw new Error(`desktop upload: ${targetName} package completion record does not match alego ${alegoVersion} and ${update.environment} update destination`)
  }

  const metadataFilename = desktopUpdateMetadataFilename(alegoVersion, target.platform)
  ...
  if (metadataVersion !== alegoVersion) {
    throw new Error(`desktop upload: ${metadataFilename} version ${metadataVersion} does not match current alego version ${alegoVersion}`)
  }
  ...
  const base = `alego-${alegoVersion}-${target.os}-${target.arch}`
