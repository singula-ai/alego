...
export function prepareDevelopmentProject(options: DevelopmentProjectOptions): string {
  ...
  if (cliManifest.name !== '@singula-ai/alego' || cliManifest.version !== options.release.version) {
    throw new Error(
      `desktop development: apps/cli must be @singula-ai/alego@${options.release.version}, found `
  ...
  if (hostManifest.name !== '@singula-ai/alego-desktop-host' || hostManifest.version !== options.release.version) {
    throw new Error(
      `desktop development: apps/desktop-host must be @singula-ai/alego-desktop-host@${options.release.version}, found `
  ...
  const alegoLink = join(destinationModules, '@singula-ai', 'alego')
  removeOwnedPath(alegoLink)
  linkDirectory(options.cliDir, alegoLink)
  const hostLink = join(destinationModules, '@singula-ai', 'alego-desktop-host')
