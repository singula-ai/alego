...
const ALEGO_OUTPUT_ROOT = BUILD_PATHS.alego
const BUILD_ROOT = mkdtempSync(join(tmpdir(), 'alego-desktop-runtime-'))
const STORE_ROOT = join(BUILD_ROOT, 'store')
const RUNTIME_ROOT = BUILD_PATHS.runtime
const PNPM_BUILD_STATE = BUILD_PATHS.alegoPnpm
...
function desktopRelease(): DesktopRelease {
  ...
  const alegoVersion = manifestVersion(resolve(APP_ROOT, '..', '..', 'package.json'), 'root alego package')
  if (version !== alegoVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @singula-ai/alego, found ${alegoVersion}`)
  }
...
async function main(): Promise<void> {
  rmSync(ALEGO_OUTPUT_ROOT, { recursive: true, force: true })
  ...
  try {
    ...
    mkdirSync(ALEGO_OUTPUT_ROOT, { recursive: true })
    cpSync(modules, join(ALEGO_OUTPUT_ROOT, 'node_modules'), {
      recursive: true, dereference: true,
      filter: source => desktopRuntimeFileExclusion(relative(modules, source), target) === undefined,
    })
    writeFileSync(join(ALEGO_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@singula-ai/alego-desktop-runtime', private: true, version: release.version, type: 'module',
    ...
    for (const file of DESKTOP_HOST_RUNTIME_FILES) {
      if (!existsSync(join(ALEGO_OUTPUT_ROOT, 'node_modules', DESKTOP_HOST_PACKAGE, file))) {
        throw new Error(`desktop runtime: missing private Host file ${file}`)
      }
    }
    if (process.platform === 'darwin') {
      await signMacOSRuntime(ALEGO_OUTPUT_ROOT, resolveDesktopAppId(process.env), resolveMacOSSigningEnvironment(process.env))
    }
    writeDesktopRuntime(ALEGO_OUTPUT_ROOT, release, packageSet.packages.map(entry => entry.name), target)
    const descriptor = await verifyDesktopRuntime(ALEGO_OUTPUT_ROOT, release.version, target)
    await new Promise<void>((accept, reject) => {
      execFile(NODE, [join(APP_ROOT, 'tests/fixtures/runtime-payload-smoke.mjs'), ALEGO_OUTPUT_ROOT],
    ...
    await smokeDesktopRuntime(ALEGO_OUTPUT_ROOT, NODE, descriptor)
    await verifyDesktopRuntime(ALEGO_OUTPUT_ROOT, release.version, target)
  } catch (error) {
    rmSync(ALEGO_OUTPUT_ROOT, { recursive: true, force: true })
    throw error
  } finally {
