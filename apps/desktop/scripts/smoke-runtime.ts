 ...
 * @param root - Materialized alego resources.
...
export async function smokeDesktopRuntime(root: string, node: string, runtime: DesktopRuntimeDescriptor): Promise<void> {
  const home = mkdtempSync(join(tmpdir(), 'alego-desktop-smoke-'))
  const profile = join(home, 'profiles', 'desktop')
  const host = new DesktopHostProcess(node, root, profile, undefined, { ...process.env, ALEGO_HOME: home })
  try {
    createPluginProfile(profile)
    const pluginName = 'desktop-runtime-smoke-plugin'
    const plugin = join(profile, 'node_modules', pluginName)
    mkdirSync(plugin, { recursive: true })
    const cordis = runtime.sharedPackages.find(entry => entry.name === '@singula-ai/cordis')
    ...
    writeFileSync(join(plugin, 'package.json'), JSON.stringify({
      ...
      peerDependencies: { '@singula-ai/cordis': cordis.version }, alego: { bundle: { patch: './bundle.yml' } },
    }))
...
`)
    ...
    manifest.alego.profile.bundles.push(pluginName)
    ...
    if (ready.alegoVersion !== runtime.release.version) throw new Error('desktop runtime: Host reported another alego release')
    const response = await host.fetch(new Request('alego-app://app/'))
