...
export function resolveDesktopBuildTarget(
  ...
  const platform = env.ALEGO_DESKTOP_TARGET_PLATFORM ?? env.npm_config_platform ?? hostPlatform
  const arch = env.ALEGO_DESKTOP_TARGET_ARCH ?? env.npm_config_arch ?? hostArch
 ...
 * @returns {{ root: string, artifacts: string, runtime: string, packageSet: string, alego: string, alegoPnpm: string, nodeExtract: string, packedAlego: string, packedVendor: string, packedLandlock: string, downloads: string }} Target paths plus the shared immutable download cache.
 */
export function desktopTargetBuildPaths(target) {
  ...
  return {
    ...
    alego: join(root, 'alego'),
    alegoPnpm: join(root, 'alego-pnpm'),
    nodeExtract: join(root, 'node-extract'),
    packedAlego: join(packed, 'alego'),
