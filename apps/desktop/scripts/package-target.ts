/** Build one release target with matching Electron, Node.js, and alego architecture. */
...
const WINDOWS_SIGNING_ENV_PREFIX = 'ALEGO_DESKTOP_WINDOWS_'
const WINDOWS_SIGNING_ENV_NAMES = [
  'ALEGO_DESKTOP_WINDOWS_CER_FILE',
  'ALEGO_DESKTOP_WINDOWS_KEY_CONTAINER',
  'ALEGO_DESKTOP_WINDOWS_SIGNTOOL',
  'ALEGO_DESKTOP_WINDOWS_TOKEN_PIN',
] as const
...
export function desktopElectronBuilderEnvironment(environment: NodeJS.ProcessEnv, unsigned: boolean): NodeJS.ProcessEnv {
  const selected: NodeJS.ProcessEnv = { ...environment, ALEGO_DESKTOP_UNSIGNED: unsigned ? '1' : '0' }
  // The bundled NSIS decoder cannot extract 7-Zip's automatic ARM64-filtered entries.
  if (environment.ALEGO_DESKTOP_TARGET_PLATFORM === 'win32') selected.ELECTRON_BUILDER_7Z_FILTER = 'BCJ'
...
function writeReleaseRecord(
  ...
  const alegoVersion = packageVersion(join(REPOSITORY_ROOT, 'package.json'), 'alego package')
  if (desktopVersion !== alegoVersion) {
    throw new Error(`desktop package: desktop version ${desktopVersion} does not match alego version ${alegoVersion}`)
  }
...
async function main(): Promise<void> {
  ...
  const targetEnv: NodeJS.ProcessEnv = {
    ...buildEnv,
    ALEGO_DESKTOP_TARGET_PLATFORM: target.platform,
    ALEGO_DESKTOP_TARGET_ARCH: target.arch,
  }
  ...
  await runPnpm(['run', 'release:pack', '--family', 'alego', '--out', buildPaths.packedAlego], buildEnv, REPOSITORY_ROOT)
  await runPnpm([
    '--dir',
    'apps/desktop-host',
    'pack',
    '--pack-destination',
    buildPaths.packedAlego,
  ...
  await runPnpm(['run', 'prepare:alego'], targetEnv)
