...
function runtimeResources(): RuntimeResources {
  ...
  const node = (development ? process.env.ALEGO_DESKTOP_NODE_BINARY : undefined)
    ?? join(process.resourcesPath, 'runtime', 'node', process.platform === 'win32' ? 'node.exe' : 'node')
  const pnpm = (development ? process.env.ALEGO_DESKTOP_PNPM_ENTRY : undefined)
    ?? join(process.resourcesPath, 'runtime', 'pnpm', 'bin', 'pnpm.mjs')
  const alego = (development ? process.env.ALEGO_DESKTOP_ALEGO_DIR : undefined) ?? join(process.resourcesPath, 'alego')
  return { node, pnpm, alego }
}

function developmentHostInspectPort(enabled: boolean): number | undefined {
  const configured = process.env.ALEGO_DESKTOP_HOST_INSPECT_PORT
  if (!enabled || configured === undefined || configured === '') return undefined
  const port = Number(configured)
  if (!Number.isSafeInteger(port) || port < 1 || port > 65_535) {
    throw new Error('alego desktop: ALEGO_DESKTOP_HOST_INSPECT_PORT must be an integer from 1 through 65535')
  }
  return port
}
...
function createWindow(preload: string, show = false): BrowserWindow {
  ...
  window.webContents.on('will-navigate', (event, url) => {
    ...
    if (action.protocol !== 'alego-recovery:' || !['restart', 'plugins', 'reset'].includes(action.hostname)) return
...
async function main(): Promise<void> {
  ...
  const backend = new DesktopBackendController((onFailure) => {
    ...
    const host = new DesktopHostProcess(resources.node, development ?? resources.alego, activeProject,
  ...
  const mutate = async (event: IpcMainInvokeEvent, mutation: Parameters<DesktopProjectManager['mutate']>[0]): Promise<void> => {
    ...
    if (development !== undefined) {
      throw new Error('alego desktop: plugin package changes require a packaged application')
    }
    await startup?.catch(() => undefined)
    pageError = undefined
    await navigateMain(startupUrl)
  ...
  ipcMain.handle(DESKTOP_IPC.pluginsAdd, (event, spec: unknown) => {
    if (typeof spec !== 'string') throw new Error('alego desktop: plugin spec must be a string')
    return mutate(event, { type: 'plugin-add', spec })
  })
  ipcMain.handle(DESKTOP_IPC.pluginsRemove, (event, name: unknown) => {
    if (typeof name !== 'string') throw new Error('alego desktop: plugin name must be a string')
  ...
  ipcMain.handle(DESKTOP_IPC.pluginsUpdate, (event, name: unknown, version: unknown) => {
    if (typeof name !== 'string' || typeof version !== 'string') {
      throw new Error('alego desktop: plugin name and version must be strings')
    }
  ...
  ipcMain.handle(DESKTOP_IPC.pluginsToggle, (event, name: unknown, enabled: unknown) => {
    if (typeof name !== 'string' || typeof enabled !== 'boolean') throw new Error('alego desktop: invalid plugin activation request')
  ...
  if (quitting) return
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
  if (mainWindow !== undefined && development !== undefined && process.env.ALEGO_DESKTOP_OPEN_DEVTOOLS !== '0') {
...
if (ownsDesktopInstance) void app.whenReady().then(main).catch(async (error: unknown) => {
  ...
  console.error(error)
  const diagnosticFile = process.env.ALEGO_DESKTOP_DIAGNOSTIC_FILE
