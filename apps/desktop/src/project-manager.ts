...
const PROJECT_NAME = '@singula-ai/alego-desktop-runtime'
const ALEGO_PACKAGE = '@singula-ai/alego'
const CORE_BUILD_PACKAGE = '@singula-ai/alego-subprocess-local'
const DESKTOP_PROFILE_BUNDLES = ['@singula-ai/alego-base', '@singula-ai/alego-web-app'] as const
...
function projectManifest(projectDir: string): DesktopProjectManifest {
  ...
  const alego = isRecord(value) && isRecord(value.alego) ? value.alego : undefined
  const profile = isRecord(alego?.profile) ? alego.profile : undefined
...
function profilePluginNames(projectDir: string): readonly string[] {
  const bundles = projectManifest(projectDir).alego.profile.bundles
...
function inspectPlugin(projectDir: string, requestedName: string): DesktopPluginRecord {
  ...
  const bundle = isRecord(alego) ? alego.bundle : undefined
  const patch = isRecord(bundle) ? bundle.patch : undefined
  if (typeof patch !== 'string' || patch === '') {
    throw new Error(`desktop project: ${requestedName}@${manifest.version} does not declare alego.bundle.patch`)
  }
...
export class DesktopProjectManager {
  ...
  /** Read the alego version supplied by this application's verified resources. */
  alegoVersion(): string {
  ...
  canRecoverProfile(): boolean {
    return this.descriptor !== undefined && existsSync(this.runtime.node) && existsSync(this.runtime.alego)
  }
  ...
  private readRuntime(): DesktopRuntimeDescriptor {
    ...
    return readDesktopRuntime(this.runtime.alego)
  }

  private prepareProfile(projectDir: string): void {
    const runtime = this.currentRuntime()
    linkDesktopHostPackages(projectDir, this.runtime.alego, runtime)
    validateDesktopPluginGraph(projectDir, this.runtime.alego, runtime, profilePluginNames(projectDir))
  }
    ...
    return this.withLock(async () => {
      ...
      if (!existsSync(this.pendingPackages) && previous?.runtimeId === desktopRuntimeId(target)
        ...
        && previous.links.every(link => existsSync(link.target)
          ...
          && realpathSync.native(link.target) === realpathSync.native(join(this.runtime.alego, 'node_modules', link.name)))) {
        return false
      }
    ...
    await this.withLock(async () => {
      ...
      if (mutation.type === 'plugins-disable-all') {
        ...
        writeJson(join(this.paths.profile, 'package.json'), {
          ...manifest,
          alego: { ...manifest.alego, profile: { ...manifest.alego.profile, bundles: [...DESKTOP_PROFILE_BUNDLES] } },
        })
        this.prepareProfile(this.paths.profile)
        await hooks.afterChange()
        return
      ...
      try {
        ...
        if (packagesChanged) linkDesktopHostPackages(this.paths.profile, this.runtime.alego, this.currentRuntime())
      }
...
export function createRuntimeProjectMetadata(projectDir: string, release: DesktopRelease): void {
  ...
  const manifest: DesktopProjectManifest = {
    ...
    alego: { profile: { bundles: [...DESKTOP_PROFILE_BUNDLES] } },
  }
