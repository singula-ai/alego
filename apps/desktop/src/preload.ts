...
import { DESKTOP_IPC, type AlegoDesktopApi, type DesktopUpdateState } from './ipc.ts'
import type { DesktopBackendState } from './backend-controller.ts'

const api: AlegoDesktopApi = {
  protocolVersion: 1,
  locale: () => ipcRenderer.invoke(DESKTOP_IPC.localeGet) as Promise<ReturnType<AlegoDesktopApi['locale']> extends Promise<infer T> ? T : never>,
  plugins: {
    list: () => ipcRenderer.invoke(DESKTOP_IPC.pluginsList) as Promise<ReturnType<AlegoDesktopApi['plugins']['list']> extends Promise<infer T> ? T : never>,
  ...
  backend: {
    status: () => ipcRenderer.invoke(DESKTOP_IPC.backendStatus) as ReturnType<AlegoDesktopApi['backend']['status']>,
...
contextBridge.exposeInMainWorld('alegoDesktop', api)
