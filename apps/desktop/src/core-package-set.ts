/** Signed local npm package set that supplies the Desktop-owned alego runtime and private Host. */
...
/** Private package installed beside alego to boot the Desktop Host process. */
export const DESKTOP_HOST_PACKAGE = '@singula-ai/alego-desktop-host'
...
/** Complete union of the first-party package closures rooted at alego and its private Desktop Host. */
...
const RELEASE_PACKAGES = [ALEGO_PACKAGE, DESKTOP_HOST_PACKAGE] as const
 ...
 * @param expectedReleaseVersion - Required alego and Desktop Host version when validating one release.
...
/** Return the local direct dependency spec for the alego package. */
export function desktopAlegoPackageSpec(packageSet: DesktopCorePackageSet): string {
  const record = packageSet.packages.find(entry => entry.name === ALEGO_PACKAGE)
  if (record === undefined) throw new Error(`desktop package set: missing ${ALEGO_PACKAGE}`)
 ...
 * @param expectedReleaseVersion - Exact alego and Desktop Host version bound to Electron.
