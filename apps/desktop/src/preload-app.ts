...
import { DESKTOP_IPC, type AlegoDesktopStartupApi } from './ipc.ts'
import type { DesktopBackendState } from './backend-controller.ts'

const startup: AlegoDesktopStartupApi = {
  protocolVersion: 1,
  locale: () => ipcRenderer.invoke(DESKTOP_IPC.localeGet) as ReturnType<AlegoDesktopStartupApi['locale']>,
  backend: {
    status: () => ipcRenderer.invoke(DESKTOP_IPC.backendStatus) as ReturnType<AlegoDesktopStartupApi['backend']['status']>,
    subscribe(listener) {
...
contextBridge.exposeInMainWorld('alegoDesktop', location.protocol === 'alego-app:' && location.hostname === 'shell'
