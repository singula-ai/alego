...
export function createElectronBuilderConfig(
  ...
  return {
    appId,
    productName: 'Alego',
    artifactName: 'alego-${version}-${os}-${arch}.${ext}',
    ...
    extraResources: [
      ...
      { from: buildPaths.alego, to: 'alego' },
      // electron-builder excludes a source directory's root node_modules.
      { from: join(buildPaths.alego, 'node_modules'), to: 'alego/node_modules' },
    ],
    mac: {
    ...
    afterPack: async context => {
      ...
      await verifyDesktopRuntime(join(context.packager.getResourcesDir(context.appOutDir), 'alego'),
        context.packager.appInfo.version, { platform: resolvedPlatform, arch: resolvedArch })
    },
    afterSign: async context => {
      if (context.electronPlatformName !== 'darwin') return
      const { verifyDesktopRuntime } = await import('./lib/types/runtime-tree.js')
      await verifyDesktopRuntime(join(context.appOutDir, `${context.packager.appInfo.productFilename}.app`, 'Contents', 'Resources', 'alego'),
