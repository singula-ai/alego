...
function temporaryRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'alego-desktop-test-'))
  roots.push(root)
  return root
}
...
if (command !== 'rebuild') {
  ...
  for (const [name, version] of Object.entries(manifest.dependencies)) {
    ...
    writeFileSync(join(packageRoot, 'package.json'), JSON.stringify({name, version,
      peerDependencies: {'@singula-ai/cordis': '^1.0.0'}, alego: {bundle: {patch: './bundle.yml'}}}))
...
function setup(): { root: string; manager: DesktopProjectManager } {
  ...
  const alego = join(root, 'resources', 'alego')
  runtimeFixture(alego)
  return { root, manager: new DesktopProjectManager(resolveDesktopPaths(join(root, '.alego')), { node: process.execPath, pnpm: writeFakePnpm(root), alego }) }
}
...
describe('desktop external plugin profile', () => {
  ...
  it('disables every third-party bundle without reading a broken plugin patch declaration', async () => {
    ...
    await manager.mutate({ type: 'plugins-disable-all' }, hooks({ afterChange: async () => {
      ...
      }).alego.profile.bundles).not.toContain('plugin')
    } }))
    expect((JSON.parse(readFileSync(join(manager.paths.profile, 'package.json'), 'utf8')) as {
      alego: { profile: { bundles: string[] } }
    }).alego.profile.bundles).not.toContain('plugin')
  ...
  it('resets the entire profile without backups while retaining its lock and shared data', async () => {
    ...
    const task = join(root, '.alego', 'task-sentinel')
    const homeEnvironment = join(root, '.alego', '.env')
  ...
  it('reports damaged application metadata as a reinstall failure', async () => {
    const { manager } = setup()
    writeFileSync(join(manager.runtime.alego, 'desktop-runtime.json'), '{broken')
    await expect(manager.applyRelease()).rejects.toThrow()
  ...
  it('retries installation after an interrupted runtime rebuild removed plugin files', async () => {
    ...
    await manager.applyRelease()
    await manager.mutate({ type: 'plugin-add', spec: 'plugin@1.0.0' }, hooks())
    const alego = join(root, 'new-node')
    runtimeFixture(alego, '1.1.0', '24.18.0')
    const failing = join(root, 'fail-install.mjs')
    writeFileSync(failing, 'process.exitCode = 1')
    const worker = new DesktopProjectManager(manager.paths, { ...manager.runtime, alego, pnpm: failing })
    ...
    const retry = new DesktopProjectManager(manager.paths, { ...manager.runtime, alego })
  ...
  it.each(['plugin-add', 'runtime-change'] as const)('retries failed rebuild after %s across manager instances', async (operation) => {
    const { root, manager } = setup()
    await manager.applyRelease()
    let alego = manager.runtime.alego
    if (operation === 'runtime-change') {
      await manager.mutate({ type: 'plugin-add', spec: 'plugin@1.0.0' }, hooks())
      alego = join(root, 'new-node')
      runtimeFixture(alego, '1.1.0', '24.18.0')
    }
  ...
  it('initializes and restarts offline without executing pnpm', async () => {
    ...
    expect(realpathSync(join(manager.paths.profile, 'node_modules/@singula-ai/cordis'))).toBe(realpathSync(join(manager.runtime.alego, 'node_modules/@singula-ai/cordis')))
  ...
  it('repairs a removed managed link without running pnpm', async () => {
    const { root, manager } = setup()
    await manager.applyRelease()
    unlinkSync(join(manager.paths.profile, 'node_modules/@singula-ai/cordis'))
    await expect(manager.applyRelease()).resolves.toBe(true)
    expect(calls(root)).toEqual([])
  })
  ...
  it.skipIf(process.platform !== 'win32')('reuses the profile when the launch path changes only Windows letter casing', async () => {
    const { manager } = setup()
    await manager.applyRelease()
    const relaunched = new DesktopProjectManager(manager.paths, { ...manager.runtime, alego: manager.runtime.alego.toUpperCase() })
  ...
  it.each(['changed', 'same-size', 'extra', 'missing'])('starts and reuses a profile without checking %s runtime bytes', async (operation) => {
    const { root, manager } = setup()
    if (operation === 'changed') writeFileSync(join(manager.runtime.alego, 'package.json'), '{}')
    if (operation === 'same-size') writeFileSync(join(manager.runtime.alego, 'package.json'), '{"type":"Module"}\n')
    if (operation === 'extra') writeFileSync(join(manager.runtime.alego, 'extra'), '')
    if (operation === 'missing') unlinkSync(join(manager.runtime.alego, 'package.json'))
    await expect(manager.applyRelease()).resolves.toBe(true)
  ...
  it('installs only plugins and checks the graph before running lifecycle scripts', async () => {
    ...
    await expect(manager.mutate({ type: 'plugin-add', spec: '@singula-ai/cordis' }, hooks())).rejects.toThrow(/host-owned/u)
    await expect(manager.applyRelease()).resolves.toBe(false)
    expect(calls(root)).toHaveLength(2)
  })
  ...
  it('keeps plugin files and patches through a compatible release and application relocation', async () => {
    ...
    const nextRoot = join(root, 'relocated', 'alego')
    runtimeFixture(nextRoot, '1.1.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, alego: nextRoot })
    ...
    expect(calls(root)).toHaveLength(2)
    expect(realpathSync(join(manager.paths.profile, 'node_modules/@singula-ai/cordis'))).toBe(realpathSync(join(nextRoot, 'node_modules/@singula-ai/cordis')))
  ...
  it('reinstalls the locked plugin graph when bundled Node changes', async () => {
    ...
    const alego = join(root, 'new-node')
    runtimeFixture(alego, '1.1.0', '24.18.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, alego })
  ...
  it('allows incompatible plugins to be disabled in recovery without deleting them', async () => {
    ...
    await manager.applyRelease()
    await manager.mutate({ type: 'plugin-add', spec: 'plugin@1.0.0' }, hooks())
    const alego = join(root, 'next-major')
    runtimeFixture(alego, '2.0.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, alego })
    await expect(next.applyRelease()).rejects.toThrow(/requires @singula-ai\/cordis/u)
  ...
  it('keeps partial package changes and restores host links after pnpm fails', async () => {
    ...
    expect(realpathSync(join(manager.paths.profile, 'node_modules/@singula-ai/cordis')))
      .toBe(realpathSync(join(manager.runtime.alego, 'node_modules/@singula-ai/cordis')))
    await manager.mutate({ type: 'plugin-remove', name: 'plugin' }, hooks())
    expect(manager.listPlugins()).toEqual([])
  })
