...
describe('Windows token signing', () => {
  it('passes only the validated BAT fields to the signing command interpreter', () => {
    expect(buildWindowsSigningEnvironment({
      ...
      ALEGO_DESKTOP_WINDOWS_TOKEN_PIN: 'inherited-token-secret',
      DEEPSEEK_API_KEY: 'api-secret',
      BUILD_PASSWORD: 'build-secret',
    }, {
      certificateFile: CERTIFICATE_FILE,
      signTool: 'C:\\tools\\signtool.exe',
      path: 'C:\\release\\Alego.exe',
      isNest: false,
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toEqual({
      SystemRoot: 'C:\\Windows',
      ALEGO_DESKTOP_WINDOWS_SIGNTOOL: 'C:\\tools\\signtool.exe',
      ALEGO_DESKTOP_WINDOWS_CER_FILE: CERTIFICATE_FILE,
      ALEGO_DESKTOP_WINDOWS_TOKEN_PIN: 'token-secret!',
      ALEGO_DESKTOP_WINDOWS_KEY_CONTAINER: 'te-container',
      ALEGO_DESKTOP_WINDOWS_SIGN_TARGET: 'C:\\release\\Alego.exe',
      ALEGO_DESKTOP_WINDOWS_SIGN_APPEND: '',
    })
  })
  ...
  it('requests an appended signature only for an electron-builder nested task', () => {
    expect(buildWindowsSigningEnvironment({}, {
      ...
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    }).ALEGO_DESKTOP_WINDOWS_SIGN_APPEND).toBe('1')
  })
  ...
  it('keeps the verified SafeNet command in an ASCII CRLF CMD file', async () => {
    ...
    expect(text).toContain('set "ALEGO_DESKTOP_WINDOWS_CER_FILE="\r\n')
    expect(text).toContain('set "ALEGO_DESKTOP_WINDOWS_TOKEN_PIN="\r\n')
  ...
  it('rejects incomplete signing identities and non-SHA-256 signing tasks', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'alego-windows-sign-tool-'))
    ...
    expect(() => createWindowsTokenSigner({
      ...
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toThrow(/ALEGO_DESKTOP_WINDOWS_CER_FILE/u)
    expect(() => createWindowsTokenSigner({
      certificateFile,
      signTool: undefined,
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toThrow(/ALEGO_DESKTOP_WINDOWS_SIGNTOOL/u)
    ...
    try {
      expect(() => createWindowsTokenSigner({
        ...
        signTool,
        tokenPin: 'token-secret!',
      })).toThrow(/ALEGO_DESKTOP_WINDOWS_KEY_CONTAINER/u)
      expect(() => createWindowsTokenSigner({
        certificateFile,
        signTool,
        tokenPin: '',
        keyContainer: 'te-container',
      })).toThrow(/ALEGO_DESKTOP_WINDOWS_TOKEN_PIN/u)
      expect(() => createWindowsTokenSigner({
        certificateFile,
        signTool,
        tokenPin: 'token]secret',
  ...
  it('removes inherited credentials and redacts SignTool process failures', () => {
    expect(scrubWindowsSigningEnvironment({
      SystemRoot: 'C:\\Windows',
      ALEGO_DESKTOP_WINDOWS_CER_FILE: 'C:\\release\\server.cer',
      ALEGO_DESKTOP_WINDOWS_SIGNTOOL: 'C:\\tools\\signtool.exe',
      ALEGO_DESKTOP_WINDOWS_TOKEN_PIN: 'token-secret',
      DEEPSEEK_API_KEY: 'api-secret',
      BUILD_PASSWORD: 'build-secret',
  ...
  it('clears a certificate table inherited beyond the generated uninstaller', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'alego-windows-sign-'))
