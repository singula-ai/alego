...
import { DESKTOP_IPC, type AlegoDesktopStartupApi } from '../src/ipc.ts'
...
it.each(['alego-app://app/index.html', 'https://shell/startup.html'])('exposes only the carrier marker to %s', async (url) => {
  vi.stubGlobal('location', new URL(url))
  await import('../src/preload-app.ts')
  expect(electron.contextBridge.exposeInMainWorld).toHaveBeenCalledWith('alegoDesktop', { protocolVersion: 1 })
})

it('provides startup controls and a removable state subscription to shell documents', async () => {
  vi.stubGlobal('location', new URL('alego-app://shell/startup.html'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls[0]?.[1] as AlegoDesktopStartupApi
  await api.locale()
