...
async function fixture(
  ...
  const root = await mkdtemp(join(tmpdir(), 'alego-desktop-upload-'))
  ...
  const base = `alego-${version}-${os}-${arch}`
  const origin = environment === 'test'
    ? TEST_ORIGIN
    : 'https://alego-updates.example.com'
  await writeFile(join(artifactsRoot, `${target}-release.json`), `${JSON.stringify({
    schemaVersion: 1,
    target,
    version,
    environment,
    publicUrl: `${origin}/_/alego/desktop/stable/${target}/`,
  })}\n`)
...
describe('desktop upload plan', () => {
  it('validates macOS artifacts and puts channel metadata last', async () => {
    ...
    expect(plan).toMatchObject({
      environment: 'test',
      version: '1.2.3',
      publicUrl: 'https://desktop-updates.example.com/_/alego/desktop/stable/mac-arm64/',
      bucket: TEST_BUCKET,
    })
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'alego-1.2.3-mac-arm64.dmg',
      'alego-1.2.3-mac-arm64.zip',
      'alego-1.2.3-mac-arm64.zip.blockmap',
      'latest-mac.yml',
    ])
  ...
  it('uploads the prerelease channel metadata emitted by electron-builder', async () => {
    ...
    const plan = await createDesktopUploadPlan('mac-arm64', paths)
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'alego-1.2.3-alpha.4-mac-arm64.dmg',
      'alego-1.2.3-alpha.4-mac-arm64.zip',
      'alego-1.2.3-alpha.4-mac-arm64.zip.blockmap',
      'alpha-mac.yml',
    ])
  })
  ...
  it('validates the Windows installer with its embedded blockmap and production destination', async () => {
    ...
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'alego-2.0.0-win-x64.exe',
      'latest.yml',
    ])
    expect(plan).toMatchObject({
      publicUrl: 'https://alego-updates.example.com/_/alego/desktop/stable/win-x64/',
  ...
  it('rejects a completed build from another alego version or deployment', async () => {
  ...
  it('rejects stale architecture metadata and modified updater bytes', async () => {
    ...
    const zipPath = join(paths.artifactsRoot, 'alego-1.2.3-mac-arm64.zip')
