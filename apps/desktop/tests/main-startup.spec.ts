  ...
  class FakeWindow extends EventEmitter {
    ...
    readonly webContents = Object.assign(new EventEmitter(), {
      ...
      send: vi.fn((channel: string, state: { phase?: string }) => {
        if (channel === 'alego-desktop:backend-state' && state.phase === 'error') errorPublished.resolve()
      }),
    })
    readonly show = vi.fn()
    ...
    async loadURL(url: string) {
      this.urls.push(url)
      if (url === 'alego-app://app/index.html') navigated.resolve()
    }
...
function invoke(channel: string): unknown {
  ...
  return handler({ senderFrame: { url: 'alego-app://shell/startup.html' } })
}
...
beforeEach(() => {
  ...
  vi.useFakeTimers()
  harness.reset()
  vi.stubEnv('ALEGO_DESKTOP_NODE_BINARY', 'test-node')
  vi.stubEnv('ALEGO_DESKTOP_PNPM_ENTRY', 'test-pnpm')
  vi.stubEnv('ALEGO_DESKTOP_ALEGO_DIR', 'test-runtime')
  vi.stubGlobal('process', { ...process, resourcesPath: 'desktop-test-resources' })
  vi.stubEnv('ALEGO_DESKTOP_HOST_INSPECT_PORT', undefined)
})
...
describe('desktop main startup', () => {
  ...
  it('withholds profile recovery after application resources fail to load', async () => {
    ...
    expect(html).toContain('alego-recovery://restart')
    expect(html).not.toContain('alego-recovery://reset')
    expect(html).not.toContain('alego-recovery://plugins')
  })
  ...
  it('reloads a crashed startup renderer in the same window', async () => {
    ...
    await harness.errorPublished.promise
    expect(window.urls).toEqual(['alego-app://shell/startup.html', 'alego-app://shell/startup.html'])
  ...
  it.each(['plugins', 'reset'])('runs %s recovery from a document with a broken preload', async (action) => {
    ...
    const event = { preventDefault: vi.fn() }
    window.webContents.emit('will-navigate', event, `alego-recovery://${action}/?`)
    ...
    expect(window.urls.at(-1)).toBe('alego-app://app/index.html')
  })
  ...
  it('shows the loading window before profile preparation and starts one actual Host', async () => {
    ...
    expect(window.urls).toEqual(['alego-app://shell/startup.html'])
    ...
    expect(harness.hosts[0]).toMatchObject({
      ...
      runtime: join('desktop-test-resources', 'alego'),
      profile: 'desktop-test-profile',
    })
    expect(harness.hosts[0]!.start).toHaveBeenCalledTimes(1)
    expect(harness.windows).toHaveLength(1)
    expect(window.urls).toEqual(['alego-app://shell/startup.html', 'alego-app://app/index.html'])
    expect(invoke(DESKTOP_IPC.backendStatus)).toEqual({ phase: 'ready' })
  })
  ...
  it('keeps startup errors and a successful retry in the same window', async () => {
    ...
    expect(harness.windows[0]!.urls).toEqual(['alego-app://shell/startup.html'])
    const nextStarted = harness.nextHostStart()
    ...
    await retry
    expect(harness.windows).toHaveLength(1)
    expect(harness.windows[0]!.urls.at(-1)).toBe('alego-app://app/index.html')
    expect(harness.dialog.showErrorBox).not.toHaveBeenCalled()
  })
