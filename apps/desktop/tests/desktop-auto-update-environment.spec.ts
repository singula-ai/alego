...
describe('desktop auto-update environment', () => {
  it('defaults packages and uploads to the test deployment', () => {
    ...
    expect(resolveDesktopAutoUpdateConfig({
      ...
      keyPrefix: '_/alego/desktop/stable/mac-arm64',
    })
  ...
  it('requires a configured HTTPS production origin', () => {
    expect(() => resolveDesktopAutoUpdateConfig({
      ALEGO_DESKTOP_AUTO_UPDATE_ENV: 'production',
    }, 'win32', 'x64')).toThrow(/DOWNLOAD_PROD_ORIGIN/u)
    expect(() => resolveDesktopAutoUpdateConfig({
      ALEGO_DESKTOP_AUTO_UPDATE_ENV: 'production',
      DOWNLOAD_PROD_ORIGIN: 'http://alego-updates.example.com',
    }, 'win32', 'x64')).toThrow(/HTTPS origin/u)
  })

  it('rejects unknown deployments and targets', () => {
    expect(() => resolveDesktopAutoUpdateEnvironment({
      ALEGO_DESKTOP_AUTO_UPDATE_ENV: 'staging',
