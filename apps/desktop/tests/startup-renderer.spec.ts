...
import type { AlegoDesktopStartupApi } from '../src/ipc.ts'
...
function startup(locale = 'en', status: Promise<DesktopBackendState> = Promise.resolve({ phase: 'starting' })) {
  ...
  const api: AlegoDesktopStartupApi = {
    protocolVersion: 1,
  ...
  Object.defineProperty(dom.window, 'alegoDesktop', { value: api })
...
it('keeps emergency diagnostics inert without shell assets', () => {
  ...
  expect([...dom.window.document.querySelectorAll('form')].map(form => form.action)).toEqual([
    'alego-recovery://restart', 'alego-recovery://plugins', 'alego-recovery://reset',
  ])
  dom.window.close()
})
...
it('offers only restart before emergency profile recovery is available', () => {
  ...
  expect([...dom.window.document.querySelectorAll('form')].map(form => form.action)).toEqual(['alego-recovery://restart'])
  dom.window.close()
})
