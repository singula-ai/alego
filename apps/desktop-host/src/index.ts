 ...
 * @module @singula-ai/alego-desktop-host
 */
...
import { ALEGO_LAUNCH_ENVIRONMENT_KEY } from '@singula-ai/alego-launch-environment'
import type {} from '@singula-ai/alego-api-gateway'
import type { ConnectionFetchHandler } from '@singula-ai/alego-client-connection'
import type {} from '@singula-ai/alego-client-modules'
import { renderIndexInjections, type IndexInjection } from '@singula-ai/alego-host-webserver'
import {
...
/** One request forwarded from Electron's `alego-app://` handler. */
...
export interface DesktopHostController {
  /** Installed alego version carried by this host. */
  readonly alegoVersion: string
...
const DESKTOP_STREAM_PATH = '/.alego/remote-stream'

const DESKTOP_TRANSPORT_SCRIPT = `globalThis.__ALEGO_TRANSPORT__={
  ownsHost:true,
...
function readManifest(path: string): PackageManifest {
  ...
  if (!isRecord(value)) throw new Error(`alego desktop: ${path} must contain a package manifest`)
  return {
...
function packageManifestPath(projectDir: string, packageName: string): string {
  ...
  if (!existsSync(path)) throw new Error(`alego desktop: installed package ${JSON.stringify(packageName)} has no manifest`)
  return path
}
...
function desktopPatches(runtimeDir: string, projectDir: string, allowLinkedPackages: boolean): PatchOptions[] {
  const alegoRoot = dirname(packageManifestPath(runtimeDir, '@singula-ai/alego'))
  const profile = loadProfileDirectory('alego desktop', projectDir, join(alegoRoot, 'package.json'))
  for (const layer of profile.layers) {
    if (!allowLinkedPackages && !isProjectPath(projectDir, layer.packageDir) && !isProjectPath(runtimeDir, layer.packageDir)) {
      throw new Error(`alego desktop: profile bundle ${JSON.stringify(layer.packageName)} resolved outside the Desktop runtime and profile`)
    }
  }
  const layers = [
    ...profile.layers.map(layer => layer.patches),
    profile.patches,
    loadOverlayPatches('alego desktop', DESKTOP_PATCH),
  ]
  ...
  if (agentPresets !== undefined) {
    layers.push([{
      ...
      config: {
        ...
        roots: [{ path: join(alegoRoot, 'config', 'agent-presets'), trust: 'system' }],
      },
    }])
  }
  return layers.flat()
...
function alegoVersion(runtimeDir: string): string {
  const manifest = readManifest(packageManifestPath(runtimeDir, '@singula-ai/alego'))
  if (typeof manifest.version !== 'string') throw new Error('alego desktop: installed alego manifest has no version')
...
function assetHandler(ctx: Context, runtimeDir: string): ConnectionFetchHandler {
  ...
  const distIndex = require.resolve('@singula-ai/alego-web-frontend/dist/index.html')
 ...
 * @param runtimeDir - immutable alego packages supplied by the Electron application.
...
export async function runDesktopHost(
  ...
  const environment = loadLayeredEnv('alego desktop')
  let current: Context | undefined
  const ctx = await boot('alego desktop', rootConfig, structuredClone(desktopPatches(
    resolve(runtimeDir),
    absoluteProject,
    options.allowLinkedPackages === true,
  )), (hostCtx) => {
    current = hostCtx
    hostCtx.provide(ALEGO_LAUNCH_ENVIRONMENT_KEY, environment)
  ...
  if (connection === undefined || clientModules === undefined || gateway === undefined) {
    ...
    throw new Error('alego desktop: composition did not provide connection, typertGateway, and clientModules')
  }
  ...
  return {
    alegoVersion: alegoVersion(resolve(runtimeDir)),
    cancel(streamId) {
      requests.get(streamId)?.abort()
    },
    async fetch(command, body) {
      if (disposing !== undefined) throw new Error('alego desktop: host is disposing')
...
async function main(): Promise<void> {
  ...
  if (runtimeDir === undefined || projectDir === undefined || process.send === undefined) {
    throw new Error('alego desktop: expected runtime and profile directories, byte pipes, and a Node IPC channel')
  }
  const option = process.argv[4]
  if (option !== undefined && option !== '--allow-linked-profile') {
    throw new Error(`alego desktop: unsupported internal option ${JSON.stringify(option)}`)
  }
  ...
  const writeResponse = (frame: Buffer): Promise<void> => {
    const write = responseWriteTail.then(async () => {
      if (responsePipe.destroyed) throw new Error('alego desktop: Electron response pipe is unavailable')
  ...
  send({
    ...
    alegoVersion: controller.alegoVersion,
  })
  ...
  const stop = (exitCode = 0): Promise<void> => {
    ...
    stopping ??= (async () => {
      ...
      const stopped = new Error('alego desktop: Host is stopping')
  ...
  const beginRequest = (frame: Extract<DesktopHostRequestFrame, { type: 'start' }>): void => {
    if (frame.streamId <= lastStreamId) {
      throw new Error(`alego desktop: Electron reused or reordered request stream ${String(frame.streamId)}`)
    }
    ...
    void run.catch(failTransport).finally(() => {
      ...
      openBody.error(new Error('alego desktop: response completed before the request body ended'))
      requestBodies.delete(frame.streamId)
      blockedRequests.delete(frame.streamId)
    ...
    switch (frame.type) {
      ...
      case 'data': {
        ...
        if (body === undefined) {
          ...
          throw new Error(`alego desktop: Electron sent body data for inactive stream ${String(frame.streamId)}`)
        }
      ...
      case 'end': {
        ...
        if (body === undefined) {
          ...
          throw new Error(`alego desktop: Electron ended inactive body stream ${String(frame.streamId)}`)
        }
        body.close()
        requestBodies.delete(frame.streamId)
        blockedRequests.delete(frame.streamId)
      ...
      case 'cancel': {
        if (frame.streamId > lastStreamId) {
          throw new Error(`alego desktop: Electron canceled unknown stream ${String(frame.streamId)}`)
        }
        const body = requestBodies.get(frame.streamId)
        body?.error(new Error('alego desktop: Electron canceled the request'))
        requestBodies.delete(frame.streamId)
        blockedRequests.delete(frame.streamId)
  ...
  requestPipe.once('end', () => {
    ...
    try {
      decoder.finish()
      failTransport(new Error('alego desktop: Electron request pipe ended'))
    } catch (error) {
      failTransport(error)
    }
  })
  ...
  process.on('message', (message: unknown) => {
    if (!isDesktopHostCommand(message)) {
      send({ type: 'fatal', message: 'alego desktop: invalid Electron IPC command' })
      void stop(1)
      return
    }
    void stop()
...
if (import.meta.main) {
  main().catch((error: unknown) => {
    ...
    else process.stderr.write(`alego desktop: ${message}\n`)
    process.exitCode = 1
  })
}
