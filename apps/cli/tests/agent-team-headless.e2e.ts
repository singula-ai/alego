...
describe('alego run with Agent Teams enabled', () => {
  it('runs two teammates, durable peer mail, dependent tasks, waiting, and final aggregation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'alego-agent-team-headless-'))
    try {
      ...
      await writeFile(join(profileDir, 'package.json'), JSON.stringify({
        ...
        private: true,
        dependencies: {
          '@singula-ai/alego-experimental-agent-team-profile': 'workspace:^',
        },
        alego: {
          profile: {
            bundles: [
      ...
      expect(
        result.exitCode,
        `alego headless profile exited unexpectedly.\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
      ).toBe(0)
