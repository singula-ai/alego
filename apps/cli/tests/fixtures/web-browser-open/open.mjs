...
export default async function open(url) {
  ...
  console.log(`alego browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__ALEGO_BOOT__'),
    apiKeyPresent: process.env.DEEPSEEK_API_KEY !== undefined,
    alegoHomePresent: process.env.ALEGO_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.alego-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
