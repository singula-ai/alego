/** Real `alego web` authentication against a temporary Harness home. */
...
const ALEGO_SOURCE_BIN = join(REPO_ROOT, 'apps/cli/src/bin.ts')
...
function cleanEnvironment(root: string, alegoHome: string): NodeJS.ProcessEnv {
...
async function startWeb(root: string, alegoHome: string, port: number): Promise<RunningWeb> {
  const child = spawn(process.execPath, [
    ...
    cwd: root,
    env: cleanEnvironment(root, alegoHome),
  ...
  const launchUrl = await new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error(`alego web did not become ready:\n${redact(output)}`))
    }, 90_000)
    ...
    child.once('exit', (code) => {
      fail(new Error(`alego web exited before readiness (${String(code)}):\n${redact(output)}`))
    })
  })
...
describe('alego web authentication through the real CLI', () => {
  it('rejects a forged loopback Host and preserves the browser cookie across restart', { timeout: 180_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'alego-web-auth-real-cli-'))
    const alegoHome = join(root, '.alego')
    const port = await freePort()
    let first: RunningWeb | undefined
    let second: RunningWeb | undefined
    try {
      first = await startWeb(root, alegoHome, port)
      ...
      await stopWeb(first)
      first = undefined
      second = await startWeb(root, alegoHome, port)
      ...
      const credentialMode = (await stat(join(alegoHome, '.credentials.yaml'))).mode & 0o777
