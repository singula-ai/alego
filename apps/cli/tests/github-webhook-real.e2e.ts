...
const MARKER = 'ALEGO_GITHUB_WEBHOOK_REAL_E2E_OK'
...
function observeProcess(child: ChildProcess): ProcessObservation {
  ...
  const timer = setTimeout(() => {
    if (!settled) rejectReady(new Error(`alego web did not become ready within 90s:\n${output}`))
  }, 90_000)
  timer.unref()
  const append = (chunk: Buffer | string): void => {
    output = `${output}${String(chunk)}`.slice(-100_000)
    const match = /alego web: (http:\/\/[^\s]+)/u.exec(output)
  ...
  child.once('exit', (code) => {
    if (!settled) rejectReady(new Error(`alego web exited before readiness (code ${String(code)}):\n${output}`))
  })
...
async function eventually<T>(
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`alego web exited while waiting for ${label} (code ${String(child.exitCode)}):\n${processOutput()}`)
    }
    try {
...
async function sendGitHubDelivery(origin: string): Promise<Response> {
  const body = JSON.stringify({
    ...
    number: 4242,
    repository: { full_name: 'singula-ai/alego' },
    pull_request: {
      title: 'Real CLI webhook e2e',
      html_url: 'https://github.com/singula-ai/alego/pull/4242',
      draft: false,
      user: { login: 'octocat' },
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('GitHub webhook through the real alego CLI and model', () => {
  it('creates, attaches, prompts, and completes a Workspace Session', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'alego-github-webhook-real-'))
    ...
    const child = spawn(process.execPath, [
      ...
      env: {
        ...
        ALEGO_GITHUB_E2E_MARKER: MARKER,
        ALEGO_GITHUB_E2E_WORKSPACE: workspacePath,
        ALEGO_GITHUB_WEBHOOK_PORT: String(webhookPort),
        ALEGO_GITHUB_WEBHOOK_SECRET: SECRET,
