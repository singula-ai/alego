...
// The alego built bin cold-starts slowly on the contended self-hosted Windows pool; the
...
const alegoBin = join(repoRoot, 'apps/cli/lib/bin.js')
...
async function runBuiltBin(
  ...
  const result = await execa(process.execPath, [alegoBin, ...args], {
    input: '',
  ...
  if (result.timedOut) {
    throw new Error(`alego built bin did not exit within ${SPAWN_TIMEOUT_MS / 1_000}s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`alego profile lifecycle marker did not appear: ${file}`)
 ...
 * alego.profile.bundles, no alego-base — proving out-of-box composition machinery without
 * booting the entire product tree.
 */
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  const home = mkdtempSync(join(tmpdir(), 'alego-profile-lifecycle-'))
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.ALEGO_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'alego-profile-lifecycle',
    private: true,
    dependencies: {},
    alego: { profile: { bundles: ['alego-lifecycle-bundle'] } },
  }, undefined, 2))
  // Hand-place the "installed" bundle where profile resolution finds it.
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  const linkTarget = join(profileDir, 'node_modules', 'alego-lifecycle-bundle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  try {
...
function startProfileLifecycle(fixture: ProfileLifecycleFixture, args: readonly string[] = []) {
  return execa(process.execPath, [alegoBin, '--profile', 'lifecycle', ...args], {
    cwd: fixture.home,
    input: '',
    timeout: SPAWN_TIMEOUT_MS,
    killSignal: 'SIGKILL',
...
function createStartupFixture(): StartupFixture {
  const home = mkdtempSync(join(tmpdir(), 'alego-profile-startup-'))
  ...
  const bundleDir = join(profileDir, 'node_modules', 'alego-startup-bundle')
  mkdirSync(bundleDir, { recursive: true })
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.ALEGO_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'alego-startup-bundle',
    version: '0.0.0',
    type: 'module',
    alego: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'alego-profile-startup',
    private: true,
    dependencies: {},
    alego: { profile: { bundles: ['alego-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
function startStartupProfile(fixture: StartupFixture, args: readonly string[]) {
  return execa(process.execPath, [alegoBin, '--profile', 'startup', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    timeout: SPAWN_TIMEOUT_MS,
...
describe.skipIf(!existsSync(alegoBin))('alego BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('alego --profile web')
    expect(help.stdout).toContain('alego plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    const home = mkdtempSync(join(tmpdir(), 'alego-app-help-'))
    try {
      ...
      expect(web.stdout).toContain('Usage: alego --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('alego web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('alego web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: alego --profile headless')
      ...
      expect(sdkHelp.stdout).toContain('Usage: alego --profile sdk')
      ...
      expect(acpHelp.stdout).toContain('Usage: alego --profile acp')
  ...
  it('reports SDK startup failure when stdin reaches EOF first', async () => {
    const home = mkdtempSync(join(tmpdir(), 'alego-built-sdk-startup-failure-'))
    const patch = join(home, 'broken-sdk.cordis.yml')
    writeFileSync(patch, [
      '- insert:',
      '    - id: missing-sdk-startup-plugin',
      '      name: "@singula-ai/alego-missing-sdk-startup-plugin"',
      '',
    ].join('\n'))
    try {
      ...
      expect(result.stderr).toContain('@singula-ai/alego-missing-sdk-startup-plugin')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('serves the SDK protocol with an absolute-path overlay plugin and exits after shutdown', async () => {
    const home = mkdtempSync(join(tmpdir(), 'alego-built-sdk-'))
    ...
    const child = execa(process.execPath, [alegoBin, '--profile', 'sdk', '--patch', patch], {
      cwd: home,
      reject: false,
      timeout: SPAWN_TIMEOUT_MS,
      killSignal: 'SIGKILL',
  ...
  it('runs a mock-backed ACP turn through the acp profile and exits on disconnect', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'alego-built-acp-'))
    const child = execa(process.execPath, [alegoBin, '--profile', 'acp'], {
      cwd: home,
      reject: false,
      timeout: SPAWN_TIMEOUT_MS,
      killSignal: 'SIGKILL',
    ...
    const clientApp = createAcpClientApp({ name: 'alego-built-acp-profile' })
    ...
    try {
      ...
      expect(initialized.agentInfo).toMatchObject({ name: 'alego-acp' })
  ...
  it('runs the headless profile through its app-owned task positional', async () => {
    const apiKey = 'built-alego-headless-key'
    const server = await startMockLlmServer({
    ...
    const home = mkdtempSync(join(tmpdir(), 'alego-built-headless-'))
    try {
      ...
      expect(result.stderr).toBe('alego: reasoning:\nInspecting the published entry.')
  ...
  it('does not load a project environment for --version', async () => {
    const project = mkdtempSync(join(tmpdir(), 'alego-version-project-'))
  ...
  it.skipIf(process.platform === 'win32')('runs through an installed-style symlink', async () => {
    const installation = mkdtempSync(join(tmpdir(), 'alego-bin-link-'))
    const installedBin = join(installation, 'alego')
    symlinkSync(alegoBin, installedBin)
    try {
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'alego-missing-profile-'))
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { ALEGO_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('alego plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS + 30_000)
  ...
  it('creates a custom profile from the shipped web template before booting it', async () => {
    const home = mkdtempSync(join(tmpdir(), 'alego-from-default-profile-'))
    try {
      const created = await runBuiltBin(
        ['--profile', 'rescue', '--from-default-profile', 'web', '--help'],
        { ALEGO_HOME: home, ALEGO_TELEMETRY_DISABLED: '1' },
      )
      expect(created.code).toBe(0)
      expect(created.stderr).toBe('')
      expect(created.stdout).toContain('Usage: alego --profile web')

      const dir = join(home, 'profiles', 'rescue')
      const manifest = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8')) as {
        dependencies: Record<string, string>
        alego: { profile: { bundles: string[]; patchReload: string } }
      }
      expect(manifest.dependencies).toEqual({})
      expect(manifest.alego.profile).toEqual({
        bundles: ['@singula-ai/alego-base', '@singula-ai/alego-web-app'],
        patchReload: 'live',
      })
      ...
      expect(reopened.stdout).toContain('Usage: alego --profile web')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 3 + 30_000)
  ...
  it('keeps a newly created profile when application boot rejects its arguments', async () => {
    const home = mkdtempSync(join(tmpdir(), 'alego-from-default-profile-failed-boot-'))
    try {
      ...
      expect(retried.stdout).toContain('Usage: alego --profile web')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('uses the launching endpoint and managed credential through the published entry', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'alego-home-environment-'))
    const project = mkdtempSync(join(tmpdir(), 'alego-home-project-'))
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: alego exited 13 with no diagnostic instead of settling
    // ([vendor/README.md](../../../vendor/README.md)).
    const home = mkdtempSync(join(tmpdir(), 'alego-invalid-patch-'))
    try {
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($ALEGO_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `alego plugin --profile x add .` from a plugin checkout must install THAT
    // checkout — pnpm's cwd is the profile directory, so an un-anchored `.`
    // would self-link the profile.
    const home = mkdtempSync(join(tmpdir(), 'alego-plugin-anchor-'))
    const checkout = mkdtempSync(join(tmpdir(), 'alego-plugin-checkout-'))
    try {
      ...
      const result = await execa(process.execPath, [alegoBin, 'plugin', '--profile', 'anchor', 'add', '.'], {
        cwd: checkout,
        input: '',
        timeout: SPAWN_TIMEOUT_MS,
        killSignal: 'SIGKILL',
      ...
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        dependencies: Record<string, string>
        alego: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.alego.profile.bundles).toContain('anchored-bundle')
      ...
      expect(afterRemove.alego.profile.bundles).not.toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained alego.bundle in a later update', async () => {
    ...
    // version declares alego.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign pnpm verb.
    const home = mkdtempSync(join(tmpdir(), 'alego-plugin-update-'))
    try {
  ...
  describe('config dump', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'alego-dump-bin-')) })
    ...
    it('creates a custom profile from a shipped template before printing it', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain('# == @singula-ai/alego-web-app')
      expect(existsSync(join(home, 'profiles', 'rescue', 'package.json'))).toBe(true)
    }, SPAWN_TIMEOUT_MS + 30_000)
    ...
    it('prints the exact standalone sdk-minimal tree without alego-base', async () => {
      const { stdout, code, stderr } = await runBuiltBin(
      ...
      expect(stdout).toContain('# == @singula-ai/alego-sdk-minimal')
      expect(stdout).not.toContain('@singula-ai/alego-base')
      expect(stdout).not.toContain('@singula-ai/alego-web-app')
    }, SPAWN_TIMEOUT_MS * 2 + 30_000)

    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the web profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { ALEGO_HOME: home })
