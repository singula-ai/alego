...
import { installProxyFromEnvironment } from '@singula-ai/alego-http-proxy'
import { ALEGO_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@singula-ai/alego-launch-environment'
import { provideCmdline, type AppReady } from '@singula-ai/alego-cmdline'
 ...
 * `alego.profile.bundles` order (a base-backed profile gets the base bundle's
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  if (composed.profile.patchReload === 'live'
    ...
    try {
      // Config-only HMR for the live profile patch layer: alego-base disables
