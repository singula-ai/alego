/**
 * Commander adapter for the `alego` command line.
 *
 ...
 * `@singula-ai/alego-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `alego --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `alego --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @singula-ai/alego/args
 */
...
/** The resolved `alego` invocation. Help, version, and errors exit inside {@link parseAlegoArgs}. */
export type AlegoInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  alego --profile web                          boot the web profile (same as: alego web)
  alego --profile rescue --from-default-profile web
                                             create rescue from the shipped web template, then boot it
  alego --profile headless "run the tests"     answer one task, print the result, and exit
  alego --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  alego --profile tui --resume <session>       arguments after the launcher flags reach the app
  alego --profile web --help                   the web app's own flags and help
  alego plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): AlegoInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseAlegoArgs(argv: readonly string[], version: string): AlegoInvocation {
  let resolved: AlegoInvocation | undefined
  ...
  program
    ...
    .description('alego: boot an Alego profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `alego -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: alego --profile <name> --help)')
    .option('--profile <name>', 'the profile under $ALEGO_HOME/profiles to boot')
    ...
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      ...
      // `alego -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: alego web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('alego: no invocation resolved')
  return resolved
}
