...
export function packageReadmeSummaryErrors(file: string, source: string): string[] {
  ...
  const template = kind === undefined
    ? '.agents/skills/alego-doc/templates/'
    : `.agents/skills/alego-doc/templates/${kind}.md`
  return [
    `${file}: Summary has ${String(words)} words; the limit is ${String(MAX_PACKAGE_README_SUMMARY_WORDS)}. Read .agents/skills/alego-doc/SKILL.md and ${template} before rewriting it.`,
  ]
}
