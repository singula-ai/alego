...
describe('documentation site build', () => {
  it.each([
    ...
    const root = mkdtempSync(join(tmpdir(), 'alego-doc-build-'))
    roots.push(root)
  ...
  it('refuses to remove the site root or an outside directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'alego-doc-build-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'alego-doc-build-outside-'))
  ...
  it('unlinks a link-shaped output without removing its target', () => {
    const root = mkdtempSync(join(tmpdir(), 'alego-doc-build-link-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'alego-doc-build-link-target-'))
    roots.push(root, outside)
    const outDir = join(root, '.dist')
  ...
  it('refuses output whose nearest existing parent resolves outside the site root', () => {
    const root = mkdtempSync(join(tmpdir(), 'alego-doc-build-parent-link-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'alego-doc-build-parent-link-target-'))
    roots.push(root, outside)
...
describe('rewriteMarkdown', () => {
  ...
  it('uses raw GitHub content for unpublished images when nothing places them', () => {
    ...
    expect(rewriteMarkdown('![logo](../packages/logo.svg)\n', {
      ...
      repoRoot: root,
      repositoryRef: 'abc123',
    })).toBe('![logo](https://raw.githubusercontent.com/singula-ai/alego/abc123/packages/logo.svg)\n')
  })
