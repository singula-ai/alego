...
export function collectSessionFormatMigrations(
  ...
  for (const discovered of globSync('packages/session/session-format-v*-to-v*/package.json', { cwd: scanRoot }).sort()) {
    ...
    const metadata = manifest.alego?.sessionFormatMigration
    if (metadata === undefined) {
      throw new Error(`gen-session-format-catalog: ${rel} lacks alego.sessionFormatMigration`)
    }
    ...
    const expectedPackageName = `@singula-ai/alego-session-format-v${from}-to-v${to}`
  ...
  if (catalog.dependencies?.['@singula-ai/alego-session'] !== undefined
    || catalog.peerDependencies?.['@singula-ai/alego-session'] === undefined
    || catalog.devDependencies?.['@singula-ai/alego-session'] === undefined) {
    throw new Error(
      'gen-session-format-catalog: catalog must share @singula-ai/alego-session through peer + dev dependencies',
    )
  }
...
export function renderSessionFormatCatalog(
  ...
  return [
    ...
    ' */',
    '',
    "import { KNOWN_SESSION_EVENT_TYPES } from '@singula-ai/alego-session'",
    "import { createSessionFormatCatalog } from '@singula-ai/alego-session-format'",
