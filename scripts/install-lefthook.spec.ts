...
const replaceLockPath = process.env.ALEGO_TEST_LEFTHOOK_REPLACE_LOCK_PATH
...
// its assertions. The value matches ALEGO_COVERAGE_TEST_TIMEOUT_MS, which the
...
describe('worktree-local Lefthook installer', { timeout: 90_000 }, () => {
  ...
  it('does not release an installer lock whose ownership changed', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, {
      ALEGO_TEST_LEFTHOOK_REPLACE_LOCK_PATH: lockPath,
    })
