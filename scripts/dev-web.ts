...
const SHELL_PACKAGE = '@singula-ai/alego-web-frontend'
...
if (isMain) {
  ...
  for (const name of Object.keys(process.env)) {
    if (name === CLIENT_BUILD_PROFILE_SELECTOR || name.startsWith('ALEGO_CLIENT_')) {
      Reflect.deleteProperty(process.env, name)
    }
  }
  for (const [name, value] of Object.entries(buildEnvironment)) {
    if (name.startsWith('ALEGO_CLIENT_') && value !== undefined) process.env[name] = value
  }
