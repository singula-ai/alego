 ...
 * `@singula-ai/alego-*` listed 49 candidate globs and `@singula-ai/alego-*\/invariant`
...
const PREFIX = '@singula-ai/alego-'

/** One workspace package the generated region maps. */
interface PackageAlias {
  /** Bare specifier, e.g. `@singula-ai/alego-session`. */
 ...
 * @returns Every directory whose manifest names a `@singula-ai/alego-` package and that carries `src`.
 */
 ...
 * `@singula-ai/alego-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@singula-ai/alego-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@singula-ai\/alego-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@singula-ai\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
