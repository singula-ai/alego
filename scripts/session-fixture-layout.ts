...
import { SessionFormatEventCollector, type SessionFormatArtifactDecoder, type SessionFormatCodec } from '@singula-ai/alego-session-format'
import { releasedV0SessionFormatCodec, releasedV1SessionFormatCodec } from '@singula-ai/alego-session-format-v0-to-v1'
import { releasedV2SessionFormatCodec } from '@singula-ai/alego-session-format-v1-to-v2'
...
function validationHeader(value: unknown): unknown {
  ...
  if (typeof header.cwd === 'string' && /^\{\{cwd\}\}(?:\/|$)/.test(header.cwd)) {
    header.cwd = header.cwd.replace('{{cwd}}', '/alego-snapshot-cwd')
  }
  return header
}
