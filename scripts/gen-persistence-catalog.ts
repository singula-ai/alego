...
const SESSION_PACKAGE = '@singula-ai/alego-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@singula-ai/alego-session/types'
 ...
 * top-level declaration (in `@singula-ai/alego-session`) and any declaration
 * merge inside a `declare module '@singula-ai/alego-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @singula-ai/alego-session/known-event-types',
    ' */',
    '',
    '/**',
