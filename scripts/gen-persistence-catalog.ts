...
const SESSION_PACKAGE = '@alego/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@alego/session/types'
 ...
 * top-level declaration (in `@alego/session`) and any declaration
 * merge inside a `declare module '@alego/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @alego/session/known-event-types',
    ' */',
    '',
    '/**',
