...
function createFixture(attributes = true): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'alego-translation-pairing-merge-'))
  fixtures.push(root)
  ...
  if (attributes) write(root, '.gitattributes', '*.i18n.yaml merge=alego-translation-pairing\n')
  return fixture
}
...
// The value matches ALEGO_COVERAGE_TEST_TIMEOUT_MS, which the Windows coverage
