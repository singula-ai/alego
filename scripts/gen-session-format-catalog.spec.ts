...
function fixture(edges: Array<[number, number]>): string {
  const root = mkdtempSync(join(tmpdir(), 'alego-session-format-catalog-'))
  ...
  for (const [from, to] of edges) {
    ...
    const name = `@singula-ai/alego-session-format-v${from}-to-v${to}`
    mkdirSync(dir, { recursive: true })
    catalogDependencies[name] = 'workspace:^'
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      name,
      alego: { sessionFormatMigration: {
        from, to, export: '.',
      ...
      dependencies: from === 0
        ? { '@singula-ai/alego-session-format': 'workspace:^' }
        : {
          '@singula-ai/alego-session-format': 'workspace:^',
          [`@singula-ai/alego-session-format-v${from - 1}-to-v${from}`]: 'workspace:^',
        },
    }))
  }
  ...
  writeFileSync(join(catalog, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@singula-ai/alego-session': 'workspace:^' },
    devDependencies: { '@singula-ai/alego-session': 'workspace:^' },
  }))
  return root
}
...
function edgeManifest(root: string, from: number, to: number): {
  ...
  value: Record<string, unknown> & {
    ...
    alego: { sessionFormatMigration: Record<string, unknown> }
  }
} {
...
describe('session format catalog generator', () => {
  ...
  it('discovers one complete adjacent chain and renders direct imports', () => {
    ...
    expect(output).toContain("from '@singula-ai/alego-session-format-v0-to-v1'")
    ...
    expect(output).toContain("from '@singula-ai/alego-session'")
  ...
  it('refuses a later edge whose declared source codec does not continue the prior target', () => {
    ...
    manifest.value.alego.sessionFormatMigration['sourceCodec'] = 'UnrelatedV1Codec'
  ...
  it('requires every later edge to depend on the package that owns its source codec', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    delete manifest.value.dependencies['@singula-ai/alego-session-format-v0-to-v1']
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must depend on @singula-ai\/alego-session-format-v0-to-v1/)
  })

  it('requires the package name to identify its declared adjacent edge', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    manifest.value['name'] = '@singula-ai/alego-session-format-other'
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/name must be @singula-ai\/alego-session-format-v1-to-v2/)
  })
  ...
  it('requires the catalog to share the installed Session package as a peer', () => {
    ...
    manifest.dependencies['@singula-ai/alego-session'] = 'workspace:^'
    delete manifest.peerDependencies['@singula-ai/alego-session']
    writeFileSync(path, JSON.stringify(manifest))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must share @singula-ai\/alego-session through peer \+ dev dependencies/)
  })
})
