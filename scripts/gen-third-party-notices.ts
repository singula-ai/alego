...
const FIRST_PARTY = new Set([
  '@singula-ai/node-addon-system',
  '@singula-ai/node-addon-system-darwin-arm64',
  '@singula-ai/node-addon-system-darwin-x64',
  '@singula-ai/node-addon-system-linux-arm64',
  '@singula-ai/node-addon-system-linux-x64',
])
...
const PYTHON_METADATA: Record<string, { license: string; repo: string; role: string }> = {
  pydantic: { license: 'MIT', repo: 'https://github.com/pydantic/pydantic', role: 'runtime dependency of `alego-sdk`' },
...
function renderNonPermissiveNote(deps: ExternalDep[]): string {
  ...
  return `\n${subject} ${named.length === 1 ? 'runs' : 'run'} only as development tooling; their code is not linked into or distributed with any Alego artifact.\n`
}
...
Alego is licensed under [MIT](LICENSE). It depends on the third-party software listed below. Each project remains under its own license; nothing in this file changes those terms.
...
The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \`@singula-ai\` scope. All are MIT-licensed; each directory preserves its upstream \`LICENSE\` file. Exact upstream commits and local modifications are recorded in [\`vendor/README.md\`](vendor/README.md).
...
\`@singula-ai/node-addon-system\` (and its platform packages) is built and released from this repository under BSD 3-Clause. It is listed here for completeness; it is first-party, not third-party.
`
}
