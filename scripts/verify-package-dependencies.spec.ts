...
function sourceFacts(
  ...
  const root = mkdtempSync(join(tmpdir(), 'alego-dependency-source-'))
  roots.push(root)
...
function generatedHostFixture(mode: 'schema' | 'object'): { root: string; manifestPath: string; source: string } {
  const root = mkdtempSync(join(tmpdir(), 'alego-generated-host-dependencies-'))
  roots.push(root)
...
describe('dependency sections', () => {
  ...
  it('validates every third-party range before writing any manifest in a repair batch', () => {
    const root = mkdtempSync(join(tmpdir(), 'alego-dependency-batch-'))
    roots.push(root)
    const valid = { ...facts({ name: '@singula-ai/alego-first' }), manifestPath: 'first.json' }
    const base = facts({ name: '@singula-ai/alego-second' })
  ...
  it('moves browser-only third-party imports to development dependencies without changing their ranges', () => {
    const manifest: PackageDependencyManifest = {
      ...
      dependencies: { '@singula-ai/alego-runtime': 'workspace:^', external: '^1.2.3' },
