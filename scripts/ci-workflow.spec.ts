...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    for (const [jobName, job] of [['windows-build', windowsBuild], ['windows-coverage', windowsCoverage], ['windows-native-tests', windowsNativeTests], ['windows-observational', windowsObservational]] as const) {
      ...
      expect(job['runs-on'], `${jobName} runs-on must use the Windows failover switch`).toContain('ALEGO_CI_FAILOVER_WINDOWS')
      expect(job['runs-on'], `${jobName} runs-on must not use the Linux failover switch`).not.toContain('ALEGO_CI_FAILOVER_LINUX')
      expect(job['runs-on']).toContain('self-hosted')
      expect(job['runs-on']).toContain('alego-win-ci')
    ...
    // windows-coverage uses the two-partition profile for hosted runners.
    expect(windowsCoverage.name).toBe('windows node 24 / coverage')
    expect(windowsCoverage.env).toMatchObject({ ALEGO_COVERAGE_PARTITIONS: '2' })
    ...
    expect(serialWindows.if).toBe("github.event_name == 'push' && github.ref == 'refs/heads/main' && vars.ALEGO_SELF_HOSTED_STANDBY_ENABLED == 'true'")
    ...
    expect(serialGate!.env).toMatchObject({ ALEGO_COVERAGE_TEST_TIMEOUT_MS: '90000' })
    ...
    expect(node24Bench.steps).toContainEqual({
      ...
      run: 'pnpm --filter @singula-ai/alego-benchmarks exec playwright install --with-deps chromium',
    })
    expect(JSON.stringify(node24Bench.steps)).not.toContain('ALEGO_CI_FAILOVER_LINUX')
    expect(node24Bench.steps).toContainEqual({
      name: 'Run performance benchmarks',
      env: { ALEGO_GATE_VERBOSE: '1' },
    ...
    // and the verdict job resolve their pool through ALEGO_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('ALEGO_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('ALEGO_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
      expect(job['runs-on']).toContain('blacksmith-16vcpu-ubuntu-2404')
    }
    expect(aggregate['runs-on']).toContain('ALEGO_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('ALEGO_CI_FAILOVER_WINDOWS')
    ...
    for (const [name, selector, variable, pool, hosted] of [
      ['linux gates', selectors.linux, 'ALEGO_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'ubuntu-24.04'],
      ['linux aggregate', selectors.linuxAggregate, 'ALEGO_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'ubuntu-latest'],
      ['windows lanes', selectors.windows, 'ALEGO_CI_FAILOVER_WINDOWS', ['self-hosted', 'alego-win-ci', 'windows'], 'windows-2025'],
    ] as const) {
    ...
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers], ['node-compat', nodeCompat]] as const) {
      expect(job.env, `${jobName} must enable fail-fast`).toMatchObject({ ALEGO_GATE_FAIL_FAST: '1' })
    }
    ...
    expect(windowsBuild.env, 'windows-build must enable fail-fast').toMatchObject({ ALEGO_GATE_FAIL_FAST: '1' })
    expect(windowsCoverage.env, 'windows-coverage must enable fail-fast').toMatchObject({ ALEGO_GATE_FAIL_FAST: '1' })
    ...
    expect(windowsObservational.env).not.toMatchObject({ ALEGO_GATE_FAIL_FAST: '1' })
  })
  ...
  it('gates standalone keyless blacksmith jobs and benchmark tiers on the failover variables', () => {
    ...
    expect(expectedFilenames['runs-on']).toContain('ALEGO_CI_FAILOVER_LINUX')
    ...
    expect(sandbox['runs-on']).toContain('ALEGO_CI_FAILOVER_LINUX')
    ...
    for (const name of ['larger-runner-benchmark', 'consolidated-runner-benchmark'] as const) {
      ...
      expect(benchmark['runs-on']).toContain('ALEGO_CI_FAILOVER_LINUX')
      expect(benchmark['runs-on']).toContain('ALEGO_CI_FAILOVER_WINDOWS')
  ...
  it('cancels superseded master runs without changing the post-merge job inventory', () => {
    ...
    for (const name of ['serial-linux-selfhosted', 'serial-windows']) {
      ...
      expect(job.if).toBe("github.event_name == 'push' && github.ref == 'refs/heads/main' && vars.ALEGO_SELF_HOSTED_STANDBY_ENABLED == 'true'")
    }
...
describe('DeepSeek e2e workflow', () => {
  ...
  it('bounds profile subprocess fan-out to the tested e2e default', () => {
    ...
    expect(step).toMatchObject({ env: { ALEGO_E2E_MAX_WORKERS: 4 } })
  })
})
  ...
  it('is manual-only and fails loud before running the focused live suite', () => {
    ...
    expect(e2b).toMatchObject({
      env: {
        ...
        ALEGO_E2E_MAX_WORKERS: '1',
        ALEGO_EXAMPLE_MODE: 'lib',
      },
    })
...
describe('Python release workflows', () => {
  it('keeps complete wheel validation separate from protected public publication', () => {
    ...
    expect(pythonCompatSteps).toContain('dist/alego_sdk-$VERSION-py3-none-any.whl')
    expect(pythonCompatSteps).toContain('dist/alego_runtime_bin-$VERSION-py3-none-manylinux_2_28_x86_64.whl')
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(installedRealApiPosix)).toContain('-u ALEGO_RUNTIME_MODE')
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e ALEGO_TELEMETRY_DISABLED')
  })
...
describe('npm release workflows', () => {
  ...
  it('runs dependency policy and npm layout checks in the ALEGO release workflow', () => {
    ...
    if (!isRecord(workflow.on) || !Array.isArray(dependencies.steps)) {
      throw new TypeError('ALEGO release workflow must define triggers and dependency steps')
    }
...
describe('Documentation site publication', () => {
  it('keeps Pages deployment dispatch-only from an alego-v* tag', () => {
    ...
    // RELEASE_PUBLISH makes release:verify reject every ref that is not an alego-v*
    ...
    expect(verify).toMatchObject({
      ...
      run: 'pnpm run release:verify --family alego',
    })
