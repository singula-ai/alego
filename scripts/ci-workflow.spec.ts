...
describe('DeepSeek e2e workflow', () => {
  it('keeps real-model branding capture opt-in and retains the default E2E suite', () => {
    const workflow = loadWorkflow('.github/workflows/e2e.yml')
    expect(workflow.on).toMatchObject({
      workflow_dispatch: { inputs: { record_brand_demo: { type: 'boolean', default: false } } },
    })
    ...
    expect(steps.find(step => step.name === 'Record real-model branding demo')).toMatchObject({
      if: 'inputs.record_brand_demo',
      env: {
