...
export interface ClientDeclaration {
  ...
  /** Exact runtime specifiers used to validate `alego.client.external` declarations. */
...
/**
 * Repair malformed or redundant `alego.client` declaration entries.
 * @param root - Absolute repository root.
    ...
    for (const specifier of new Set(pkg.external)) {
      ...
      if (supplier === pkg.name) {
        ...
        if (pkg.runtimeSourceSpecifiers[specifier] === undefined) {
          violations.push(
            pkg.manifest + ': alego.client.external ' + JSON.stringify(specifier)
