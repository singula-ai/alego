/** Application-entrypoint classification and alego-launch enforcement. */
...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'alego-application-entrypoints-'))
  cleanups.push(root)
  return root
}
...
describe('application entrypoints', () => {
  ...
  it('rejects a package-level application bin', () => {
    ...
    expect(applicationEntrypointViolations(root)).toEqual([
      'packages/example/app/package.json: package bin bypasses the alego launcher; applications use apps/cli profiles',
    ])
  })
  ...
  it('rejects a private Python application carrier outside alego', () => {
    const root = fixture()
  ...
  it('rejects a new root demo until its launch role is classified', () => {
    const root = fixture()
    write(root, 'package.json', JSON.stringify({ scripts: { 'demo:new-app': 'alego --profile new-app' } }))

    expect(applicationEntrypointViolations(root)).toEqual([
      'package.json scripts.demo:new-app: demo launcher has no explicit alego or in-process classification',
    ])
  })
})
