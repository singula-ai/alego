...
describe('Client UI i18n source check', () => {
  ...
  it('accepts the product wordmark without exempting copy that contains it', () => {
    expect(messages('<svg><text>alego</text></svg>')).toEqual([])
    expect(messages('<span>alego settings</span>')).toEqual(['alego settings'])
  })
