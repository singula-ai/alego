...
describe('collectPackageGraph', () => {
  ...
  it('rejects a missing in-repo peer', () => {
    ...
    expect(() => collectPackageGraph(root, ['client'], 'fixture'))
      .toThrow('fixture: @singula-ai/alego-consumer references missing in-repo peer @singula-ai/alego-missing')
  })
})
...
describe('renderModuleGraph', () => {
  it('renders the same peer edge in both generated languages', () => {
    const packages = [
      { short: 'provider', name: '@singula-ai/alego-provider', group: 'core', rel: 'packages/core/provider', deps: [] },
      { short: 'consumer', name: '@singula-ai/alego-consumer', group: 'core', rel: 'packages/core/consumer', deps: ['provider'] },
    ]
