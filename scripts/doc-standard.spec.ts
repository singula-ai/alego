 ...
 * stays valid, the consolidated `alego-doc` skill carries no stale copied
...
/**
 * The kind system: each label maps to exactly one template in the alego-doc
...
const KIND_TEMPLATES: Readonly<Record<string, string>> = {
  'package-group': '.agents/skills/alego-doc/templates/package-group.md',
  'package-reference': '.agents/skills/alego-doc/templates/package-reference.md',
  'package-library': '.agents/skills/alego-doc/templates/package-library.md',
  'package-bundle': '.agents/skills/alego-doc/templates/package-bundle.md',
}
 ...
 * bundle (`alego.bundle.patch`). Each entry names why the package is a
...
/** Whether the package manifest declares `alego.bundle.patch`. */
function declaresBundle(dir: string): boolean {
  const manifest = resolve(root, dir, 'package.json')
  if (!existsSync(manifest)) return false
  const metadata = JSON.parse(readFileSync(manifest, 'utf8')) as { alego?: { bundle?: { patch?: string } } }
  return metadata.alego?.bundle?.patch !== undefined
}
...
function validateSessionFormatRelease(source: string, currentWriterVersion: number): SessionFormatRelease {
  ...
  if (typeof evidenceTag !== 'string'
    || !/^alego-v\d+\.\d+\.\d+(?:-[\dA-Za-z]+(?:[.-][\dA-Za-z]+)*)?(?:\+[\dA-Za-z]+(?:[.-][\dA-Za-z]+)*)?$/u.test(evidenceTag)) {
    throw new Error('evidenceTag must be a non-empty alego-v version tag without URL delimiters')
  }
  const repository = 'https://github.com/singula-ai/alego'
  for (const link of [
...
describe('Session format release authority', () => {
  ...
  it('rejects empty, malformed, and URL-injecting evidence tags', () => {
    const { record, links } = sessionFormatReleaseFixture()
    for (const tag of [
      null, true, 1, '', ' ', 'alego-v', record.evidenceTag.replace('alego-v', 'v'),
      ...
      expect(() => validateSessionFormatRelease(source, record.latestReleasedVersion), String(tag)).toThrow('alego-v version tag')
    }
  })
...
describe('alego-doc skill consolidation', () => {
  it('carries no prototype-era language', () => {
    const files = [
      '.agents/skills/alego-doc/SKILL.md',
      '.agents/skills/alego-doc/references/metadata-links-i18n.md',
      '.agents/skills/alego-doc/references/structure-hierarchy.md',
      '.agents/skills/alego-doc/references/style.md',
      '.agents/skills/alego-doc/references/review.md',
      '.agents/skills/alego-doc/references/website-sync.md',
    ]
  ...
  it('copies no stale website sidebar or section-owner values', () => {
    const source = readFileSync(resolve(root, '.agents/skills/alego-doc/references/website-sync.md'), 'utf8')
  ...
  it('keeps the reference example linked from the skill', () => {
    const skill = readFileSync(resolve(root, '.agents/skills/alego-doc/SKILL.md'), 'utf8')
  ...
  it('defines controlled English as a precision-preserving review discipline', () => {
    const skill = readFileSync(resolve(root, '.agents/skills/alego-doc/SKILL.md'), 'utf8')
    const style = readFileSync(resolve(root, '.agents/skills/alego-doc/references/style.md'), 'utf8')
  ...
  it('maps every kind label to exactly one skill template that exists', () => {
    const templateFiles = globSync('.agents/skills/alego-doc/templates/*.md', { cwd: root }).map(path => path.split(sep).join('/')).sort()
