...
const tmp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'alego-profile-'))
  tempRoots.push(dir)
  return dir
}
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    expect(manifest.alego?.profile?.patchReload).toBe('live')
    ...
    expect(readProfileManifest('t', dir).alego?.profile?.patchReload).toBe('live')
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @singula-ai/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web?.bundles).toContain('@singula-ai/alego-base')
    ...
    expect(readProfileManifest('t', resolveProfileDir('web', home)).alego?.profile?.patchReload)
      .toBe('live')
  })
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    ...
    delete retiredManifest.alego!.profile!.patchReload
    writeProfileManifest(stock, retiredManifest)
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).alego?.profile).toEqual({
  ...
  it('adds a shipped reload default only to an exact stock tuple and preserves explicit choices', () => {
    ...
    delete stockManifest.alego!.profile!.patchReload
    writeProfileManifest(stock, stockManifest)
    expect(loadProfile('t', 'web', anchor, stockHome).patchReload).toBe('live')
    expect(readProfileManifest('t', stock).alego?.profile?.patchReload).toBe('live')
  ...
  it('fails loud on an unknown patch reload value from disk', () => {
    ...
    initProfile(dir, [])
    const manifest = readProfileManifest('t', dir)
    const rawProfile = manifest.alego!.profile as { patchReload?: string }
...
describe('healProfilesModuleFallback', () => {
  ...
  it('throws when a fallback entry is a foreign file or directory', async () => {
    ...
    for (const kind of ['file', 'directory']) {
      const home = tmp()
      const entry = join(home, 'profiles', 'node_modules', 'alego-app')
  ...
  it('keeps selected bundle closures profile-local without overriding installation packages', async () => {
    ...
    const ownedA = join(profileA.dir, '.alego-module-fallback', 'node_modules', '@scope', 'bundle-only')
    const ownedB = join(profileB.dir, '.alego-module-fallback', 'node_modules', '@scope', 'bundle-only')
  ...
  it('discovers dependencies beside a symlinked bundle real path', async () => {
    ...
    expect(readlinkSync(join(dir, '.alego-module-fallback', 'node_modules', 'bundle-only')))
  ...
  it('traverses every explicit bundle root even when a nested package has the same name', async () => {
    ...
    const ownedModules = join(dir, '.alego-module-fallback', 'node_modules')
  ...
  it('ignores owned projections while recomputing an ordered bundle closure', async () => {
    ...
    const owned = join(dir, '.alego-module-fallback', 'node_modules', 'bundle-only')
  ...
  it('cleans owned projections without removing profile-managed entries', async () => {
    ...
    await healProfilesModuleFallback({ installAnchor: installationAnchor, profile, home })
    const ownedModules = join(profile.dir, '.alego-module-fallback', 'node_modules')
  ...
  it('retains current links while repairing a missing sibling', async () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    await healProfilesModuleFallback({ installAnchor: anchor, home })
    const appTarget = readlinkSync(join(fallback, 'alego-app'))
    unlinkSync(join(fallback, 'bundle-a'))

    await healProfilesModuleFallback({ installAnchor: anchor, home })

    expect(readlinkSync(join(fallback, 'alego-app'))).toBe(appTarget)
  ...
  it('waits for the module-fallback writer lock before publishing entries', async () => {
    ...
    expect(existsSync(join(modules, 'alego-app'))).toBe(false)
    releaseLock?.()
    await Promise.all([holder, healer])
    expect(lstatSync(join(modules, 'alego-app')).isSymbolicLink()).toBe(true)
  })
  ...
  it('writes real ESM proxies for a packaged executable', async () => {
    ...
    try {
      ...
      const proxyManifest = JSON.parse(readFileSync(join(proxy, 'package.json'), 'utf8')) as {
        version: unknown
        exports: unknown
        alego: { moduleFallback: { targets: Record<string, unknown> } }
      }
      expect(proxyManifest).toMatchObject({
        version: '0.0.0',
        exports: { '.': './entry-0.js', './feature': './entry-1.js' },
      })
      expect(proxyManifest.alego.moduleFallback.targets['.']).toEqual(expect.stringContaining('/bundle-a/index.js'))
  ...
  it('preserves the installation path while resolving packaged exports', async () => {
    ...
    try {
      ...
      const proxyManifest = JSON.parse(readFileSync(
        ...
        'utf8',
      )) as { alego: { moduleFallback: { targets: Record<string, string> } } }
      expect(proxyManifest.alego.moduleFallback.targets['.']).toContain('/app/node_modules/linked-esm/index.js')
    } finally {
      delete (process as NodeJS.Process & { pkg?: unknown }).pkg
    }
  })
  ...
  it('skips executable-only and declaration-only packages without import entries', async () => {
    for (const marker of ['bin', 'types', 'typings']) {
      ...
      delete manifest.main
      manifest[marker] = marker === 'bin' ? { alego: './lib/bin.js' } : './index.d.ts'
      ...
      try {
        ...
        await healProfilesModuleFallback({ installAnchor: anchor, home })
        const fallback = join(home, 'profiles', 'node_modules')
        expect(existsSync(join(fallback, 'alego-app'))).toBe(false)
  ...
  it('rejects foreign packaged fallback directories with valid or invalid metadata', async () => {
    ...
    try {
      for (const metadata of ['{}', '{']) {
        ...
        await expect(healProfilesModuleFallback({ installAnchor: anchor, home })).rejects.toThrow(
          'exists and is not an alego-managed module proxy',
        )
      }
    } finally {
      delete (process as NodeJS.Process & { pkg?: unknown }).pkg
