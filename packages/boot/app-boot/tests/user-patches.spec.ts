...
const tmp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'alego-user-patches-'))
  tempRoots.push(dir)
  return dir
}
  ...
  it.each([
    ...
    writeFileSync(patchPath, JSON.stringify([
      ...
      { insert: [
        ...
        { id: 'bare', name: '@singula-ai/alego-system-prompt' },
    ...
    expect(patches[1]?.insert?.map(entry => entry.name)).toEqual([
      pluginUrl, pluginUrl, '@singula-ai/alego-system-prompt', 'cordis:group',
    ])
