 ...
 * resolve first. Alego-owned links add packages carried only by selected
 * bundles, while `$ALEGO_HOME/profiles/node_modules` supplies the installation
...
import { withFileLock } from '@singula-ai/alego-atomic-write'
...
import type { AlegoPackageManifest, ProfilePatchReload } from '@singula-ai/alego-package-manifest'
...
const PROFILE_MODULE_FALLBACK_DIR = '.alego-module-fallback'
...
export type ProfileManifest = Partial<AlegoPackageManifest>
...
export const PROFILE_TEMPLATES: Record<string, ProfileTemplate> = {
  acp: {
    bundles: ['@singula-ai/alego-base', '@singula-ai/alego-acp-app'],
    patchReload: 'startup',
  },
  web: {
  ...
  headless: {
    bundles: ['@singula-ai/alego-base', '@singula-ai/alego-headless'],
    patchReload: 'startup',
  },
  sdk: {
    bundles: ['@singula-ai/alego-base', '@singula-ai/alego-sdk-app'],
    patchReload: 'startup',
  },
  'sdk-minimal': {
    bundles: ['@singula-ai/alego-sdk-minimal'],
    patchReload: 'startup',
  },
}
...
/** The bundle list an `alego plugin` init uses for a name with no shipped template. */
...
export function initProfile(
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      ...
      private: true,
      dependencies: {},
      alego: { profile: { bundles: [...bundles], patchReload } },
    }
...
/** Ensure `link` is a symlink to `target`, replacing a wrong link or an alego-managed packaged proxy. */
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      ...
      if (existing?.alego?.moduleFallback?.targets === undefined) {
        throw new Error(`alego: ${link} exists and is not a symlink or alego-managed module proxy; remove it so alego can manage the installation fallback`)
      }
...
interface ModuleProxyManifest {
  ...
  alego: { moduleFallback: { targets: Record<string, string> } }
}

interface ModuleProxyRecord {
  version?: unknown
  alego?: { moduleFallback?: { targets?: unknown } }
}
...
function packageEntryFromPackage(
  ...
  try {
    ...
    throw new Error(`alego: cannot resolve ESM export ${specifier} from installed package ${packageName}`, { cause: error })
  }
  for (const candidate of candidates ?? []) {
    ...
    if (!target.startsWith('./') || /^\.\.(?:[\\/]|$)/u.test(relativeEntry)) {
      throw new Error(`alego: installed package ${packageName} export ${subpath} resolves outside its package: ${target}`)
    }
...
function packageProxySource(
  ...
  if (typeof manifest.version !== 'string' || manifest.version.length === 0) {
    throw new Error(`alego: installed package ${packageName} must declare a non-empty version`)
  }
  ...
  if (declared === undefined) {
    ...
    try {
      ...
      throw new Error(`alego: installed package ${packageName} main entry is missing at ${entry}`, { cause: error })
    }
  }
...
function ensureModuleProxy(
  ...
  if (stat !== undefined) {
    ...
    if (existing?.alego?.moduleFallback?.targets === undefined) {
      throw new Error(`alego: ${link} exists and is not an alego-managed module proxy; remove it so alego can manage the installation fallback`)
    }
    if (existing.version === version
      && JSON.stringify(existing.alego.moduleFallback.targets) === JSON.stringify(targets)
...
function moduleFallbackEntryCurrent(modulesDir: string, entry: ModuleFallbackEntry): boolean {
  ...
  try {
    ...
    return existing?.version === entry.version
      && JSON.stringify(existing.alego?.moduleFallback?.targets) === JSON.stringify(entry.targets)
...
export interface ProfileModuleFallbackOptions {
  /** Absolute package.json path of the running alego installation. */
  installAnchor: string
  /** Loaded profile whose selected bundles may carry profile-local plugins. */
  profile?: Profile
  /** Harness home; defaults to {@link resolveAlegoHome}. */
  home?: string
}

/**
 * Maintain module fallbacks for one profile launch. The shared
 * `$ALEGO_HOME/profiles/node_modules` mirrors the alego installation dependency
...
export async function healProfilesModuleFallback(options: ProfileModuleFallbackOptions): Promise<void> {
  const { installAnchor, profile, home = resolveAlegoHome() } = options
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const needsReloadDefault = manifest.alego?.profile?.patchReload === undefined && isCurrentTuple
  ...
  const normalized: ProfileManifest = {
    ...
    alego: {
      ...manifest.alego,
      profile: {
        ...manifest.alego?.profile,
        bundles: [...template.bundles],
        patchReload: manifest.alego?.profile?.patchReload ?? template.patchReload,
      },
    },
  }
 ...
 * @param installAnchor - absolute path of the owning alego app's package.json.
...
export function loadProfileDirectory(
  ...
  const bundles = manifest.alego?.profile?.bundles ?? []
  const rawPatchReload: unknown = manifest.alego?.profile?.patchReload
  if (rawPatchReload !== undefined && rawPatchReload !== 'live' && rawPatchReload !== 'startup') {
    throw new Error(
      `${binName}: profile manifest ${join(dir, 'package.json')} alego.profile.patchReload must be "live" or "startup"`,
    )
  }
