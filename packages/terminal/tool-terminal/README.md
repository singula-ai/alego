...
The tools need `ctx.terminals` — a backend must be mounted — and the system-prompt service for the guidance section. Background sends additionally require the jobs service and its model-facing controller (`@singula-ai/alego-tool-jobs`).
...
The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-tool-terminal) and [tool catalog](../../../docs/tool-catalog.md#singula-aialego-tool-terminal) are the exhaustive sources for config fields and schemas.
...
- [Tool catalog](../../../docs/tool-catalog.md#singula-aialego-tool-terminal) — the six generated schemas and result shapes.
...
- **Background mode requires the jobs surface** — `run_in_background` needs both `@singula-ai/alego-jobs` and its model-facing controller (`@singula-ai/alego-tool-jobs`); without them the argument is rejected.
