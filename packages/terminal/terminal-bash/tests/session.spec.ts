...
import { LocalPtySession } from '@singula-ai/alego-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@singula-ai/alego-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@singula-ai/alego-terminal'
import type {
  SubprocessOutcome,
...
describe('LocalPtySession readiness and output', () => {
  it('answers split cursor-position queries before publishing prompt readiness', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07alego> \x1b[')
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07alego> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07alego> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
