...
import type { TerminalBackend, TerminalBackendSpawnSpec, TerminalSendOperation } from '@singula-ai/alego-terminal'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    ALEGO_SHELL: '1',
    ALEGO_SESSION_ID: spec.owner.id,
    ALEGO_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {
...
async function startupSession(
  ...
  const start = async (): Promise<void> => {
    ...
    // require controlled-prompt evidence; PSReadLine can wait for another byte
    // while echoed setup input is still incomplete. Follow-up sends bridge silence
