...
/**
 * A backend-generated default root name: `alego-spill-` plus the 6-character
 * suffix `mkdtemp` appends. Discovery matches this
 * EXACT shape, not the bare prefix, so an unrelated `alego-spill-test-*` fixture
 * or a foreign tool's differently-shaped `alego-spill-…` directory is never
...
export interface SweepRoot {
   ...
   * pruned. Set for DISCOVERED prior-default `alego-spill-*` roots (one per past
...
/**
 * Discover prior default spill roots: the `alego-spill-<6 chars>` directories
 ...
 * prefix, so an unrelated `alego-spill-test-*` fixture or a foreign
