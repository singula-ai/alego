...
import LocalSpillStore, {
  ...
  sessionDir,
  sweepSpillRoots,
} from '@singula-ai/alego-spill-local'
import type { SweepRoot } from '@singula-ai/alego-spill-local'
...
describe('LocalSpillStore service', () => {
  ...
  it('defaults cleanupPeriodDays to 30', async () => {
    ...
    const emptyBase = mkdtempSync(join(tmpdir(), 'alego-empty-'))
...
describe('startup cleanup sweep', () => {
  ...
  it('prunes an emptied DISCOVERED default root but never the active root', async () => {
    ...
    const prior = mkdtempSync(join(tmpdir(), 'alego-spill-'))
  ...
  it('covers the configured root AND discovered default roots (real gatherRoots)', async () => {
    ...
    const fakeTmp = mkdtempSync(join(tmpdir(), 'alego-faketmp-'))
...
describe('discoverDefaultRoots', () => {
  it('returns only real alego-spill-* directories, excluding symlinks and non-matches', async () => {
    const base = mkdtempSync(join(tmpdir(), 'alego-disc-'))
    try {
      // A real backend-shaped root (alego-spill-<6>) via mkdtemp — the only match.
