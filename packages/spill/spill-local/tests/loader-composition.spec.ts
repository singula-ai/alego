...
import LocalSpillStore, { sessionDir } from '@singula-ai/alego-spill-local'
...
describe('spill-local real Loader composition through cordis.yml', () => {
  it('loads cleanupPeriodDays and prunes only expired session contents', async () => {
    root = await mkdtemp(join(tmpdir(), 'alego-spill-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier !== '@singula-ai/alego-spill-local') throw new Error(`unexpected Loader import: ${specifier}`)
