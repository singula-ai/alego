...
为跨包边界且可能被混淆的值添加品牌——`alego-llm` 中的 `ToolCallId`、`alego-session` 中共享的 agent/会话 `SessionId`、`alego-jobs` 中的 `JobId`，以及 `alego-session` 中的 `SessionSeq` 与 `SessionLogOffset`。保持局部或无法混淆的值不需要这种抽象。
