...
A package brands the ids it owns — `CallId` in `alego-llm`, the shared agent/session `SessionId` in `alego-session`, and `JobId` in `alego-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `alego-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
