...
```ts
import { brandNumber, type BrandedNumber } from '@singula-ai/alego-brand'
...
Brand values that cross package boundaries and could plausibly be confused — `ToolCallId` in `alego-llm`, the shared agent/session `SessionId` in `alego-session`, `JobId` in `alego-jobs`, and `SessionSeq` versus `SessionLogOffset` in `alego-session`. Values that stay local or cannot be confused do not need this abstraction.
...
Keeping these helpers in their own package means `alego-jobs` can brand `JobId` without importing an unrelated capability package, while each capability still owns the meaning and validation of its concrete ids.
