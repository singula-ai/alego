---
description: "Shared TypeScript declarations for package identity, runtime requirements, and ALEGO plugin metadata."
...
Use `AlegoPackageManifest` for package metadata, `AlegoManifest` for the public fields under `alego`, and member types such as `AlegoClientManifest` for one domain. Each reader owns JSON parsing, validation, and default resolution.
...
- **Compatibility is declarative.** Current installers and loaders do not enforce `alego.manifestVersion` or `engines.alego`; declaring a range does not reject incompatible hosts or validate SemVer syntax.
