...
Alego 用户数据的共享文件系统路径辅助工具。

## ALEGO 主目录

`resolveAlegoHome()` 解析 Alego 的单根主目录。优先级从高到低为：显式配置的路径、`$ALEGO_HOME`、`~/.alego`。harness 将所有用户数据保存在同一根目录下。

`alegoHomePath(...segments)` 使用 Node 的平台路径规则，将子路径段拼接到解析后的主目录下。不传入任何路径段时，返回主目录本身。

`alegoHomeDisplay()` 以符号方式表示当前根目录，用于面向用户的路径：默认主目录表示为 `~/.alego`，任何已配置的主目录表示为 `$ALEGO_HOME`。它绝不会泄露机器的绝对路径。

`ALEGO_HOME_DIR_NAME` 定义默认用户数据目录名：`.alego`。

`defaultAlegoHome()` 使用 Node 的平台路径规则，将操作系统主目录与 `.alego` 拼接，并返回默认 Alego 主目录。
