---
description: "Alego 主目录与用户数据路径的共享解析，供需要统一根目录、波浪号展开与稳定监听路径的包使用。"
...
面向用户的路径请以符号形式渲染根目录，而不是机器路径：默认主目录显示为 `~/.alego`，任何已配置的主目录显示为 `$ALEGO_HOME`。展示形式绝不会泄露机器的绝对路径。
...
`resolveAlegoHome` 先读显式覆盖值，然后读 `$ALEGO_HOME`，最后回退到操作系统主目录拼接 `.alego`。选中的值经过波浪号展开并规范化为绝对路径；`alegoHomePath` 用 Node 的平台路径规则拼接子路径段。`alegoHomeDisplay` 把解析出的路径与默认根目录比较并返回符号标签，因此已配置的主目录绝不泄露其绝对路径。
...
- [shell 环境](../../shell/shell-env/README.zh.md)——`ALEGO_HOME` 如何到达模型 shell 调用。
