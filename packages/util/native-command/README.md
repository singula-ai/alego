...
```ts
import { runNativeCommand } from '@singula-ai/alego-native-command'
...
- **No output bounding** — both streams buffer unbounded in memory; every current caller invokes small native tools whose output is a path or an error line. Adopt `alego-output-retention` bounding before pointing this at commands with meaningful output volume.
