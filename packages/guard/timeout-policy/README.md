...
The `src/index.ts` FIXME asks to settle a `@singula-ai/alego-timeout-guard` rename; the [naming ledger](../../../.agents/notes/archived/architecture/2026-08-11-repository-naming-contract-and-rename-ledger.md) already records `@singula-ai/alego-tool-call-timeout-policy` as the decided name, so the FIXME is stale pending a code cleanup.
