---
description: "Client Tool presentation plugin for the alego web client: whole-call tree composition, the keyed per-tool view slot, and the built-in atomic tool cards."
...
The terminal model uses `hasSpillNotice` from the browser-safe `@singula-ai/alego-spill-policy/notice` entry, not an independent UI pattern. The [spill-policy README](../../spill/spill-policy/README.md#shared-notice-ownership) owns notice formatting and recognition. This check conservatively selects generic output; matching text cannot authenticate its source, and replay leaves recorded result bytes untouched.
</details>
