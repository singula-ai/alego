...
import { resolveWorkspacePath } from '@singula-ai/alego-util-workspace-path'
import { hasSpillNotice } from '@singula-ai/alego-spill-policy/notice'
...
function shellCall(name: string, args: Record<string, unknown>): ShellCall | null {
  ...
  if (description === undefined) {
    // Standard alego-tool-bash and alego-tool-pwsh schemas require `description`;
...
/**
 * Parse the marker literals owned by `@singula-ai/alego-shell/render` without
