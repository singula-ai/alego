...
import type { MainPanelId } from '@singula-ai/alego-client-ui-layout/client'
import type { PaneId, SplitId, TabId } from '@singula-ai/alego-client-ui-dockkit'
import { dockPaneIds, getPane } from '@singula-ai/alego-client-ui-dockkit'
...
async function mountSeat(viewportWidth = 1440, canShow = true, entryCount = 0) {
  ...
  await act(async () => {
    runtime.ctx.sidebarRightTabs.register({
      id: 'test/text', kind: 'text', priority: 'builtin', patterns: ['alego-resource://file/**'],
  ...
  const open = (name = 'a.txt', options?: Parameters<typeof controller.openResource>[1]) => {
    act(() => { controller.openResource(`alego-resource://file/session/s-test/${name}`, options) })
...
describe('RightbarSeat fullscreen entry', () => {
  ...
  it.each(['close', 'push', 'session', 'unmount'])('ignores a late completion after %s', async (change) => {
    ...
    if (change === 'close') fireEvent.click(element(h.view.container, '[data-sidebar-right-toggle]'))
      ...
      act(() => { h.controller.openResource('alego-resource://file/session/s-other/b.txt') })
...
describe('slot-owned useTabInfo', () => {
  ...
  it('keeps session records and navigation while unmounted, aborting only removal and plugin unload', async () => {
    ...
    act(() => { h.controller.openResource('alego-resource://file/session/s-other/other.txt', { params: { line: 9 } }) })
    ...
    act(() => { info.tab.actions.openResource('alego-resource://file/session/s-test/b.txt') })
