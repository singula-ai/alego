...
import {
  ...
  createUserMessage,
} from '@singula-ai/alego-llm/message'
import { brandString } from '@singula-ai/alego-brand'
import type { MessageId, ToolCallId } from '@singula-ai/alego-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import { LlmAttemptId } from '@singula-ai/alego-llm/brand'
import {
  AssistantStreamAccumulator,
  expandAssistantStream,
  type AssistantStreamRecord,
} from '@singula-ai/alego-llm/assistant-stream'
import type { AttachmentIdType, ImageAttachmentRef } from '@singula-ai/alego-attachment'
import type {
  SessionEvent,
  SessionId,
  SessionSeqCursor,
} from '@singula-ai/alego-session/types'
import { SESSION_FORMAT_VERSION, SessionSeq } from '@singula-ai/alego-session/types'
...
import type { CommandDefinitionId, CommandId } from '@singula-ai/alego-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@singula-ai/alego-commands/types'
import type { CredentialInfo } from '@singula-ai/alego-credentials/types'
import type { DirectoryListing as FixtureDirectoryListing } from '@singula-ai/alego-host-directory-picker/types'
import type { SettingsDescribeValue, SettingsNamespaceView } from '@singula-ai/alego-settings/types'
import { deriveEventMessage, foldSurface } from '@singula-ai/alego-session/surface'
...
const WEB_SEARCH_META = {
  answer: 'Alego is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://github.com/singula-ai/alego',
      title: 'Alego — plugin-based agent harness',
    ...
    {
      url: 'https://docs.alego.dev/plugins',
...
const FIXTURE_SYSTEM_PROMPT = '你是 Alego 的 fixture 助手。用简洁的中文回答，并在需要时调用工具。'
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    if (turn === 0) {
      push({
        ...
        data: { turn, step: 0, message: createSystemMessage(FIXTURE_SYSTEM_PROMPT, '@singula-ai/alego-system-prompt') },
      })
    }
    const userSeq = push({
...
/** Fixture-local mirror of the goal projection value (alego-goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ...
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@singula-ai/alego-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@singula-ai/alego-tool-read'\n" }],
  ])
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      ...
      'deepseek-web', 'alego', 'deepseek-app', 'deepseek-landing-blog',
    ]],
  ])
    ...
    list(id: SessionId): RpcResult<readonly CommandDescriptor[]> {
      ...
      return {
        ...
        value: [
          ...
          { definitionId: brandString<CommandDefinitionId>('@singula-ai/alego-command-goal'), name: 'goal', description: 'Set or view the goal for a long-running task', input: { hint: '<objective>', attachments: true } },
          { definitionId: brandString<CommandDefinitionId>('@singula-ai/alego-permission-presets'), name: 'permission', description: 'Switch the permission preset (sandbox mode + approval policy)', input: { hint: '<preset>' } },
          { definitionId: brandString<CommandDefinitionId>('@singula-ai/alego-plan-mode'), name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
        ],
      }
    },
      ...
      const value = sessions
        ...
        .map((item) => {
          ...
          return {
            ...
            mention: `@[${label}](alego-session:${encoded})`,
          }
        })
  ...
  const sessionApi: FixtureSessionApi = {
    ...
    prompt: (request) => {
      ...
      // user-rpc source member is declared by alego-api-session-controller,
