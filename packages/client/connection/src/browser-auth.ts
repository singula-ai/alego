...
import type { CredentialProvider, CredentialRecord } from '@singula-ai/alego-credentials'
import type {
...
const COOKIE_PREFIX = 'alego-auth-'
...
export class BrowserAuth {
  ...
  authorizeIndex(req: ConnectionIndexRequest, res: ConnectionIndexResponse): boolean {
    ...
    const url = new URL(req.url ?? '/', 'http://alego.invalid')
  ...
  private writeUnauthorized(req: ConnectionIndexRequest, res: ConnectionIndexResponse): void {
    ...
    res.end(req.method === 'HEAD'
      ? undefined
      : 'alego web authentication required; reopen the URL printed by alego web.\n')
  }
}
