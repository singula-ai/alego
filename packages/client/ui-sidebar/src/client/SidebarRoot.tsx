...
import {
  AlegoMark, IconNewChatOutline16, IconPanelLeftOutline16, Tooltip,
} from '@singula-ai/alego-client-ui-primitives'
import type { InjectFace, PropsRenderSlots, PropsRuntime } from '@singula-ai/alego-client-ui-slots'
import type {
...
function localBuildVersion(): string | undefined {
  const version = process.env.ALEGO_CLIENT_VERSION
  if (version === undefined) return undefined
  const commit = process.env.ALEGO_CLIENT_COMMIT_HASH
  return version
    + (commit === undefined ? '' : `-${commit}`)
    + (process.env.ALEGO_CLIENT_GIT_DIRTY === 'true' ? '-dirty' : '')
}
          ...
          >
            <span className={css.brandIdentity} aria-hidden="true">
              <span className={css.brandMark}>
                {renderSlot('sidebar.brand.mark', { size: 24 }, { fallback: <AlegoMark size={24} /> })}
              </span>
