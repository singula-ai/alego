---
description: "alego Web 客户端的主题与正文字号设置：--dsw-* token 样式表、ThemeRuntime 状态、「通用」设置行与插件前引导。"
...
当主机组合包含 HTTP 服务器时，宿主侧会把已注册的 `ui-theme` 设置或 schema 默认值嵌入每份 index 响应。浏览器在加载页面渲染前设置 `color-scheme`、`body[data-ds-dark-theme]` 与 `--alego-content-font-size`，因此首帧绘制就采用所选调色板与字号。
...
- [ui-conversation](../ui-conversation/README.zh.md)——为 composer 席位消费 `--alego-scrollbar-width` 的消费方。
