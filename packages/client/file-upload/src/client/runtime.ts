...
import { bytesToBase64 } from '@singula-ai/alego-util-crypto'
...
export class FileUploadRuntime extends Service implements FileUploadService {
  ...
  constructor(ctx: Context) {
    ...
    const hook = (globalThis as ClientFileUploadGlobal).__ALEGO_FILE_UPLOAD__
...
function workerTransport(): FileUploadTransport {
  return {
    post(request) {
      ...
      const worker = new Worker(workerUrl, { name: 'alego-file-upload' })
...
function resolveUrl(path: string): URL {
  ...
  const origin = typeof pageLocation === 'object' && pageLocation !== null
    ...
    ? pageLocation.origin
    : undefined
  return new URL(path, origin === undefined || origin === 'null' ? 'http://alego.internal' : origin)
}
