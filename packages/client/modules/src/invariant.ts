/**
 * Package-owned invariant companion for `@singula-ai/alego-client-modules`.
 * @module @singula-ai/alego-client-modules/invariant
 */
...
const PACKAGE_NAME = '@singula-ai/alego-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __ALEGO_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
