 ...
 * @module @singula-ai/alego-client-modules
 */
...
import { exactPackageSpecifier, parseAlegoClient, stripClientSuffix } from './client/manifest.ts'
...
function comboSectionMap(record: WebPluginRecord): Record<string, unknown> {
  ...
  const base = new URL(`/plugins/${record.entry.id}/client.js.map`, 'http://alego.invalid')
...
const CLIENT_MODULES_ID = '@singula-ai/alego-client-modules'
...
window.__ModuleLoader__={
  ...
  rows.push({ kind: 'global', name: '__ALEGO_BOOT__', value: graph })
  return rows
}
...
export class ClientModuleRegistry extends Service {
  ...
  private resolveMeta(loaderName: string, baseUrl: string): ResolvedPkgMeta | null {
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${packageName} declares alego.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
