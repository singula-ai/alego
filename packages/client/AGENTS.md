...
Packages here are named with the directory prefix: `@alego/client-<name>`.
...
2. **A dynamic package declares internal dynamic relationships as peer plus dev.** Production source imports, re-exports, module augmentations, and type-only references to an `@alego/*` package count, as does a package named by `alego.client.inject`. A test-only internal dependency stays dev-only.
