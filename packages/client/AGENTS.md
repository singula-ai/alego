...
3. **Browser and type relationships are development-only.** Workspace and third-party Client imports, type-only imports, module augmentations, TypeScript project references, `alego.client.inject`, and metadata-only peers belong only in `devDependencies`. Configuration-only entries absent from imports are listed in the dependency policy; Node companion value imports follow the Host rules below.
...
6. `pnpm run test:gui` green; if the component changes visible assembled output, also run `ALEGO_SNAPSHOT=replay pnpm run test:web`.
