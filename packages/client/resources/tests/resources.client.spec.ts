...
import type { RemoteFailure, RemoteResult } from '@singula-ai/alego-typert-protocol'
...
describe('protocolOf', () => {
  it('reads the alego-resource host, lower-cased, and reports none for any other address', () => {
    expect(protocolOf('alego-resource://file/session/s1/home/ys/b.txt')).toBe('file')
    expect(protocolOf('ALEGO-RESOURCE://File/session/s1/a')).toBe('file')
    expect(protocolOf('alego-resource://chat/node/1')).toBe('chat')
    ...
    expect(protocolOf('alego-resource:///no-host')).toBeUndefined()
...
describe('ResourceRegistry stream generations', () => {
  ...
  it('streams another protocol as plain numbers', async () => {
    ...
    const source = b.registry.source('alego-resource://counter/one')
    const unsubscribe = source.subscribe(() => {})
