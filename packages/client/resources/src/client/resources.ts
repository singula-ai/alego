...
import { createSnapshotStore, type ObservableSnapshot, type SnapshotStore } from '@singula-ai/alego-client-store'
import type {
  ResourceOpenContext,
  ResourceProtocol,
  ResourceProvider,
...
interface ResourceRecord {
  ...
  /** The address's protocol key (`alego-resource://` host); absent when the address is not a resource address. */
...
/**
 * The one URL scheme resource addresses use: `alego-resource://<type>/…`, where
 * the host names the protocol. Other schemes (`sidebar://…`) are navigation
 * addresses and name no resource.
 */
export const RESOURCE_SCHEME = 'alego-resource'

/**
 * The protocol key of one address: the host of an `alego-resource://` URL, as the
