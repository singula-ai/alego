...
This package gives an `official` client build the Alego mark and name in the sidebar. Other build profiles keep the shell's Alego block mark and local-build label, while the conversation hero always uses the animated Alego block mark. Choose it for deployments branded as Alego; deployments with another identity should provide a replacement brand package. It has no runtime state and does not affect model requests.
