...
describe('BootPage', () => {
  ...
  it('keeps loading while entries are active or loading', () => {
    const { el, page } = mount()
    page.setTotal(2)
    const spinner = el.querySelector<HTMLElement>('[data-alego-boot-spinner]')
    expect(spinner?.style.getPropertyValue('--alego-boot-arc')).toBe('72deg')
    page.setState('a', 'active')
    expect(spinner?.style.getPropertyValue('--alego-boot-arc')).toBe('180deg')
    page.setState('b', 'loading')
    expect(el.querySelector('[data-alego-boot-spinner]')).toBe(spinner)
    page.setState('b', 'active')
    expect(spinner?.style.getPropertyValue('--alego-boot-arc')).toBe('288deg')
    expect(el.textContent).toContain('Loading plugins…')
