...
import type { SessionListState, SessionSnapshot } from '@singula-ai/alego-api-session-controller/client'
import type { WorkspaceSnapshot, WorkspaceView } from '@singula-ai/alego-api-workspace-controller/client'
...
} from '@singula-ai/alego-client-test-runtime'
import type { SessionId } from '@singula-ai/alego-session/types'
import type { SessionPendingInteractionSnapshot } from '@singula-ai/alego-client-ui-session/client'
import type { WorkspaceId } from '@singula-ai/alego-workspace/types'
...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    ...
    expect(view.getByText('ALEGO').parentElement?.textContent).toBe('Build AI Agents with ALEGO')
  ...
  it('keeps the rainbow brand in the localized Chinese headline', () => {
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>(() => null)
    const view = render(<HeroShell t={makeTranslate(zh, commonZh)} renderSlot={renderSlot} />)
    expect(view.getByText('ALEGO').parentElement?.textContent).toBe('使用 ALEGO 构建 AI 智能体')
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('ALEGO').parentElement?.textContent).toBe('使用 ALEGO 构建 AI 智能体')
  ...
  it('keeps a rejected first prompt engaging instead of returning to the Hero', () => {
    ...
    expect(b.view.queryByText('ALEGO')).toBeNull()
  })
  ...
  it('publishes the column width as a px variable for the shared width axis', () => {
    ...
    act(() => { fireResize(root) })
    expect(root.style.getPropertyValue('--alego-conversation-column-width')).toBe('1200px')
    // No dragged preference: the user-width override stays absent so the
    // adaptive clamp term applies.
    expect(root.style.getPropertyValue('--alego-chat-user-width')).toBe('')
  })
  ...
  it('drag → persist → window clamp round-trip on a width handle', () => {
    ...
    try {
      ...
      expect(root.style.getPropertyValue('--alego-chat-user-width')).toBe('970px')
      expect(localStorage.getItem('alego.conversation.contentWidth')).toBe('970')
      ...
      act(() => { fireResize(root) })
      expect(root.style.getPropertyValue('--alego-chat-user-width')).toBe('724px')
      expect(localStorage.getItem('alego.conversation.contentWidth')).toBe('970')
