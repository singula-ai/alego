...
import type { GlobalStandardProps, RenderOpts } from '@singula-ai/alego-client-ui-slots'
...
function mountFrame(windowWidth = frameWidth) {
  ...
  const element = () => (
    <AppFrame
      ...
      t={key => key === 'brand.localBuild' ? 'ALEGO Local Build' : key}
    />
  )
...
beforeEach(() => {
  ...
  nextFrame = 1
  vi.stubEnv('ALEGO_CLIENT_TITLE', undefined)
...
describe('AppFrame', () => {
  ...
  it('switches only the keyed main outlet when the active panel changes', () => {
    ...
    for (const panelId of ['panel-a' as MainPanelId, 'panel-b' as MainPanelId, null]) {
      ...
      expect(document.title).toBe(panelId === null ? 'Session title — ALEGO Local Build' : 'ALEGO Local Build')
    }
  })
})
