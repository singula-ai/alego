...
describe('MessageFeedbackActions theme styles', () => {
  ...
  it('slot-injected actions ride the content font-size axis like their host row', () => {
    ...
    expect(block('.action')).toMatch(/width:\s*calc\(28px \+ var\(--alego-content-font-delta, 0px\)\)/)
    expect(block('.action')).toMatch(/height:\s*calc\(28px \+ var\(--alego-content-font-delta, 0px\)\)/)
    expect(block('.action svg')).toMatch(/width:\s*calc\(15px \+ var\(--alego-content-font-delta, 0px\)\)/)
    expect(block('.noteOpen')).toMatch(/line-height:\s*calc\(28px \+ var\(--alego-content-font-delta, 0px\)\)/)
  })
})
