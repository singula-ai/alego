...
import type { RemoteFailure } from '@singula-ai/alego-api-remotes/client'
...
describe('reference preview', () => {
  ...
  it('finishes the first click after a shared warm fetch and reloads after reconnect', async () => {
    ...
    await candidates
    expect(openResource).toHaveBeenCalledExactlyOnceWith('alego-resource://file/session/preview//skills/review/SKILL.md')
  ...
  it('keeps pending clicks bound to their own Session when another Session opens', async () => {
    ...
    await secondDone
    expect(openResource).toHaveBeenLastCalledWith('alego-resource://file/session/second//second/SKILL.md')
    first.resolve({ ok: true, value: { skills: rows } })
    await firstDone
    expect(openResource).toHaveBeenLastCalledWith('alego-resource://file/session/first//skills/review/SKILL.md')
