 ...
 * determinism
 * lives host-side — the pre-step boundary (`alego-tool-skill`) recognizes a
...
import type { SkillEntry } from '@singula-ai/alego-api-remotes/client'
...
import { fileAddressFor } from '@singula-ai/alego-util-workspace-path'
import { rankByName } from '@singula-ai/alego-client-ui-primitives'
...
export function apply(ctx: ClientContext): void {
  ...
  const source: InputTriggerSource = {
    ...
    onPick({ candidate }) {
      ...
      // pre-step boundary (alego-tool-skill) recognizes the leading /name and
