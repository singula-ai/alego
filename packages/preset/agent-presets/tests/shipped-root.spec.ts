 ...
 *
 * `$ALEGO_HOME` is repointed per test for the same reason as the user-root
...
import Include, { entryListSchema } from '@singula-ai/cordis-plugin-include'
import SessionProjectionRegistry from '@singula-ai/alego-session-projection'
import * as yaml from 'js-yaml'
import { afterEach, beforeEach, describe, expect, it } from 'vitest'
import AgentPresets, { SHIPPED_PRESET_ROOT, type Config } from '@singula-ai/alego-agent-presets'
...
beforeEach(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), 'alego-shipped-root-'))
