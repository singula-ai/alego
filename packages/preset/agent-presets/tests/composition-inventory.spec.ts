...
import { Context, FiberState } from '@singula-ai/cordis'
...
import AgentPresets, { COMPOSITION_FILE, METADATA_FILE } from '@singula-ai/alego-agent-presets'
...
describe('fileComposition', () => {
  it('flattens groups and keeps refused expressions conditional', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'alego-composition-'))
    roots.push(dir)
...
describe('AgentPresets.compositionInventory', () => {
  it('reads unmounted presets from their files, marking the default and metadata', async () => {
    const userRoot = await mkdtemp(join(tmpdir(), 'alego-composition-roster-'))
    roots.push(userRoot)
    ...
    expect(await ctx.agentPresets.compositionInventory()).toEqual([
      ...
      },
      {
        ...
        isDefault: false,
        rows: [
          { entryId: 'prompt', moduleName: '@singula-ai/alego-system-prompt', enabled: true },
          // The platform-gate shape: the service evaluates it with the
          // Loader's own scope, so the file answer matches a mount's.
          { entryId: 'gated', moduleName: '@singula-ai/alego-system-prompt', enabled: false, condition: '1 === 1' },
          // An expression the evaluator refuses stays a mount's decision.
          {
            entryId: 'undecidable',
            moduleName: '@singula-ai/alego-system-prompt',
            enabled: 'conditional',
  ...
  it('prefers the standing mount over a file that broke after mounting', async () => {
    const root = await mkdtemp(join(tmpdir(), 'alego-composition-volatile-'))
    roots.push(root)
