 ...
 * authors into `<alegoHome>/.agent-presets`, the way `alego-skill-filesystem` owns
 * `<alegoHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$ALEGO_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@singula-ai/alego-agent-presets'
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'alego-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'alego-preset-explicit-'))
    const ctx = await roster({
      roots: [
