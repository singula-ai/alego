...
import type SettingsService from '@singula-ai/alego-settings'
...
export class AgentPresets extends TypertRemoteService {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = [
      ...
      ...config.roots,
      ...config.includeUserRoot ? [{ path: alegoHomePath(USER_PRESET_DIR), trust: 'user' } satisfies PresetRoot] : [],
    ]
   ...
   * scope key. The binding is alego-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `alego-skill-filesystem` watches its
