 ...
 * @module @singula-ai/alego-sdk-app
 */

import { Command } from 'commander'
import type { Context } from '@singula-ai/cordis'
import z from '@singula-ai/schemastery'
import { exitOnStdinEnd, parseCmdline } from '@singula-ai/alego-cmdline'
...
function sdkCommand(profile: string): Command {
  return new Command()
    .name(`alego --profile ${profile}`)
    .description('Serve Alego SDK clients over stdio JSON-RPC.')
    .helpOption('-h, --help', 'show this help')
    .addHelpText('after', `
Example:
  alego --profile ${profile}     serve one SDK runtime until its client disconnects
`)
}
