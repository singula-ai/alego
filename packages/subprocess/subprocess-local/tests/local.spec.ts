...
import type { SubprocessSpawnSpec, SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@singula-ai/alego-subprocess'
import { childEnv } from '../src/spawn.ts'

function mockWin32ForIsolatedRuntime(): void {
  vi.doMock('@singula-ai/alego-win32-process', () => ({
...
function unmockWin32ForIsolatedRuntime(): void {
  vi.doUnmock('@singula-ai/alego-win32-process')
}
...
describe('LocalSubprocessRuntime', () => {
  ...
  it('rechecks native prerequisites for every eligible spawn and prepares storage before launch', async () => {
    ...
    const prepareManagedProcessBinding = vi.fn(() => ({ spillDir: '/tmp/alego-test-spill' }))
