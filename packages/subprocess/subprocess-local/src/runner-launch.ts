...
export const SUBPROCESS_RUNNER_ENV = 'ALEGO_SUBPROCESS_RUNNER' as const
...
export function spawnRunnerInvocation(): RunnerInvocation {
  ...
  if (extname(fileURLToPath(import.meta.url)) !== '.ts') {
    return [process.execPath, fileURLToPath(import.meta.resolve('@singula-ai/alego-subprocess-local/runner'))]
  }
  return [
    process.execPath,
    '--import',
