...
const bootstrap: InspectorClientBootstrap = {
  ...
  protocol: 'alego-inspector-v0-token',
  maxQueuedRecords: 16,
...
describe('experimental Inspector Client plugin', () => {
  ...
  afterEach(() => {
    ...
    delete globalThis.__ALEGO_INSPECTOR__
    Reflect.deleteProperty(globalThis, '__ALEGO_BOOT__')
  })

  it('provides ctx.inspector and sends observations after the Worker accepts the source', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__ALEGO_INSPECTOR__ = bootstrap
  ...
  it('rotates a copied session identity while its original page remains live', async () => {
    ...
    try {
      ...
      sessionStorage.setItem('alego.experimental-inspector.client-source-id.v0', first.sourceId)
  ...
  it('does not report queue loss again after a replacement absorbs it', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__ALEGO_INSPECTOR__ = { ...bootstrap, maxQueuedRecords: 1 }
    const ctx = new Context()
    const fiber = ctx.plugin({ apply })
    await fiber.await()
    const socket = FakeWebSocket.sockets[0]!
  ...
  it('discovers and serves its built Client bundle through the source protocol', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__ALEGO_INSPECTOR__ = bootstrap
    Reflect.set(globalThis, '__ALEGO_BOOT__', {
      rev: 'graph',
      entries: [{
        id: '@singula-ai/alego-experimental-inspector',
        url: '/plugins/@singula-ai/alego-experimental-inspector/client.js?rev=bundle-rev',
        rev: 'bundle-rev',
      }],
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(script?.url).toContain('/plugins/@singula-ai/alego-experimental-inspector/client.js?rev=bundle-rev')
      expect(script?.sourceMapUrl)
        .toContain('/plugins/@singula-ai/alego-experimental-inspector/client.js.map?rev=bundle-rev')
