...
export class CdpSession implements NetworkSink {
  ...
  constructor(
    ...
    this.unsubscribeSources = sources.subscribeStatus(() => {
      if (this.diagnosticsEnabled) this.sendEvent('ALEGOInspector.sourcesChanged', { sources: this.sources.describe() })
    })
  }
  ...
  receive(value: unknown): void {
    ...
    try {
      ...
      } else if (request.method === 'ALEGOInspector.enable') {
        this.diagnosticsEnabled = true
        result = { sources: this.sources.describe() }
      } else if (request.method === 'ALEGOInspector.disable') {
        this.diagnosticsEnabled = false
        result = {}
      } else if (request.method === 'ALEGOInspector.getSources') {
        result = { sources: this.sources.describe() }
      } else if (request.method === 'ALEGOInspector.getCordisTree') {
