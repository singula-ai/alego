---
description: "Private Agent Teams profile layer over alego-base, for source-checkout users who want Team-scoped coordination tools while retaining one-shot delegation."
kind: "package-bundle"
---
...
```sh
pnpm alego plugin --profile headless add ./packages/experimental/agent-team-profile
pnpm alego --profile headless "Use Agent Teams to split this task between two teammates, wait, and summarize."
```
...
The bundle adds the Team policy and tool schemas described by `alego-tool-team`; it adds no prompt text of its own.
