 ...
 * @module @singula-ai/alego-experimental-webworker-runtime/src/worker-host
 */
import { setActiveModuleLoader, WorkerModuleLoader, type StaticModuleFactory } from './module-system/module-loader.ts'
import type { TypertGateway } from '@singula-ai/alego-api-gateway'
import type { HostConnectionHandle } from '@singula-ai/alego-client-connection'
...
export interface WorkerHostOptions {
  ...
  /** Environment for the process shim; `ALEGO_HOME` defaults to `<root>/home`. */
...
export function createWorkerHost(options: WorkerHostOptions): WorkerHost {
  ...
  const start = async (): Promise<void> => {
    try {
      ...
      installProcessGlobal({ cwd: root, env: { ALEGO_HOME: home, HOME: home, ...options.env } })
      ...
      const appBoot = require('@singula-ai/alego-app-boot') as {
        boot(
          binName: string,
          configPath: string,
          patches: unknown[],
      ...
      const cmdline = require('@singula-ai/alego-cmdline') as {
      ...
      const ctx = await appBoot.boot('alego-webworker', configPath, patches, (hostCtx) => {
...
export function installLogSink(ctx: HostContext, require: (specifier: string) => unknown): void {
  const { Logger } = require('@singula-ai/cordis') as { Logger: LogRenderer }
...
function bootPatches(
  ...
  if (configPath.endsWith('.json')) {
    ...
    const include = loader.load(loader.resolve('@singula-ai/cordis-plugin-include', root)) as { entryListSchema: unknown }
