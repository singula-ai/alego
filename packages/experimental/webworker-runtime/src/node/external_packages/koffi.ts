...
interface KoffiType {
  readonly __alegoKoffiType: string
...
const token = (label: string, size: number, alignment = Math.min(size, 8) || 1): KoffiType =>
  ({ __alegoKoffiType: label, size, alignment })
...
const typeOf = (target: unknown): KoffiType => {
  ...
  if (descriptor?.__alegoKoffiType === undefined) {
...
const describe = (target: unknown): string =>
  typeof target === 'string' ? target : (target as KoffiType | undefined)?.__alegoKoffiType ?? 'anonymous'
...
function array(target: unknown, length: number): KoffiType {
  ...
  return token(`array(${element.__alegoKoffiType}, ${String(length)})`, element.size * length, element.alignment)
}
