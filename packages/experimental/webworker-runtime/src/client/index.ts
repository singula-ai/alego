 ...
 * @module @singula-ai/alego-experimental-webworker-runtime/client
 */
import { IMAGE_FILE_NAME } from '../image-layout.ts'
import type { ClientFileUploadHooks } from '@singula-ai/alego-client-file-upload/types'
...
interface ClientFileUploadGlobal {
  __ALEGO_FILE_UPLOAD__?: ClientFileUploadHooks
}
...
interface BootReadyGlobal {
  __ALEGO_BOOT_READY__?: PromiseWithResolvers<void>
}

function bootReadyGate(): PromiseWithResolvers<void> {
  return (globalThis as BootReadyGlobal).__ALEGO_BOOT_READY__ ??= Promise.withResolvers<void>()
}
 ...
 * bundles, `__ALEGO_BOOT__`, the theme bootstrap — in table order. The
 * boot-readiness deferred (`__ALEGO_BOOT_READY__`) is installed before the
...
export async function connectWorkerHost(worker: Worker, options?: WorkerHostConnectOptions): Promise<WorkerHostConnection> {
  ...
  try {
    ...
    ;(globalThis as ClientTransportGlobal).__ALEGO_TRANSPORT__ = {
    ...
    ;(globalThis as ClientFileUploadGlobal).__ALEGO_FILE_UPLOAD__ = {
      fetch: (input, init) => tunnel.fetch(input, init),
    }
