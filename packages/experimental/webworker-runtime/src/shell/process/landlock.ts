...
import { ALEGO_TMP } from '../../storage/paths.ts'
...
function vfsPath(path: string, cwd: string): string {
  ...
  if (absolute === '/tmp') return ALEGO_TMP
  if (absolute.startsWith('/tmp/')) return `${ALEGO_TMP}${absolute.slice('/tmp'.length)}`
  return absolute
}
...
/** Throw the denial dialect consumed by `alego-bash-sandbox`. */
