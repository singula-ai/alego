 ...
 * @module @singula-ai/alego-experimental-webworker-runtime/src/compile/transform
 */
import { parse } from 'acorn'

const HELPER_SOURCE: Record<string, string> = {
  def: 'const __alego$def=(t,k,get)=>Object.defineProperty(t,k,{enumerable:true,configurable:true,get});',
  default: 'const __alego$default=(m)=>(m&&m.__esModule?m.default:m);',
  ns: 'const __alego$ns=(m)=>(m&&m.__esModule?m:Object.assign({},m,{default:m}));',
  exportAll: 'const __alego$exportAll=(t,m)=>{for(const k of Object.keys(m))if(k!=="default"&&!(k in t))__alego$def(t,k,()=>m[k]);};',
  dynImport: 'const __alego$dynImport=(s)=>Promise.resolve().then(()=>__alego$ns(require(s)));',
}
...
class Transformer {
  ...
  private visit(node: unknown, context: {
    ...
    switch (record.type) {
      ...
      case 'MetaProperty': {
        ...
        if (meta.name === 'import') {
          this.moduleSyntax = true
          this.replace(record.start, record.end, '__alego$meta')
        }
        break
      }
  ...
  run(): string {
    ...
    for (const { exported, local } of this.bindings) {
      prologue.push(`__alego$def(exports,${JSON.stringify(exported)},()=>${local});`)
    }
