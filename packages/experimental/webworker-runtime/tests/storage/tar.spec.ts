...
describe('tar codec', () => {
  ...
  it('mounts as a VFS with directories synthesized along file paths', () => {
    ...
    expect(vfs.existsSync('/alego/config/cordis.yml')).toBe(true)
    expect(vfs.readFileSync('/alego/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
    expect(vfs.existsSync('/alego/config')).toBe(true)
    expect(vfs.existsSync('/alego/workspace')).toBe(true)
    expect(vfs.existsSync('/alego/absent')).toBe(false)
  })
  ...
  it('applies ordered data overlays without exposing runtime paths', () => {
    ...
    expect(vfs.readFileSync('/alego/workspace/status.txt', 'utf8')).toBe('fixture')
    expect(vfs.readFileSync('/alego/home/sessions/example/session.jsonl', 'utf8')).toBe('{}\n')
    expect(() => loadVfsOverlay(packTar({
      'config/cordis.yml': encoder.encode('replaced'),
    }), '/alego', vfs)).toThrow(/overlay entry must stay under home\/ or workspace/)
    expect(vfs.readFileSync('/alego/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
  })
})
