...
import type { ShellRunOutcome } from '@singula-ai/alego-experimental-webworker-runtime/src/shell/types.ts'
...
async function run(command: string, options: { stdin?: string; cwd?: string } = {}): Promise<ShellRunOutcome> {
  return await runShellCommand(command, {
    ...
    env: { HOME: '/alego/home', PWD: WORKSPACE, GREETING: 'hello world' },
    stdin: options.stdin,
  })
}
