...
} from '@singula-ai/alego-experimental-webworker-runtime/src/shell/process/landlock.ts'
import { spawnSubprocess } from '@singula-ai/alego-subprocess-local/src/spawn.ts'

vi.mock('node:child_process', async () =>
  await import('@singula-ai/alego-experimental-webworker-runtime/src/node/builtin_modules/implemented/child_process.ts'))

const WORKSPACE = '/alego/workspace'
const HOME = '/alego/home'
const TMP = '/alego/tmp'
...
it('keeps the native Landlock package API and CLI failure contract', async () => {
  ...
  const missingGrant = spawn(launcherPath(), ['--rw', '/alego/missing', '--', 'touch', `${WORKSPACE}/never`], { cwd: WORKSPACE })
  expect(await collect(missingGrant)).toEqual({
    stdout: '',
    stderr: 'landlock-run: cannot open rule path: /alego/missing: No such file or directory\n',
    code: 125,
  })
