...
function loaderRequire(): WorkerRequire {
  ...
  const loader = new WorkerModuleLoader({ vfs, root: '/alego', staticModules: createNodeBuiltins() })
  return loader.createRequire('/alego/')
}
...
describe('module identity through the loader', () => {
  it('exposes async and synchronous resolution through the Cordis internal seam', async () => {
    const vfs = new MemoryVfs()
    vfs.seedDirectory('/alego/node_modules/example')
    vfs.writeFileSync('/alego/node_modules/example/package.json', JSON.stringify({ main: 'index.js' }))
    vfs.writeFileSync('/alego/node_modules/example/index.js', 'module.exports = {}\n')
    const loader = new WorkerModuleLoader({ vfs, root: '/alego', staticModules: createNodeBuiltins() })

    expect(loader.internal.resolveSync('example', 'file:///alego/app.js')).toEqual({
      format: 'commonjs',
      url: 'file:///alego/node_modules/example/index.js',
    })
    await expect(loader.internal.resolve('node:fs', 'file:///alego/app.js')).resolves.toEqual({
      format: 'builtin',
      url: 'node:fs',
    })
  })
  ...
  it('exposes the package search paths used by the VFS resolver', () => {
    ...
    expect(require.resolve.paths('workspace-package')).toEqual(['/alego/node_modules'])
    expect(require.resolve.paths('./local.js')).toEqual(['/alego'])
  })
})
