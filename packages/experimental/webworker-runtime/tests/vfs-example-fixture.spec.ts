...
import { SESSION_FORMAT_VERSION, Session, SessionId, type SessionEvent } from '@singula-ai/alego-session'
import {
  generationLogFilename,
  scanLog,
} from '@singula-ai/alego-session-persistence-jsonl/src/format.ts'
import { foldSubagentDescriptor } from '@singula-ai/alego-subagent'
import { projectionCacheDomainSpec } from '@singula-ai/alego-session-projection-cache'
import {
...
function readSession(id: string): ReturnType<typeof scanLog> {
  const path = `home/sessions/--alego-workspace--/${id}/${generationLogFilename(SESSION_FORMAT_VERSION, 'none')}`
...
describe('WebWorker preview VFS example', () => {
  it('matches its deterministic source byte for byte', () => {
    ...
    const predecessors = Object.values(VFS_EXAMPLE_SESSION_IDS)
      .map(id => `home/sessions/--alego-workspace--/${id}/session.v2.jsonl`)
