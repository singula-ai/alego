 ...
 * @module @singula-ai/alego-experimental-webworker-packer/src/pack
 */
...
} from '@singula-ai/alego-experimental-webworker-runtime'
import picomatch from 'picomatch'
import yaml from 'js-yaml'
import { entryListSchema } from '@singula-ai/cordis-plugin-include'
import { REPLACED_EXTERNAL_PACKAGES } from '@singula-ai/alego-experimental-webworker-runtime/src/node/external_packages/replaced-externals.ts'
import { MODULE_PROXIES, MODULE_PROXY_PREFIXES } from '@singula-ai/alego-experimental-webworker-runtime/src/module-proxies.ts'
...
export { DEFAULT_ROOT } from '@singula-ai/alego-experimental-webworker-runtime'
...
function sweepImage(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    try {
      ...
      const external = importer.startsWith('node_modules/') && !importer.startsWith('node_modules/@singula-ai/')
...
function materialize(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    for (const field of ['dependencies', 'peerDependencies'] as const) {
      ...
      // (@singula-ai/cordis is a peerDependency of every harness package),
