 ...
 * assumptions about pnpm workspaces or the `alego` CLI.
 * @module @singula-ai/alego-experimental-webworker-packer/src/repository
 */
import { execFileSync } from 'node:child_process'
import { existsSync, mkdtempSync, readFileSync, readdirSync, rmSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join, relative } from 'node:path'
import { ALEGO_HOME_ENV } from '@singula-ai/alego-home-paths'
...
/** Composition entry point package: the `alego` CLI, run from source. */
const CLI_PACKAGE = 'apps/cli'

/** Composition entry point: the `alego` CLI, run from source. */
 ...
 * layers only, so the image is the shipped profile: the machine's `$ALEGO_HOME`
...
export function composeProfile(repoRoot: string, profile: string): string {
  const home = mkdtempSync(join(tmpdir(), 'alego-pack-home-'))
  try {
    return execFileSync(
      process.execPath,
      ['--import', 'tsx/esm', join(repoRoot, CLI_ENTRY), '--profile', profile, '--dump-default-config'],
      { cwd: repoRoot, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, env: { ...process.env, [ALEGO_HOME_ENV]: home } },
    )
  } finally {
...
/** One `alego.configTrees` declaration entry, validated field by field. */
 ...
 * (`alego.configTrees` in its package.json): `path` is relative to the CLI
...
export function configTrees(repoRoot: string): ConfigTree[] {
  ...
  const manifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as {
    alego?: { configTrees?: unknown }
  }
  const declared = manifest.alego?.configTrees
  if (declared === undefined) return []
  if (!Array.isArray(declared)) {
    throw new Error(`vfs image: ${CLI_PACKAGE} alego.configTrees must be an array`)
  }
  const mounts = new Set<string>()
  return declared.map((entry, index) => {
    const tree = entry as Partial<ConfigTreeDeclaration> | null
    const at = `${CLI_PACKAGE} alego.configTrees[${String(index)}]`
