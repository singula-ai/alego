 ...
 *
 * Usage: alego-pack-vfs-image --out <file> [--profile web] [--root /alego]
 *        node --import tsx/esm src/bin.ts --out ../../apps/web/dist/preview/vfs-image.tar.gz
 * @module @singula-ai/alego-experimental-webworker-packer/src/bin
 */
...
function flag(name: string, fallback?: string): string {
  ...
  if (index === -1) {
    ...
    throw new Error(`alego-pack-vfs-image: --${name} is required`)
  }
  const value = process.argv[index + 1]
  if (value === undefined || value.startsWith('--')) {
    throw new Error(`alego-pack-vfs-image: --${name} needs a value`)
  }
  return value
}
