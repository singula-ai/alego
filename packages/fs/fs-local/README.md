...
The **local-filesystem implementation** of the `ctx.fs` provider contract ([`@alego/fs`](../fs)). Backs the twelve `FileSystem` primitives with the host filesystem; loading it as a plugin populates `ctx.fs`.

```ts ignore-check
import { LocalFileSystem } from '@alego/fs-local'

await ctx.plugin(LocalFileSystem, { cwd: process.cwd() })
// ctx.fs uses the local backend; load @alego/fs-observation-policy for the
// freshness policy gate and @alego/tool-fs to expose read/write/edit.
```
