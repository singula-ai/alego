...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@singula-ai/alego-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@singula-ai/alego-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@singula-ai/alego-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@singula-ai/alego-fs-local` | the host-filesystem implementation |
...
Indirectly, through `alego-tool-fs`, which renders provider text and errors as bounded, retained filesystem tool results.
