...
import { SessionId, SessionLogOffset, SessionSeq } from '@singula-ai/alego-session'
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'alego-tool-fs-session-cwd-'))
...
function withPersona(...sections: string[]): string {
  return ['You are an AI agent powered by Alego.', ...sections].join('\n\n')
}
