...
// --------------------------------------------------------------------------
describe('default deployment (with alego-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'alego-tool-fs-'))
    ctx = new Context()
...
// --------------------------------------------------------------------------
describe('bare provider (no alego-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'alego-tool-fs-bare-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: dir })
...
// caller-selected session workspace wins, matching alego-tool-bash.
describe('per-session cwd', () => {
  let sessionDir: string
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'alego-tool-fs-cfg-'))
    sessionDir = await mkdtemp(join(tmpdir(), 'alego-tool-fs-session-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
