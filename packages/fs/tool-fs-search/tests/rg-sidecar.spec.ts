...
describe('ripgrep resolution', () => {
  it('uses the native sidecar beside the current executable', async () => {
    ...
    process.execPath = '/runtime/alego'
    existsSync.mockReturnValue(true)
    const sidecar = '/runtime/alego-rg'
    const { resolveRgPath } = await import('@singula-ai/alego-tool-fs-search')
  ...
  it('uses a conventional executable name for the Windows ripgrep sidecar', async () => {
    ...
    process.execPath = 'C:\\runtime\\alego-sdk-runtime-win-x64.exe'
    existsSync.mockReturnValue(true)
    const sidecar = 'C:\\runtime\\alego-sdk-runtime-win-x64-rg.exe'
    const { resolveRgPath } = await import('@singula-ai/alego-tool-fs-search')
