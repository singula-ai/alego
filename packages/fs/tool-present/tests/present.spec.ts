...
import ToolRuntime, { defineTool } from '@singula-ai/alego-tools'
...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), 'alego-present-minimal-'))
...
it('declares readable files outside the Session directory using absolute and relative paths', async () => {
  ...
  const outside = await mkdtemp(join(tmpdir(), 'alego-present-external-'))
