...
`alego-invariants` runs package-owned runtime checks — invariants — inside an Alego composition: any package can ship a `./invariant` companion that verifies its own durable relationships (authoritative event streams and mutable snapshots) while the composition runs. Checks run automatically, and a failed check reports an `InvariantError` attributed to the package that owns the violated relationship. Choose it for compositions that want self-checking diagnostics with a global switch and package-name filters; the standard agent composition already mounts it with the four core companions, and loading the service alone installs no checks.
...
Use the registry for compositions that want live diagnostics. [`alego-sdk-minimal`](../../bundle/sdk-minimal/README.md) mounts it with the four core stateful companions — `alego-session`, `alego-agent`, `alego-scope`, and `alego-agent-loop`; `alego-base` deliberately omits runtime diagnostics. Custom compositions mount the registry and add companions for any other loaded package whose contracts they want checked. Loading the registry alone installs no checks: it ships no product checks of its own, so a composition that never mounts a companion observes no diagnostic behavior.
...
The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-invariants) is the exhaustive source for every accepted field and its JSDoc.
...
| Companion | Checks |
|---|---|
| `alego-session`, `alego-agent`, `alego-scope`, `alego-agent-loop` | Session log enclosure and call/result trace, agent-status transitions, scope-filtered dispatch subjects, loop-built request reconstruction |
| `alego-llm`, `alego-llm-retry`, `alego-tools`, `alego-system-prompt` | LLM stream grammar, retry-failure shape, tool-pipeline stage pairing and frozen results, prompt-assembly section names |
| `alego-compaction`, `alego-hook-protocol`, `alego-sandbox-policy` | Compaction stream pairing, hook invocation/result pairing, sandbox mode values |
| `alego-fs`, `alego-subagent`, `alego-workflow`, `alego-tool-workflow` | Filesystem event identity, subagent provider and start/end pairing, workflow lifecycle identity, workflow record shape |
| `alego-goal`, `alego-goal-round-driver` | Durable goal-stream folds and reconstructed continuation prompts |
...
- [Generated configuration catalog](../../../docs/config-catalog.md#singula-aialego-invariants) — every accepted config field and its source declaration.
...
- **Request reconstruction covers loop-built requests only** — the `alego-agent-loop` companion reconstructs requests explicitly built by the loop; direct one-shot LLM calls remain outside that contract even when callers freeze them or attach a session id.
