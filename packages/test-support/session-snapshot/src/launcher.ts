 ...
 *
 * @module @singula-ai/alego-session-snapshot/launcher
 */
...
import { entryListSchema, type PatchOptions } from '@singula-ai/cordis-plugin-include'
...
export interface AgentUnderTest {
  ...
  configPath: string
  /** Named alego profile; omitted only for test-only fake bins with their own config grammar. */
  profile?: string
...
export function launchAcpTestAgent(options: AcpTestLaunchOptions): LaunchedAcpTestAgent {
  ...
  const launch = resolveExampleLaunch({
    ...
    configArgs: agent.profile === undefined
      ...
      : profileArgs(agent.profile, agent.configPath, selectedConfig, options.env?.ALEGO_SNAPSHOT, cwd),
    tsconfigPath: agent.tsconfigPath,
    ...agent.profile === undefined ? {} : { sourceImport: 'tsx/esm' },
    env: {
      ...options.env,
      ALEGO_HOME: join(cwd, '.alego'),
  ...
  const clientApp = createAcpClientApp({ name: 'alego-acp-test-client' })
...
/** Build one alego profile invocation from the base and optional scenario patches. */
function profileArgs(
  ...
  const materializedRoot = join(cwd, '.alego-profile-patches')
 ...
 * profile fallback. This mirrors `alego plugin` while retaining the bare entry
...
function linkProfilePackage(source: string, cwd: string, packageName: string): void {
  ...
  // The package may instead belong to the alego installation; profile boot heals those links.
  if (packageDir === undefined) return
  const link = join(cwd, '.alego', 'profiles', 'node_modules', packageName)
