 ...
 *
 * @module @singula-ai/alego-session-snapshot/harness
 */
...
import { clearedProxyEnv } from '@singula-ai/alego-http-proxy'
import {
...
export interface RunOptions {
   ...
   * them to `alego-llm-replay` via `$ALEGO_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/alego-acp-snap-${key}`
}
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const initialWorkspace = await captureWorkspaceSnapshot(cwd, {
      ignoredRootEntries: ['.agents', '.alego', '.alego-profile-patches', '.alego-snapshot-stream-ready'],
    })
    const env: NodeJS.ProcessEnv = {
      ...
      ...clearedProxyEnv(),
      ALEGO_SNAPSHOT: opts.mode,
      ALEGO_SNAPSHOT_FILE: opts.fixtureFile,
      ALEGO_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      ALEGO_SNAPSHOT_SPILL_ROOT: spillRoot,
      ALEGO_SNAPSHOT_SPILL_LOCATOR_ROOT: snapshotSpillRoot(opts.fixtureFile),
      ALEGO_HOME: join(cwd, '.alego'),
      ALEGO_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { ALEGO_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { ALEGO_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in alego-llm-replay's loadSessionScripts
