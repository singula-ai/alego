...
describe('runScenario', () => {
  ...
  it('builds alego profile argv and rebases relative modules in live and replay patches', async () => {
    ...
    const live = launchAcpTestAgent({
      ...
      env: { ALEGO_SNAPSHOT: 'record', ALEGO_SNAPSHOT_FILE: fixtureFile },
    })
    await live.spawned
    await live.close()
    const materializedRoot = join(dir, '.alego-profile-patches')
    ...
    expect(await realpath(join(dir, '.alego', 'profiles', 'node_modules', 'example-package')))
      .toBe(await realpath(packageDir))
    expect(await realpath(join(dir, '.alego', 'profiles', 'node_modules', '@fixture', 'example-package')))
    ...
    const replay = launchAcpTestAgent({
      ...
      cwd: dir,
      configPath: selectedPatch,
      env: { ALEGO_SNAPSHOT: 'replay', ALEGO_SNAPSHOT_FILE: fixtureFile },
    })
    ...
    const conflictLink = join(dir, '.alego', 'profiles', 'node_modules', 'conflict-package')
    await Promise.all([
