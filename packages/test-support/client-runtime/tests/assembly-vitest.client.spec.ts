...
const test = createClientTest({ roster: webApp.closure(['@singula-ai/alego-api-gateway']) })
...
afterAll(async () => {
  ...
  expect('__ALEGO_TRANSPORT__' in globalThis).toBe(false)
})
...
const missingConnection = createClientTest({ roster: webApp.pick(['@singula-ai/alego-typert-registry']) })
