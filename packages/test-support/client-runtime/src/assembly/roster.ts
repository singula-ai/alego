 ...
 * @module @singula-ai/alego-client-test-runtime/src/assembly/roster
 */
import type { Context } from '@singula-ai/cordis'
import type { WebBootEntry, WebBootGraph } from '@singula-ai/alego-client-modules/client'
import { PLATFORM_MODULES } from '@singula-ai/alego-client-web/src/platform.ts'

/** One browser plugin row as `alego.client` declares it, keyed by package name. */
export interface ClientRosterRow {
  /** Package name (== manifest entry id == Loader entry name). */
  readonly name: string
  /** Package-name dependency edges from `alego.client.inject` ([] when absent). */
  readonly inject: readonly string[]
  /** Stage-one prefetch mark from `alego.client.immediately` (false when absent). */
