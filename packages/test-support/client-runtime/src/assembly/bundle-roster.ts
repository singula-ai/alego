/**
 * The browser roster of an `alego --profile`, read from its bundle patch files
 * the way the launcher composes them: each bundle's `alego.bundle.patch` list is
 ...
 * declares `alego.client.platform === 'web'` becomes a roster row carrying that
 ...
 * repository.
 * @module @singula-ai/alego-client-test-runtime/src/assembly/bundle-roster
 */
...
import { applyEntryPatches, entryListSchema, type PatchOptions } from '@singula-ai/cordis-plugin-include'
import { exactPackageSpecifier, parseAlegoClient } from '@singula-ai/alego-client-modules/client'
import * as yaml from 'js-yaml'
import { ClientRoster, type ClientRosterRow } from './roster.ts'

/** The `web` profile's bundle layers, in the order `alego --profile web` applies them (app-boot `PROFILE_TEMPLATES.web`). */
export const WEB_PROFILE_BUNDLES: readonly string[] = ['@singula-ai/alego-base', '@singula-ai/alego-web-app']

interface PackageManifest {
  name?: unknown
  alego?: { bundle?: { patch?: unknown }; client?: unknown }
}
...
export function bundleRoster(bundles: readonly string[], anchor: string = fileURLToPath(import.meta.url)): ClientRoster {
  ...
  for (const { entry, disabled } of flattenGroups(entries)) {
    ...
    const declaration = parseAlegoClient(name, manifest.alego?.client)
...
function readLayer(bundle: string, anchor: string): BundleLayer {
  ...
  const patch = readManifest(manifestPath).alego?.bundle?.patch
  if (typeof patch !== 'string') throw new Error(`client-test-runtime: bundle ${bundle} declares no alego.bundle.patch in ${manifestPath}`)
