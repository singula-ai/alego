...
import { FsError, FsTargetKey, FsVersion } from '@singula-ai/alego-fs'
...
describe('SessionMediaReferences /api/file', () => {
  ...
  beforeEach(async () => {
    root = await realpath(await mkdtemp(join(tmpdir(), 'alego-media-references-')))
  })
  ...
  it('reads files and symlink targets outside the default cwd without a workspace registry', async () => {
    const route = await mount()
    const outside = await mkdtemp(join(tmpdir(), 'alego-media-outside-'))
    try {
