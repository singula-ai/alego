...
import type { SettingsDescriptor } from '@singula-ai/alego-settings'
import { RemoteError, remoteErrorOf, remoteMethods } from '@singula-ai/alego-typert-protocol'
  ...
  it('reports the actionable configuration error while no settings provider is mounted', async () => {
    ...
    for (const call of calls) {
      ...
      expect(remoteErrorOf(failure)).toMatchObject({
        ...
        message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @singula-ai/alego-settings-file) in its composition',
        details: {},
      })
    }
  })
