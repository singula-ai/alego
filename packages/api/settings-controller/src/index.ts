 ...
 *
 * @module @singula-ai/alego-api-settings-controller
 */
...
import {
  ...
  openNativePath,
  openNativeTextFile,
} from '@singula-ai/alego-native-command'
import type { SettingsDescriptor, SettingsPathOp, SettingsProvider } from '@singula-ai/alego-settings'
import type {
...
export class SettingsController extends TypertRemoteService {
  ...
  private provider(): SettingsProvider {
    ...
    if (settings === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'settings service is absent: this deployment does not mount a settings provider (e.g. @singula-ai/alego-settings-file) in its composition',
        {},
      )
    }
    return settings
