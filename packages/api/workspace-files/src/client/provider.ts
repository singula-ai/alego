 ...
 * `alego-resource://file/session/<sessionId>/<path>`, carries an absolute path or one
 ...
 * An `absolute` address, `alego-resource://file/absolute/<path>`, carries no
 ...
 */
import type { ResourceProvider } from '@singula-ai/alego-client-resources/client'
...
import { parseFileAddress } from '@singula-ai/alego-util-workspace-path'
...
function unsupportedAddress(address: string): RemoteError<'workspace-file/unsupported-address'> {
  return new RemoteError(
    ...
    `${address} is not an alego-resource://file/session/<sessionId>/<path> or alego-resource://file/absolute/<path> address`,
    { address },
  )
}
...
function unknownWorkspace(address: string): RemoteError<'workspace-file/unknown-workspace'> {
  return new RemoteError(
    ...
    `${address} requires an alego-resource://file/session/<sessionId>/<path> address`,
    { address },
  )
}
