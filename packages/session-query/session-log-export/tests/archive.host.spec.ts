...
import type { FileAttachmentRef, ImageAttachmentRef } from '@singula-ai/alego-attachment'
...
import { SessionPersistenceNotFoundError } from '@singula-ai/alego-session-persistence'
import type { SessionAccess, SessionHandle } from '@singula-ai/alego-session-persistence'
import { HostConnectionService } from '@singula-ai/alego-client-connection'
import type { BrowserAuth } from '@singula-ai/alego-client-connection/src/browser-auth.ts'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root log serialized as canonical JSONL', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('alego-session-session-root.zip')
