 ...
 * trusted, per-execution `ALEGO_*` variables consumed by the model-facing shell
 * tools (`alego-tool-bash`, `alego-tool-pwsh`). Built-in shell facts are owned by
...
/** Model-visible metadata for one managed `ALEGO_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `ALEGO_*` keys this contributor may return. */
  variables: Readonly<Record<AlegoEnvironmentKey, BashEnvVariable>>
  /**
   ...
   */
  resolve(execution: ToolExecution): Readonly<Partial<Record<AlegoEnvironmentKey, string>>>
}
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `ALEGO_*` environment variable name. */
  key: AlegoEnvironmentKey
}
...
/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `ALEGO_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `ALEGO_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<AlegoEnvironmentKey, string>()
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [AlegoEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(ALEGO_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(ALEGO_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): AlegoEnvironment {
    const values: Record<AlegoEnvironmentKey, string> = {
      [ALEGO_HOME_ENV]: this.alegoHome,
      [ALEGO_SHELL_KEY]: '1',
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as AlegoEnvironmentKey
  ...
  list(): BashEnvVariableInfo[] {
    return [...this.contributors.values()]
      .flatMap(contributor => Object.entries(contributor.variables).map(([key, variable]) => ({
        ...
        key: key as AlegoEnvironmentKey,
      })))
