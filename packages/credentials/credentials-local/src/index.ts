/**
 * File-backed credentials provider over `$ALEGO_HOME/.credentials.yaml`, layered
 ...
 * > $ALEGO_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… alego`, a CI
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveAlegoHome(config.alegoHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `alego-atomic-write`, this is a
...
/** File-backed credentials provider (`$ALEGO_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start alego from instead',
      )
    }
  }
