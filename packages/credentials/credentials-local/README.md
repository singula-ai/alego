...
The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-credentials-local) is the exhaustive source for every accepted field and its JSDoc.
...
- **A key the launching environment supplies is read-only** — `DEEPSEEK_API_KEY=… alego` wins for this run; saving or removing it is refused. Clear the variable in the launching shell first.
...
| — | No runtime invariant companion is published; the Service Definition companion (`alego-credentials/invariant`) owns the `credentials/reference-updated` lifecycle contract; this provider's file/environment layering is asynchronous I/O pinned by its unit suite. |
