...
生成的[配置目录](../../../docs/config-catalog.zh.md#singula-aialego-credentials-local)是每个受支持字段及其 JSDoc 的穷尽式真源。
