...
The local store README owns the full configuration surface; the generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-credentials-local) is the exhaustive field list.
...
- **A key the launching environment supplies cannot be overwritten** — `DEEPSEEK_API_KEY=… alego` (or a CI secret, a container `-e`) wins for this run and is reported read-only; clear the variable in the launching shell before storing a different value.
