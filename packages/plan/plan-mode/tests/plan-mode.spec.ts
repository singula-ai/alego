...
import ToolRuntime, { RUN_CODE_NAME, defineContentToolFixture } from '@singula-ai/alego-tools'
...
import { CodeRuntime, type CodeRunRequest, type CodeRunResult } from '@singula-ai/alego-code-runtime'
...
/**
 * Drives the REAL plugin: mounts `alego-plan-mode` beside real `SystemPrompt` and
...
describe('/plan', () => {
  it('registers only when a commands service is composed and optionally submits the next-step message', async () => {
    ...
    expect(ctx.commands.list(plainAgent)).toEqual([
      { definitionId: '@singula-ai/alego-plan-mode', name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
    ])
