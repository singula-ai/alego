...
import { ToolCallId, type StreamChunk } from '@singula-ai/alego-llm'
...
import { SessionPersistenceRevision, type SessionPersistenceSnapshot } from '@singula-ai/alego-session-persistence'
...
describe('automation-only ACP bridge', () => {
  ...
  it('advertises the standard automation controls without private metadata', async () => {
    ...
    expect(response).toEqual({
      protocolVersion: PROTOCOL_VERSION,
      agentInfo: { name: 'alego-acp', version: '0.0.1' },
      agentCapabilities: {
  ...
  it('lists and resumes persisted sessions after an equivalent process restart', async () => {
    const persistenceRoot = await mkdtemp(join(tmpdir(), 'alego-acp-restart-'))
    try {
