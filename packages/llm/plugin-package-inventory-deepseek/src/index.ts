 ...
 * @module @singula-ai/alego-plugin-package-inventory-deepseek
 */
...
import type { Entry, EntryTree } from '@singula-ai/cordis-plugin-loader'
...
async function collectActivePluginPackages(
  ...
  if (sessionId !== undefined && ctx.get('agentPresets') !== undefined) {
    ...
    if (agent !== undefined) {
      ...
      const { standingMountFor } = await import('@singula-ai/alego-agent-presets')
