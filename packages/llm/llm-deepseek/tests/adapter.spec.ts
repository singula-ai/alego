...
import type { PreparedDeepSeekLlmApiExtensions } from '@singula-ai/alego-deepseek-llm-api-extensions'
...
describe('DeepSeekAdapter against a mock server', () => {
  it('merges prepared extension fields and accepts them once after HTTP 2xx', async () => {
    ...
    expect(server.requests[0]).toMatchObject({ alego_test: { version: 1 } })
  ...
  it('does not accept extensions on non-2xx and does accept before a later stream failure', async () => {
    ...
    const adapter = new DeepSeekAdapter({
      ...
      resolveApiKey: () => Promise.resolve('k'),
      resolveUserId: () => TEST_USER_ID,
      prepareExtensions: () => Promise.resolve({ fields: { alego_test: 1 }, accept: async () => { accept() } }) as never,
    })
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-dsh-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-dsh-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-dsh-compact')
  })
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    expect(server.headers[1]?.['x-dsh-compact']).toBe('1')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-dsh-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-dsh-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-dsh-compact']).toBe('1')
  })
