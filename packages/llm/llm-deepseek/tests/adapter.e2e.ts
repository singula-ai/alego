...
import LlmRuntime, { createUserMessage, ToolCallId, ReasoningEffortId, createMessage, createSystemMessage } from '@singula-ai/alego-llm'
...
import LocalAttachments from '@singula-ai/alego-attachment-local'
import type {
...
import * as PluginPackageInventoryDeepSeek from '@singula-ai/alego-plugin-package-inventory-deepseek'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'alego-e2e-user-id-'))
  vi.stubEnv('ALEGO_HOME', identityHome)
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-deepseek e2e (real API)', () => {
  ...
  it('serves a real request with the key held only by a credentials-local document', async () => {
    const key = process.env.DEEPSEEK_API_KEY
    if (key === undefined) throw new Error('e2e ran without DEEPSEEK_API_KEY')
    const dir = await mkdtemp(join(tmpdir(), 'alego-e2e-credentials-'))
    try {
