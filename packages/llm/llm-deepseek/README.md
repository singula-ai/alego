...
Choose this adapter when the deployment targets DeepSeek's official API, optionally behind an OpenAI-compatible gateway named by `baseURL`. Choose `alego-llm-pi-ai` when the same composition also routes other providers or hand-declared gateways through pi-ai's catalogs; the two adapters can be mounted together because their route names do not collide. Registering any other adapter for `deepseek-official` fails with `DUPLICATE_ADAPTER`.
...
The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-llm-deepseek) is the exhaustive source for every accepted field and its JSDoc.
