/** Durable DeepSeek attachment-to-file-id index. @module alego-llm-deepseek/upload-index */
...
import { ImageVariantId } from '@singula-ai/alego-attachment'
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@singula-ai/alego-attachment'
...
/** Atomic local index shared by every DeepSeek session in this ALEGO home. */
export class DeepSeekUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `ALEGO_HOME/llm-deepseek/files-v3.json`.
   */
  constructor(path = join(resolveAlegoHome(), 'llm-deepseek', 'files-v3.json')) {
    this.path = path
  }
