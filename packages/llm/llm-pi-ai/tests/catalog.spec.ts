...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@singula-ai/alego-llm'
...
import { PiAiAdapter } from '@singula-ai/alego-llm-pi-ai'
...
/** A throwaway $ALEGO_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'alego-pi-catalog-'))
  homes.push(dir)
