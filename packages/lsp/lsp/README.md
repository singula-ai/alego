...
Server commands, extension mappings, and the filesystem/subprocess pairing are configured in the provider and tool packages; see [alego-lsp-stdio](../lsp-stdio/README.md) and [alego-tool-lsp](../tool-lsp/README.md).
...
- **Capability seam, Service Definition role.** The package owns `ctx.lsp` and the provider registry; providers register capabilities, not tools, and `alego-tool-lsp` is the only owner of the model-facing surface.
...
- [alego-lsp-stdio](../lsp-stdio/README.md) — the stdio provider that registers against this seam.
- [alego-tool-lsp](../tool-lsp/README.md) — the model-facing tool over this seam.
...
Indirectly, through `alego-tool-lsp`, which owns the model-facing `lsp` schema, prompt guidance, and rendered results while this registry contributes no prompt or schema itself.
