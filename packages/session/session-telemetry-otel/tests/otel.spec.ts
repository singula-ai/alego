...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionLogOffset, SessionSeq } from '@singula-ai/alego-session'
import MessageFeedbackService from '@singula-ai/alego-message-feedback'
import JsonlPersistence from '@singula-ai/alego-session-persistence-jsonl'
...
// pin ALEGO_HOME to a temp dir so the suite never touches the ambient ~/.alego.
let tempHome: string
let previousAlegoHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'alego-otel-home-'))
  previousAlegoHome = process.env.ALEGO_HOME
  process.env.ALEGO_HOME = tempHome
})
afterAll(() => {
  if (previousAlegoHome === undefined) delete process.env.ALEGO_HOME
  else process.env.ALEGO_HOME = previousAlegoHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships only the submitted prefix through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'alego' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@singula-ai/alego-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@singula-ai/alego-session-telemetry-otel/ops')
...
describe('OpenTelemetrySessionBackend route and feedback', () => {
  ...
  it.each(['deepseek-official', 'mock', undefined])('uploads live ratings, notes and withdrawal for %s without further interaction', async (provider) => {
    const { url, captures } = await mockCollector()
    const root = mkdtempSync(join(tmpdir(), 'alego-otel-live-'))
    const ctx = new Context()
    try {
      await ctx.plugin(SessionStore)
  ...
  it.each([SessionTelemetryMode.FEEDBACK_ONLY])('restores a cold fork with its exact inherited cut in %s', async (mode) => {
    const { url, captures } = await mockCollector()
    const root = mkdtempSync(join(tmpdir(), 'alego-otel-cold-fork-'))
    const ctx = new Context()
    try {
      await ctx.plugin(SessionStore)
      await ctx.plugin(JsonlPersistence, { root, compression: 'none' })
  ...
  it.each([
    ...
    const { url, captures } = await mockCollector()
    const root = mkdtempSync(join(tmpdir(), 'alego-otel-cold-'))
    const ctx = new Context()
    const warn = vi.spyOn(ctx.logger, 'warn').mockImplementation(() => {})
    try {
      await ctx.plugin(SessionStore)
...
describe('alego-session-telemetry-otel real-load-path guard', () => {
