...
The backend is a thin adapter over the OTel JS SDK: it owns feedback authorization, resource identity, and an outer shutdown deadline. Canonical ledger records use the `@singula-ai/alego-session-telemetry-otel` instrumentation scope; this backend captures no operational records. Resource identity carries `service.name`/`service.version` from `alego-llm`'s `APP_IDENTITY` plus the anonymous `user.id` (from `$ALEGO_HOME/.anonymous-user-id`), once per export batch rather than per record.
