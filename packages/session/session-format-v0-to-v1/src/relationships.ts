...
import { SessionFormatError } from '@singula-ai/alego-session-format'
import type { SessionFormatArtifact, SessionFormatEvent, SessionFormatJsonValue } from '@singula-ai/alego-session-format'
...
function assertTitleSources(
  ...
  if (event.type === 'session/title-llm-request') {
    ...
    if (messages.length !== 1 || message?.['role'] !== 'user' || content?.length !== 1
      || source?.['kind'] !== 'plugin' || source['plugin'] !== 'alego-session-title-llm') {
