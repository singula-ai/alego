...
The cache opens its domain through the storage stack, so base mounts `storage`, `storage-json` (root `alegoHomePath('storages')`), and `storage-domain` (`backend: json`) before it:
...
The plugin injects `storageDomain`, `sessionProjections`, and `sessions`. The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-session-projection-cache) is the exhaustive source for every accepted field and its JSDoc.
