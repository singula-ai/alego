...
import { SESSION_FORMAT_VERSION, SessionId, SessionSeq } from '@singula-ai/alego-session'
...
vi.mock('@singula-ai/node-addon-system/flock', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@singula-ai/node-addon-system/flock')>()
  return {
...
async function freshRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'alego-jsonl-lease-'))
  dirs.push(root)
  return root
}
