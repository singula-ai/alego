import { MessageId, createMessage } from '@singula-ai/alego-llm'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { Context } from '@singula-ai/cordis'
import { appendFile, mkdtemp, mkdir, rm, readFile, writeFile, readdir, stat, symlink } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join, relative, resolve } from 'node:path'
import { SESSION_FORMAT_VERSION, SessionLogOffset, SessionSeq, SessionId } from '@singula-ai/alego-session'
...
import { releasedV1SessionFormatCodec } from '@singula-ai/alego-session-format-v0-to-v1'
...
runLiveWritePathContract('jsonl', LIVE_WRITE_BATCH_MAX_DELAY_MS, async () => {
  const dir = await mkdtemp(join(tmpdir(), 'alego-jsonl-live-'))
  dirs.push(dir)
