...
const nativePrefix: readonly SessionFormatJsonObject[] = [
  ...
  { type: 'system/message', surfaceOp: 'append', data: {
    turn: 1, step: 1, message: {
      id: 'native-system', role: 'system', source: { kind: 'plugin', plugin: '@singula-ai/alego-system-prompt' },
...
const migrationRefusals = [
  ...
  }))),
  {
    ...
    diagnostic: '@singula-ai/alego-session-format-v2-to-v3 refuses this format v2 Session: format v2 delivery marker claims target format v3',
  },
  {
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'alego-migration-refusal-'))
})
