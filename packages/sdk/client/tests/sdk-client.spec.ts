...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): Alego {
  const harness = new Alego({ launch: fakeLaunch(env, extra) })
...
describe('Alego', () => {
  ...
  it('supports await using disposal', async () => {
    let captured: Alego
    {
      await using harness = new Alego({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('fails fast when the command does not exist', async () => {
    const client = new HarnessClient({ command: join(tmpdir(), 'alego-no-such-runtime-bin') })
    cleanups.push(() => client.close())
