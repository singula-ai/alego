/**
 * High-level run API over {@link HarnessClient}: `Alego` owns one
...
import type { ContentBlock, AlegoOptions, HarnessNotification, RunResult, SdkPromptContentBlock } from './types.ts'

/**
 * Reusable SDK for running Alego agent turns in a runtime
 ...
 * child is reaped.
 */
export class Alego implements AsyncDisposable {
  private clientInstance: HarnessClient
  private readonly createClient: () => HarnessClient
  private readonly cwd: string
  private readonly provider: string
  private readonly model: string
  private readonly reasoningEffort: AlegoOptions['reasoningEffort']
  private readonly maxTokens: number | undefined
  private initialized: Promise<void> | undefined
  private closed = false

  /** @param options - alego launch configuration plus the session route, effort, and output cap. */
  constructor(options?: AlegoOptions)
  constructor(options: AlegoOptions = {}, clientFactory?: () => HarnessClient) {
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, cleanupError],
            'Alego initialization and cleanup failed',
          )
        }
...
export function createProcessAlego(
  runtime: RuntimeProcessOptions,
  options: AlegoOptions = {},
): Alego {
  const Constructor = Alego as unknown as new (
    publicOptions: AlegoOptions,
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: Alego, readonly id: string) {}
