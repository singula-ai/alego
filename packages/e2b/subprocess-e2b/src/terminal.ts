...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'alego_state=$1',
  'mapfile -d \'\' -t alego_env < "$alego_state/environment"',
  'mapfile -d \'\' -t alego_argv < "$alego_state/argv"',
  'alego_output_marker=$(<"$alego_state/output-marker")',
  'rm -f -- "$alego_state/environment" "$alego_state/argv" "$alego_state/output-marker" "$alego_state/runner.bash"',
  'if (( ${#alego_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$alego_output_marker"',
  'exec env -i -- "${alego_env[@]}" "${alego_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`alego-e2b-bootstrap:${randomUUID()}`)
