...
import ToolRegistry, { defineContentToolFixture } from '@singula-ai/alego-tools'
import { FILE_REFERENCE_PROMPT } from '@singula-ai/alego-file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'alego-file-reference-service-'))
  roots.push(root)
