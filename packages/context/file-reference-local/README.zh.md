...
- [文件系统工具目录](../../../docs/tool-catalog.zh.md#singula-aialego-tool-fs)——发现能力必须匹配其命名空间的 `read` 工具。
