...
| `persona` | 无 | 供 `alego-system-prompt` 使用的部署 persona 模板。 |
| `toolOrder` | 字典序 | 供 `alego-system-prompt` 使用的显式面向模型工具顺序。 |
| `tools` | `{ mode: 'native' }` | Native、Code Mode 或组合式模型工具传输。 |
| `alegoHome` | `$ALEGO_HOME` 或 `~/.alego` | bash 与本地 skill（技能）发现共享的 harness 主目录。 |
...
`alego-acp-demo [--config path-to-cordis.yml]`（短形式 `-c`；默认为 `./cordis.yml`）会加载 gitignore 排除的 `.env`，回放模式除外；`ALEGO_SNAPSHOT=replay` 选择同级 `cordis.snapshot.yml`；stdin EOF 会在退出前 dispose（资源释放）上下文并刷新会话。Loader 已安装的可选对等依赖（peer dependency）`node-addon-require-builtin` 使纯 Node 下构建后的 bin 可以解析裸插件说明符。诊断使用 stderr，因为 stdout 是 ACP wire。

## 模型体验

模型体验由 `alego-agent-spine-demo` 和叶节点的面向模型插件间接提供。ACP 提示词文本会成为普通的已记录用户消息；协议元数据与权限选择不会进入模型请求。
