...
模型通过 `alego-system-prompt`、`alego-tool-skill`、`alego-tool-bash`、`alego-tools` 和 `alego-llm-retry` 间接获得体验；还会通过 `alego-tool-goal` 与 Goal Round 提示词获得体验，前提是启用 `goals`。组合包自身不添加面向模型的包装内容。
