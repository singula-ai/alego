...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldAlegoHome = process.env.ALEGO_HOME
  const oldAgentsHome = process.env.ALEGO_AGENTS_HOME
  process.env.ALEGO_HOME = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-home-'))
  process.env.ALEGO_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldAlegoHome === undefined) {
      delete process.env.ALEGO_HOME
    } else {
      process.env.ALEGO_HOME = oldAlegoHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.ALEGO_AGENTS_HOME
    } else {
      process.env.ALEGO_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('alego-agent-spine-demo bundle', () => {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Alego.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level alegoHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'alego-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'alego-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-alego-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-alego-home'),
      rootCallId: CallId('agent-core-alego-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ ALEGO_HOME: home, ALEGO_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested ALEGO home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        alegoHome: '/global-alego-home',
        workspaceContext: false,
        skills: { filesystem: { alegoHome: '/nested-alego-home' } },
      })
    }).toThrow('agent-spine-demo: alegoHome and skills.filesystem.alegoHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'alego-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
