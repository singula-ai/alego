...
使用 `alego-sandbox`，可以让子进程及其派生的所有进程在逐调用文件访问策略下运行。命令可以禁止写入（`read-only`）、只写入工作区（`workspace-write`），或不受限制地运行（`danger-full-access`）。无法强制执行所请求的模式时，调用以 `SANDBOX_UNAVAILABLE` 失败，绝不会不受限制地运行。调用被拒绝后，模型可以请求一个严格更宽的模式，交由人类批准一次。这种限制只适用于与宿主共享内核和文件系统的进程；需要隔离整个环境时，请使用容器、microVM 或远程执行器。
...
```yaml
- id: sandbox
  name: '@singula-ai/alego-sandbox-local'     # the per-platform backend provider (ctx.sandbox)
- id: sandbox-policy
  name: '@singula-ai/alego-sandbox-policy'    # the deployment default mode and workspace-write root
  config:
...
- id: bash
  name: '@singula-ai/alego-bash-sandbox'      # the confined executor behind ctx.shell
```
...
通过 [`alego-bash-sandbox`](../../shell/bash-sandbox/README.zh.md) 和 [`alego-tool-bash`](../../shell/tool-bash/README.zh.md)，请求的受限模式没有可用后端时会产生错误码 `SANDBOX_UNAVAILABLE` 及下方精确错误；执行期 runner 失败会追加 ` Runner failure: <detail>`。
