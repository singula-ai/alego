 ...
 * execution pipeline.
 * @module @singula-ai/alego-tools
 */

import { Context, Service } from '@singula-ai/cordis'
import z from '@singula-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@singula-ai/alego-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@singula-ai/alego-scope'
import type { CallId, ContentBlock, ToolSchema } from '@singula-ai/alego-llm'
import { assertNever, deepFreeze, HarnessError } from '@singula-ai/alego-llm'
import type { Agent } from '@singula-ai/alego-agent'
import { snapshotJsonValue } from '@singula-ai/alego-session'
import type { JsonValue, UserMessage } from '@singula-ai/alego-session'
import type { ToolProviderResult } from '@singula-ai/alego-system-prompt'
import type { CodeRuntime } from '@singula-ai/alego-code-runtime'
...
export type { JsonValue } from '@singula-ai/alego-session'
...
// lives in its own UI-facing module; re-export it so `@singula-ai/alego-tools`
...
declare module '@singula-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@singula-ai/alego-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@singula-ai/alego-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@singula-ai/alego-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@singula-ai/alego-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@singula-ai/alego-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`alego-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @singula-ai/alego-code-runtime-worker-thread) or set tools mode to "native"`)
    }
