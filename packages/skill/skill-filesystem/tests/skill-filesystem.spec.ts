...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@singula-ai/alego-fs'
...
async function tempDir(name: string): Promise<string> {
  const dir = await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `alego-${name}-`)))
  tempDirs.push(dir)
...
describe('FileSystemSkillProvider', () => {
  ...
  it.each([false, true])('publishes regular-file paths for linked skills while retaining their resource roots (filesystem service: %s)', async (withFileSystem) => {
    ...
    const fiber = ctx.plugin(SkillFileSystem, { alegoHome: join(home, '.alego'), agentsHome: join(home, '.agents'), watch: false })
    await fiber
    ...
    try {
      ...
      for (const name of ['linked-dir', 'linked-flat']) {
        ...
        expect(loaded?.resourceBase).toEqual({ kind: 'directory', path: name === 'linked-dir' ? join(home, '.alego/skills', name) : join(home, '.alego/skills') })
        expect((await lstat(path)).isFile()).toBe(true)
      }
      await writeFile(join(external, 'replacement.md'), '---\nname: linked-flat\ndescription: Replacement\n---\n\nReplacement body.\n')
      await rm(join(home, '.alego/skills/linked-flat.md'))
      await symlink(join(external, 'replacement.md'), join(home, '.alego/skills/linked-flat.md'))
