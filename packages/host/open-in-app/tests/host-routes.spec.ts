...
import type { NativeCommandRunner } from '@singula-ai/alego-native-command'
import {
  createLaunchEnvironmentSnapshot, ALEGO_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentLayerInput,
} from '@singula-ai/alego-launch-environment'
...
async function boot(layers: readonly LaunchEnvironmentLayerInput[] = []): Promise<string> {
  ...
  root = await mkdtemp(join(tmpdir(), 'alego-open-in-app-loader-'))
  ...
  context = new Context()
  context.provide(ALEGO_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot(layers))
  ...
  const modules = new Map<string, unknown>([
    ['@singula-ai/alego-host-webserver', WebServer],
    ['@singula-ai/alego-host-open-in-app', OpenInApp],
  ])
...
describe('open-in-app host routes (real Loader composition)', () => {
  ...
  it('serves the resolved catalog, one cached icon, and launches from the same resolution', async () => {
    ...
    const home = await mkdtemp(join(tmpdir(), 'alego-open-in-app-home-'))
  ...
  it('removes all three routes when the plugin row is disposed (HMR safety)', async () => {
    ...
    const entry = [...(context as Context).loader.entries()]
      .find(candidate => candidate.options.name === '@singula-ai/alego-host-open-in-app')
