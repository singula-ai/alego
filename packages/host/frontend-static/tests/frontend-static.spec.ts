...
import * as Connection from '@singula-ai/alego-client-connection'
import LocalCredentials from '@singula-ai/alego-credentials-local'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'alego-frontend-static-'))
  ...
  const modules = new Map<string, unknown>([
    ['@singula-ai/alego-credentials-local', LocalCredentials],
    ['@singula-ai/alego-host-webserver', HttpServer],
    ['@singula-ai/alego-client-connection', Connection],
    ['@singula-ai/alego-host-frontend-static', FrontendStatic],
  ])
...
describe('real Loader composition', () => {
  it('serves explicit index entries and files while preserving HTTP error semantics', { timeout: 60_000 }, async () => {
    ...
    expect(await request(port, '/')).toMatchObject({
      ...
      body: 'alego web authentication required; reopen the URL printed by alego web.\n',
    })
