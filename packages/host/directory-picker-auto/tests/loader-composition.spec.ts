...
import {
  createLaunchEnvironmentSnapshot, ALEGO_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot,
...
async function loadComposition(
  ...
  context = new Context()
  if (options.launchEnvironment !== undefined) context.provide(ALEGO_LAUNCH_ENVIRONMENT_KEY, options.launchEnvironment)
...
function stubAttendedHost(): void {
  fakeBin = mkdtempSync(join(tmpdir(), 'alego-picker-bin-'))
