...
async function setupWith(adapter: MockAdapter | GatedAdapter, park = true) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'alego-tool-subagent-control-'))
  roots.push(root)
...
describe('alego-tool-subagent-control', () => {
...
describe('alego-tool-subagent-control interrupt_agent', () => {
