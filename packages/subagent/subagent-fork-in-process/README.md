...
Mount this backend when delegated work must build on the parent's conversation. The common path mirrors spawn: load the subagent service and this backend, then point a delegation tool such as `alego-tool-subagent` at the `fork` provider.
...
The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-subagent-fork-in-process) is the exhaustive source for every accepted field and its JSDoc.
...
- [alego-subagent-spawn-in-process](../subagent-spawn-in-process/README.md) — the fresh-child sibling backend.
- [alego-tool-subagent](../tool-subagent/README.md) — the model-facing delegation tool that reaches this provider.
- [Generated configuration catalog](../../../docs/config-catalog.md#singula-aialego-subagent-fork-in-process) — every accepted config field and its source declaration.
