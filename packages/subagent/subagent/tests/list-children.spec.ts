...
} from '@singula-ai/alego-storage-json'
import {
  apply as storageDomainApply, Config as storageDomainConfig, inject as storageDomainInject, name as storageDomainName,
} from '@singula-ai/alego-storage-domain'
...
async function setup(
  ...
  const root = mkdtempSync(join(tmpdir(), 'alego-subagent-list-'))
  roots.push(root)
  ...
  if (options.projectionCache === true) {
    const root = mkdtempSync(join(tmpdir(), 'alego-subagent-projcache-'))
