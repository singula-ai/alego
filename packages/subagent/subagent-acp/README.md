...
The generated [configuration catalog](../../../docs/config-catalog.md#singula-aialego-subagent-acp) is the exhaustive source for every accepted field and its JSDoc.

An Alego child uses the product launcher and an explicit absolute `ALEGO_HOME`. The isolated home prevents a nested runtime from discovering the launching person's profiles or credentials; the generic ACP provider does not impose this requirement on non-ALEGO agents.
...
- id: subagent-acp
  ...
  config:
    ...
    permission: reject
    env:
      ALEGO_HOME: /absolute/path/to/isolated-child-home
...
- [alego-subprocess seam](../../subprocess/subprocess/README.md) — the process-spawn and teardown machinery behind each run.
- [Generated configuration catalog](../../../docs/config-catalog.md#singula-aialego-subagent-acp) — every accepted config field and its source declaration.
