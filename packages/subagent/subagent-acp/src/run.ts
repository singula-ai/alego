...
import type { SubprocessHandle, SubprocessOutcome, SubprocessSpawnSpec } from '@singula-ai/alego-subprocess'
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  const clientApp = createAcpClientApp({ name: 'alego-subagent-acp' })
