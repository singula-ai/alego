...
- [Generated configuration catalog](../../../docs/config-catalog.md#singula-aialego-subagent-codex) — every accepted config field and its source declaration.
...
- **No human approval path** — known unattended approval requests are denied and unknown server requests fail closed; the three Profile modes never create a ALEGO interaction channel or per-call allow policy.
