...
export const name = 'subagent-alego-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('alego-sdk'),
  alegoBin: z.string(),
  profile: z.string().default('sdk'),
  patches: z.array(z.string()).default([]),
  alegoHome: z.string().required(),
  cwd: z.string(),
...
type ResolvedConfig = Required<Omit<Config, 'cwd' | 'maxTokens' | 'alegoBin'>> & Pick<Config, 'cwd' | 'maxTokens' | 'alegoBin'>
...
function resolveConfiguredFile(field: string, value: string): string {
  ...
  throw new TypeError(`subagent-alego-sdk ${field} must name an existing file: ${path}`)
}

/** ALEGO SDK can apply Agent route options while the other start features remain child-owned. */
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    ...
    let cwd: string
    try {
      cwd = resolveChildCwd('subagent-alego-sdk', this.config.cwd, request.parent.session.header.cwd)
    } catch (error: unknown) {
      const failure = sdkConfigurationFailure(error)
      this.ctx.logger.warn(`subagent-alego-sdk "${this.name}": child start failed: %o`, error)
      throw failure
    }
    const route = resolveSdkRoute(this.config, request.agentOptions)
    const spec: SdkRunSpec = {
      ...this.config.alegoBin === undefined ? {} : { alegoBin: this.config.alegoBin },
      profile: this.config.profile,
      patches: this.config.patches,
      alegoHome: this.config.alegoHome,
      cwd,
      ...route,
      env: this.config.env,
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-alego-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-alego-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-alego-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-alego-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-alego-sdk maxTokens must be a positive safe integer')
  }
  if (!isAbsolute(resolved.alegoHome)) throw new TypeError('subagent-alego-sdk alegoHome must be an absolute path')
  const launchPaths: ResolvedConfig = {
    ...resolved,
    patches: resolved.patches.map((path, index) => resolveConfiguredFile(`patches[${String(index)}]`, path)),
    ...resolved.alegoBin === undefined ? {} : { alegoBin: resolveConfiguredFile('alegoBin', resolved.alegoBin) },
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-alego-sdk', resolved.cwd)
