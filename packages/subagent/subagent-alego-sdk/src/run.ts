...
import type { ContentBlock, ReasoningEffortId } from '@singula-ai/alego-llm'
import type { SessionEvent, SessionId, TurnEndReason } from '@singula-ai/alego-session'
...
export interface SdkRunSpec {
  /** Explicit alego CLI module; omission resolves the SDK client's same-version dependency. */
  alegoBin?: string
  /** Named child profile. */
  profile: string
   ...
   * current `ALEGO_*` fact survives while ambient namesakes never leak.
   */
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-alego-sdk: ${failureDiagnostic(facts)}`, { cause })
    this.name = 'SdkRunFailure'
  }
}

/** Runtime constructor seam replaced only by package-local fake-runtime tests. */
export const internals: { createHarness(options: AlegoOptions): Alego } = {
  createHarness: options => new Alego(options),
}
 ...
 * @returns an Error whose message contains only the fixed ALEGO SDK failure line.
 */
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = internals.createHarness({
    ...spec.alegoBin === undefined ? {} : { alegoBin: spec.alegoBin },
    profile: spec.profile,
    patches: spec.patches,
    alegoHome: spec.alegoHome,
    processCwd: spec.cwd,
