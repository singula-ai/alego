...
const configPath = fileURLToPath(new URL('alego-sdk.patch.yml', fixtureDir))
...
async function childLaunch(failure = false): Promise<{
  ...
  const childHome = await mkdtemp(join(tmpdir(), 'alego-sdk-subagent-home-'))
  ...
  return {
    ...
    env: {
      ...
      ...(failure ? { ALEGO_TEST_CHILD_FAILURE: '1' } : {}),
    },
  }
}
...
describe('SDK subagent routing and diagnostics through the production profile', () => {
  it('runs the selected child route in the parent session workspace', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'alego-sdk-subagent cwd composition smoke',
        tempDirPrefix: 'alego-sdk-subagent-cwd-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        ...
        env: {
          ...child.env,
          ALEGO_TEST_CHILD_DEFAULT_ROUTE: '1',
          ALEGO_TEST_PARENT_MODEL_RECORD: '.parent-model-routes',
        },
        inspect: async (cwd) => {
  ...
  it('presents the child error diagnostic separately from partial output', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'alego-sdk-subagent diagnostic composition smoke',
        tempDirPrefix: 'alego-sdk-subagent-diagnostic-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        tsconfigPath: repoTsconfig,
      ...
      expect(toolResultText(events)).toBe(
        ...
        + 'Diagnostic: Subagent failure (provider: ALEGO SDK; stage: session-run; category: child-error)\n'
