...
The id lives in `$ALEGO_HOME/.anonymous-user-id` (`$ALEGO_HOME` defaults to `~/.alego`) as a plain UUID text file. Delete that file to get a fresh id at the next launch; the running process keeps its current id until it exits. Separate harness homes keep separate ids, and no machine or account detail ever goes into the value.
...
- [alego-llm-deepseek](../../llm/llm-deepseek/README.md) — sends `x-deepseek-harness-user-id` on provider requests.
