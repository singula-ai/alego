...
- **DeepSeek requests** — every provider request carries the `x-dsh-user-id` header, so usage can be attributed per installation.

### Observing and resetting the id

The id lives in `$ALEGO_HOME/.anonymous-user-id` (`~/.alego` by default) as a plain UUID text file. Delete that file to get a fresh id at the next launch; the running process keeps its current id until it exits. Separate harness homes keep separate ids, and no machine or account detail ever goes into the value.
...
- [alego-home-paths](../../util/home-paths/README.md) — owns `$ALEGO_HOME` and `~/.alego` resolution.
- [alego-session-telemetry-otel](../../session/session-telemetry-otel/README.md) — reports the id as the OTel Resource `user.id`.
- [alego-command-feedback](../../feedback/command-feedback/README.md) — embeds the id in the feedback acknowledgement.
- [alego-llm-deepseek](../../llm/llm-deepseek/README.md) — sends `x-dsh-user-id` on provider requests.
...
- **No cross-home identity** — different `$ALEGO_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `alego-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
