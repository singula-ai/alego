/** Recorded-session replay through the shipped headless `alego` profile. */
...
import { releasedV0SessionFormatCodec } from '@singula-ai/alego-session-format-v0-to-v1'
import type { SessionFormatEvent, SessionFormatMigrationContext } from '@singula-ai/alego-session-format'
...
import { parseSessionLog, prepareSessionSnapshotFixtureForComparison } from '@singula-ai/alego-llm-replay'

const repoRoot = fileURLToPath(new URL('../../', import.meta.url))
const snapshotsRoot = fileURLToPath(new URL('./', import.meta.url))
const alegoBin = join(repoRoot, 'apps/cli/src/bin.ts')
...
const mode = snapshotMode(process.env.ALEGO_SNAPSHOT)
const RUNTIME_WORKSPACE_ENTRIES = ['.agents', '.alego', '.snapshot-patches'] as const
...
async function persistedSessions(cwd: string): Promise<SessionLog[]> {
  const root = join(cwd, '.alego', 'sessions')
...
function stderrFromSession(log: string): string {
  ...
  return `${output}alego: ${error.code}: ${error.message}\n`
}
...
const workspaceSetups: Record<string, (cwd: string) => Promise<void>> = {
  async 'editing-cordis-skill'(cwd) {
    const target = join(cwd, '.alego', 'skills', 'editing-cordis-compositions', 'SKILL.md')
...
describe('headless recorded-session snapshots', () => {
  ...
  it('replays original inbox mentions before normalized user messages', () => {
    ...
    const original = 'Use @[Research](alego-session:InJlZmVyZW5jZS1zb3VyY2Ui)'
    const log = [
  ...
  it.each(['assistant/message', 'assistant/attempt'] as const)(
    ...
    (eventType) => {
      ...
      expect(stderrFromSession(log)).toBe('alego: reasoning:\nfirst thought\n')
    },
  )
  ...
  it.each([10, 20])('assigns sibling roles by catalog order when the first child timestamp is %i', async (firstCreatedAt) => {
    const cwd = await mkdtemp(join(tmpdir(), 'alego-headless-catalog-order-'))
    try {
      const logs = [
        [
      ...
      for (const content of logs) {
        const directory = join(cwd, '.alego', 'sessions', String(headerOf(content).id))
  ...
  it('writes header sidecars without replacing a retained Session generation', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'alego-headless-sidecars-'))
    try {
      ...
      const content = [
        ...
        { type: 'system/message', seq: 2, time: 3, data: {
          ...
          message: { role: 'system', content: [{ type: 'text', text: 'fresh system prompt' }],
            source: { kind: 'plugin', plugin: '@singula-ai/alego-system-prompt' }, id: 'fresh-msg' },
        }, surfaceOp: 'append' },
        {
          type: 'request/header',
          seq: 3,
  ...
  for (const scenario of scenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through alego --profile headless`, async () => {
      ...
      try {
        result = await runLoaderSmoke({
          ...
          expectedExitCode: turnReasonFromSession(primaryFixture)?.kind === 'completed'
            ...
            : 1,
          env: {
            ALEGO_SNAPSHOT: replaying ? 'replay' : 'record',
            ALEGO_SNAPSHOT_PROVIDER: model.provider,
            ALEGO_SNAPSHOT_MODEL: model.model,
            ALEGO_SNAPSHOT_SPILL_ROOT: spillRoot,
            ALEGO_SNAPSHOT_SPILL_LOCATOR_ROOT: locatorRoot,
            ALEGO_SNAPSHOT_FILE: join(scenario.dir, fixtureFiles[0] as string),
            ...(replaying && fixtureFiles.length > 1
              ? { ALEGO_SNAPSHOT_CHILD_FILES: fixtureFiles.slice(1).map(file => join(scenario.dir, file)).join(delimiter) }
              : {}),
            ...
            ...(scenario.manifest.permission === undefined
              ? {}
              : { ALEGO_PERMISSION_MODE: scenario.manifest.permission }),
