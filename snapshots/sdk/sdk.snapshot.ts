 ...
 * the real `alego --profile sdk` runtime through
 * `@singula-ai/alego-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `ALEGO_SNAPSHOT=record`
 * re-records against the live API; `ALEGO_SNAPSHOT=refresh` replays committed
...
const mode = process.env.ALEGO_SNAPSHOT ?? 'replay'
...
const RUNTIME_WORKSPACE_ENTRIES = [
  ...
  '.child-alego',
  '.alego',
  '.alego-sdk-background-release',
  '.replay-fixtures',
  '.snapshot-patches',
] as const
const alegoSdkDiagnosticChildPatch = fileURLToPath(new URL(
  './subagent-alego-sdk-diagnostic/child.cordis.yml',
  import.meta.url,
))
const alegoSdkChildConfig = fileURLToPath(new URL(
  '../../packages/subagent/subagent-alego-sdk/tests/fixtures/loader/child.patch.yml',
  import.meta.url,
))
...
interface SdkAssertions {
  ...
  /** A separate ALEGO SDK child whose persisted session joins the evidence. */
  alegoSdkChild?: {
...
const SDK_ASSERTIONS: Readonly<Record<string, SdkAssertions>> = {
  ...
  'subagent-continuable': {
    environment: { ALEGO_SNAPSHOT_HUMAN_STEER: '1' },
  },
  'subagent-alego-sdk-diagnostic': {
    environment: { ALEGO_TEST_CHILD_PATCH: alegoSdkDiagnosticChildPatch },
  },
  'persistent-tools': {
    environment: { ALEGO_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
    ...
    runtimeContext: {
      includes: ['Current ALEGO file policy: danger-full-access', 'Approval prompts are disabled in this session'],
      excludes: ['workspace-write'],
    },
  },
  'subagent-alego-sdk-dynamic-route': {
    environment: { ALEGO_TEST_PARENT_PROVIDER: 'deepseek-official' },
    alegoSdkChild: {
      config: alegoSdkChildConfig,
      agentConfig: {
        provider: 'mock',
        model: 'mock-routed',
        reasoningEffort: 'max',
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@singula-ai/alego-system-prompt') return []
...
/** One SDK-controlled recorded scenario against a fresh `alego --profile sdk` subprocess. */
async function runScenario(scenario: CorpusScenario): Promise<{
  ...
  const alegoHome = join(cwd, '.alego')
  const sessionsRoot = join(alegoHome, 'sessions')
  ...
  if (assertions.alegoSdkChild !== undefined) {
    const childHome = join(cwd, '.child-alego')
    const childPatch = materializeProfilePatch(assertions.alegoSdkChild.config, cwd, patchRoot, patches.length)
    await mkdir(childHome, { recursive: true })
    childSessionsRoot = join(childHome, 'sessions')
    childEnvironment = {
      ALEGO_TEST_CHILD_PATCHES: JSON.stringify([childPatch]),
      ALEGO_TEST_CHILD_HOME: childHome,
    }
  }
  ...
  const env: Record<string, string> = {
    ...
    ALEGO_SNAPSHOT: mode,
    ALEGO_SNAPSHOT_PROVIDER: route.provider,
    ALEGO_SNAPSHOT_MODEL: route.model,
    ALEGO_TELEMETRY_DISABLED: '1',
    ALEGO_AGENTS_HOME: join(cwd, '.agents'),
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      ALEGO_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { ALEGO_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
    ...!recording && scenario.manifest.replay?.override === true
      ? { ALEGO_SNAPSHOT_OVERRIDE: join(scenario.dir, 'replay.override.json') }
      : {},
  ...
  try {
    ...
    try {
      ...
      for (const action of turnActions(primaryFixture)) {
        ...
        results.push(result)
        if (scenario.manifest.environment?.ALEGO_SNAPSHOT_FEEDBACK === '1') {
...
function orderLogs(logs: PersistedLog[], expectedCount: number, separateAlegoSdkChild: boolean): PersistedLog[] {
  if (separateAlegoSdkChild) {
...
async function verifyHeaders(
  ...
  ordered: readonly PersistedLog[],
  ctx: NormalizeContext,
  alegoSdkChildConfig?: Readonly<Record<string, unknown>>,
): Promise<void> {
  ...
  for (const [logIndex, log] of ordered.entries()) {
    ...
    for (const [index, header] of headers.entries()) {
      ...
      const configured = logIndex === 1 && alegoSdkChildConfig !== undefined
        ? { ...base as JsonObject, config: alegoSdkChildConfig }
        : base
...
describe('TypeScript SDK snapshots over the jsonrpc runtime', () => {
  for (const scenario of sdkScenarios) {
    const scenarioTest = recording
      ...
      ? it.skip
      : it
    scenarioTest(`${mode}s ${scenario.name} through alego --profile sdk`, async () => {
      ...
      const ordered = orderLogs(
        ...
        assertions.alegoSdkChild !== undefined,
      )
      ...
      await verifyHeaders(scenario, ordered, actualContext, assertions.alegoSdkChild?.agentConfig)
      ...
      if (ordered.length > 1 && assertions.alegoSdkChild === undefined) {
