...
| `ctx.agentLoop` | `bundle` | [`agent-loop`](../packages/core/agent-loop) | - | [`agent-spine-demo`](../packages/examples/agent-spine-demo) | - | 唯一的具体循环插件；扩展包依赖 alego-agent 的事件和服务，而不依赖此包。 |
...
| `ctx.clientModules` | `core` | `modules` | - | `hmr` | - | 通过增量 `alego.client` 扫描组合 __ALEGO_BOOT__ 入口图，提供插件组合包，并通知重建／图变更订阅方。 |
