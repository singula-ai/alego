...
- **Service Definition** (`alego-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`alego-bash-local`) — executes commands on the local machine
- **Consumer** (`alego-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  alego-shell   │────▶│  alego-bash-local  │     │ alego-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@singula-ai/alego-my-cap'
...
```yaml
- name: '@singula-ai/alego-my-cap-local'
- name: '@singula-ai/alego-tool-my-cap'
```
