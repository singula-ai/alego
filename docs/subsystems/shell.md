...
The bash execution seam is split across a Service Definition ([alego-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([alego-bash-local](../../packages/shell/bash-local) and [alego-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([alego-tool-bash](../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Managed-range mechanics live behind the [subprocess seam](subprocess.md).
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `alego-tool-bash`. The local executor scrubs ambient credentials before merging explicit caller-supplied env.
...
`start()` returns a handle with no id or owner. `alego-tool-bash` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the underlying process settles and never rejects; a subprocess provider rejection becomes a `killed` process with a stage-neutral error on stderr. Reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
