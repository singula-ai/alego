...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'ALEGO_SNAPSHOT_MAX_CONCURRENCY',
...
if (process.env.ALEGO_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.ALEGO_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.ALEGO_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
